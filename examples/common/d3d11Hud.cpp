...
static const char *s_VS = "cbuffer cbPerFrame : register( b0 )\n"
                          "{\n"
...
D3D11hud::D3D11hud(ID3D11DeviceContext *deviceContext) : _deviceContext(deviceContext), _vbo(0), _staticVbo(0), _fontTexture(0), _inputLayout(0), _shaderResourceView(0), _samplerState(0), _vertexShader(0), _pixelShader(0), _rasterizerState(0) {}
...
void D3D11hud::Init(int width, int height, int frameBufferWidth, int frameBufferHeight)
{
    ...
    D3D11_INPUT_ELEMENT_DESC inputElementDesc[] = {{"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},
...
void D3D11hud::Rebuild(int width, int height, int framebufferWidth, int framebufferHeight)
{
