...
D3D11PtexMipmapTexture::D3D11PtexMipmapTexture() : _width(0), _height(0), _depth(0), _layout(0), _texels(0), _layoutSRV(0), _texelsSRV(0), _memoryUsage(0) {}
...
/*static*/
const char *D3D11PtexMipmapTexture::GetShaderSource()
{
...
static ID3D11Buffer *genTextureBuffer(ID3D11DeviceContext *deviceContext, int size, void const *data)
{
    ...
    if (FAILED(hr))
    {
        OpenSubdiv::Far::Error(OpenSubdiv::Far::FAR_RUNTIME_ERROR, "Fail in CreateBuffer\n");
        return 0;
    }

    D3D11_MAPPED_SUBRESOURCE resource;
    hr = deviceContext->Map(buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
    if (FAILED(hr))
    {
        OpenSubdiv::Far::Error(OpenSubdiv::Far::FAR_RUNTIME_ERROR, "Fail in Map buffer\n");
        buffer->Release();
        return 0;
    }
...
D3D11PtexMipmapTexture *D3D11PtexMipmapTexture::Create(ID3D11DeviceContext *deviceContext, PtexTexture *reader, int maxLevels, size_t targetMemory)
{
    ...
    int numChannels = reader->numChannels() + padAlpha, numFaces = loader.GetNumFaces();

    ID3D11Buffer *layout = genTextureBuffer(deviceContext, numFaces * 6 * sizeof(short), loader.GetLayoutBuffer());
    if (!layout)
        return NULL;
