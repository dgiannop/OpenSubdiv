...
inline void cross(float *n, const float *v1, const float *v2)
{
...
inline void cross(float *n, const float *p0, const float *p1, const float *p2)
{
...
inline void multMatrix(float *d, const float *a, const float *b)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            d[i * 4 + j] = a[i * 4 + 0] * b[0 * 4 + j] + a[i * 4 + 1] * b[1 * 4 + j] + a[i * 4 + 2] * b[2 * 4 + j] + a[i * 4 + 3] * b[3 * 4 + j];
        }
    }
}

inline void inverseMatrix(float *d, const float *m)
{
...
inline void perspective(float *m, float fovy, float aspect, float znear, float zfar)
{
...
inline void ortho(float *m, float left, float top, float right, float bottom)
{
    identity(m);
    m[0]  = 2.0f / (right - left);
    m[5]  = 2.0f / (top - bottom);
...
inline void apply(float *v, const float *m)
{
    float r[4];
...
inline void pickMatrix(float *m, float x, float y, float width, float height, const int *viewport)
{
    float sx, sy;
    float tx, ty;
