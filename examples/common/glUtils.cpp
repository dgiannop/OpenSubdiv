...
void InitializeGL() { OpenSubdiv::internal::GLLoader::applicationInitializeGL(); }

static void _argParseBool(bool *ret, const char *lbl, int i, int argc, char **argv)
{
    if (i < argc - 1)
    {
    ...
    {
        fprintf(stderr, "Please specify \"on\" or \"off\" for %s.\n", lbl);
        exit(1);
    }
}

void SetMinimumGLVersion(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (!strcmp(argv[i], "-glVersion"))
        {
            if (i < argc - 1)
            {
                ...
                if (len == 3 && versionStr[1] == '.' && versionStr[0] >= '0' && versionStr[0] <= '9' && versionStr[2] >= '0' && versionStr[2] <= '9')
                {
            ...
            {
                fprintf(stderr, "Please specify a version number for glVersion "
...
// opengl string
std::string GetShaderVersionInclude() { return "#version " + GetShaderVersion() + "\n"; }
