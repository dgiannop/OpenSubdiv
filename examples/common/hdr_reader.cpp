...
static bool readLine(unsigned char *line, int length, FILE *fp)
{
...
unsigned char *loadHdr(const char *filename, HdrInfo *info, bool convertToFloat)
{
    ...
    if (strncmp(info->format, "32-bit_rle_rgbe", 15))
        goto error;
        ...
        for (int i = 1; i < n; ++i)
        {
            if (buffer[i] == 'X')
            {
                if (!(info->flag & HDR_Y_MAJOR))
                    info->flag |= HDR_X_MAJOR;
                info->flag |= (char)((buffer[i - 1] == '-') ? HDR_X_DEC : 0);
                info->width = atoi(&buffer[i + 1]);
            }
            else if (buffer[i] == 'Y')
            {
                if (!(info->flag & HDR_X_MAJOR))
                    info->flag |= HDR_Y_MAJOR;
    ...
    {
        if (!readLine(line, info->scanLength, fp))
            goto error;
