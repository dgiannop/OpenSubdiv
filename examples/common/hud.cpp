...
Hud::Hud() : _visible(true), _windowWidth(0), _windowHeight(0), _framebufferWidth(0), _framebufferHeight(0), _requiresRebuildStatic(true) { _capturedSlider = -1; }

Hud::~Hud() {}

void Hud::Init(int width, int height, int framebufferWidth, int framebufferHeight)
{
    _windowWidth       = width;
...
int Hud::GetWidth() const { return _windowWidth; }

int Hud::GetHeight() const { return _windowHeight; }
...
bool Hud::KeyDown(int key)
{
    for (std::vector<RadioButton>::iterator it = _radioButtons.begin(); it != _radioButtons.end(); ++it)
    {
        if (key == it->shortcut)
        {
            ...
            if (it->sharedShortcut)
            {
                ...
                int maxLocalIndex = 0;
                for (std::vector<RadioButton>::iterator it2 = _radioButtons.begin(); it2 != _radioButtons.end(); ++it2)
                {
                ...
                if (nextLocalIndex > maxLocalIndex)
                    nextLocalIndex = 0;
            }
            for (std::vector<RadioButton>::iterator it2 = _radioButtons.begin(); it2 != _radioButtons.end(); ++it2)
            {
    ...
    for (std::vector<CheckBox>::iterator it = _checkBoxes.begin(); it != _checkBoxes.end(); ++it)
    {
    ...
    for (std::vector<PullDown>::iterator it = _pulldowns.begin(); it != _pulldowns.end(); ++it)
    {
        if (key == it->shortcut)
        {
...
bool Hud::MouseClick(int x, int y)
{
    if (!IsVisible())
    {
        return false;
...
bool Hud::MouseCapture() const { return _capturedSlider != -1; }

void Hud::MouseRelease() { _capturedSlider = -1; }

void Hud::AddLabel(const char *label, int x, int y)
{
    Item l;
    l.label = label;
    l.x     = x;
...
void Hud::AddCheckBox(const char *label, bool checked, int x, int y, CheckBoxCallback callback, int data, int shortcut)
{
    CheckBox cb;
    cb.label        = label;
    cb.checked      = checked;
...
void Hud::AddRadioButton(int group, const char *label, bool checked, int x, int y, RadioButtonCallback callback, int data, int shortcut)
{
    RadioButton rb;
    rb.group          = group;
    rb.label          = label;
...
void Hud::AddSlider(const char *label, float min, float max, float value, int x, int y, int width, bool intStep, SliderCallback callback, int data)
{
    Slider slider;
    slider.label        = label;
    slider.x            = x;
...
int Hud::AddPullDown(const char *label, int x, int y, int width, PullDownCallback callback, int shortcut)
{
...
void Hud::AddPullDownButton(int handle, const char *label, int value, bool checked)
{
...
int Hud::drawChar(std::vector<float> &vboSource, int x, int y, float r, float g, float b, char ch)
{
...
int Hud::drawString(std::vector<float> &vboSource, int x, int y, float r, float g, float b, const char *c)
{
    while (*c)
    {
        char ch = (char)((*c) & 0x7f);
...
void Hud::DrawString(int x, int y, const char *fmt, ...)
{
    char    buf[1024];
    va_list args;
    va_start(args, fmt);
...
void Hud::DrawString(int x, int y, float r, float g, float b, const char *fmt, ...)
{
    char    buf[1024];
    va_list args;
    va_start(args, fmt);
...
void Hud::Rebuild(int width, int height, int framebufferWidth, int framebufferHeight)
{
    ...
    int x, y;
    // add UI elements
    for (std::vector<Item>::const_iterator it = _labels.begin(); it != _labels.end(); ++it)
    {
        getWindowPos(*it, &x, &y);
        drawString(_staticVboSource, x, y, 1, 1, 1, it->label.c_str());
    }
    // draw radio buttons
    for (std::vector<RadioButton>::const_iterator it = _radioButtons.begin(); it != _radioButtons.end(); ++it)
    {
        getWindowPos(*it, &x, &y);
        if (it->checked)
        {
    ...
    // draw checkboxes
    for (std::vector<CheckBox>::const_iterator it = _checkBoxes.begin(); it != _checkBoxes.end(); ++it)
    {
        getWindowPos(*it, &x, &y);
        if (it->checked)
        {
    ...
    // draw sliders
    for (std::vector<Slider>::const_iterator it = _sliders.begin(); it != _sliders.end(); ++it)
    {
        getWindowPos(*it, &x, &y);
        int sx = x;
    ...
    // draw pulldowns
    for (std::vector<PullDown>::const_iterator it = _pulldowns.begin(); it != _pulldowns.end(); ++it)
    {
        getWindowPos(*it, &x, &y);
    ...
    drawString(_staticVboSource, _windowWidth - 128, _windowHeight - 44, .5, .5, .5, "\x06\x07\x08\x09");
    drawString(_staticVboSource, _windowWidth - 128, _windowHeight - 28, .5, .5, .5, "\x16\x17\x18\x19");
    drawString(_staticVboSource, _windowWidth - 92, _windowHeight - 36, .5, .5, .5, "\x0a\x0b\x0c\x0d");
    drawString(_staticVboSource, _windowWidth - 58, _windowHeight - 36, .5, .5, .5, "\x1a\x1b\x1c\x1d\x1e\x1f");
}
...
bool Hud::IsVisible() const { return _visible; }

void Hud::SetVisible(bool visible) { _visible = visible; }

std::vector<float> &Hud::getVboSource() { return _vboSource; }

std::vector<float> &Hud::getStaticVboSource() { return _staticVboSource; }
