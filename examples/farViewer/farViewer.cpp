...
//------------------------------------------------------------------------------
int g_level = 3, g_currentShape = 7;
...
// GUI variables
int g_fullscreen = 0, g_mbutton[3] = {0, 0, 0}, g_running = 1;

int g_displayPatchColor = 1, g_FarDrawMode = kDRAW_FACES, g_FarDrawVertIDs = false, g_FarDrawEdgeIDs = false, g_FarDrawFaceIDs = false, g_FarDrawPtexIDs = false, g_FarDrawEdgeSharpness = false, g_FarDrawGregogyBasis = false, g_FarDrawFVarVerts = false,
    g_FarDrawFVarPatches = false, g_FarDrawFVarPatchTess = 5, g_numPatches = 0, g_maxValence = 0, g_currentPatch = 0, g_Adaptive = true, g_useStencils = true;
...
GLuint g_transformUB = 0, g_lightingUB = 0;
...
static void createVertNumbers(OpenSubdiv::Far::TopologyRefiner const &refiner, std::vector<Vertex> const &vertexBuffer)
{

    int maxlevel = refiner.GetMaxLevel(), firstvert = 0;
...
static void createEdgeNumbers(OpenSubdiv::Far::TopologyRefiner const &refiner, std::vector<Vertex> const &vertexBuffer, bool ids = false, bool sharpness = false)
{
...
static void createFaceNumbers(OpenSubdiv::Far::TopologyRefiner const &refiner, std::vector<Vertex> const &vertexBuffer)
{
...
static void createPatchNumbers(OpenSubdiv::Far::PatchTable const &patchTable, std::vector<Vertex> const &vertexBuffer)
{

    if (!g_currentPatch)
        return;

    int patchID = g_currentPatch - 1, patchArray = -1;
    ...
    OpenSubdiv::Far::ConstIndexArray const cvs = patchTable.GetPatchVertices(patchArray, patchID);
...
static void createFVarPatchNumbers(OpenSubdiv::Far::PatchTable const &patchTable, std::vector<Vertex> const &fvarBuffer)
{
    ...
    if (patch >= 0 && patch < patchTable.GetNumPatchesTotal())
    {
        ...
        OpenSubdiv::Far::ConstIndexArray const cvs = patchTable.GetPatchFVarValues(handle, channel);
...
static GLMesh fvarVerts, fvarWire;

static void createFVarPatches(OpenSubdiv::Far::TopologyRefiner const &refiner, OpenSubdiv::Far::PatchTable const &patchTable, std::vector<Vertex> const &fvarBuffer)
{
    ...
    if (g_FarDrawFVarPatches)
    {
        ...
        int tessFactor = g_FarDrawFVarPatchTess, npatches = patchTable.GetNumPatchesTotal(), nvertsperpatch = (tessFactor) * (tessFactor), nverts = npatches * nvertsperpatch;
...
static void createGregoryBasis(OpenSubdiv::Far::PatchTable const &patchTable, std::vector<Vertex> const &vertexBuffer)
{
    ...
    for (int array = 0; array < (int)patchTable.GetNumPatchArrays(); ++array)
    {
        if (patchTable.GetPatchArrayDescriptor(array).GetType() == PatchDescriptor::GREGORY_BASIS)
        {
    ...
    for (int patch = 0; patch < npatches; ++patch)
    {
        ...
        static int basisedges[40] = {0, 1, 0, 2, 1, 3, 2, 4, 5, 6, 5, 7, 6, 8, 7, 9, 10, 11, 10, 12, 11, 13, 12, 14, 15, 16, 15, 17, 16, 18, 17, 19, 1, 7, 6, 12, 11, 17, 16, 2};

        int offset = patch * 20, *vpe = &vertsperedge[offset], *indices = &edgeindices[patch * 40];

        OpenSubdiv::Far::ConstIndexArray const cvs = patchTable.GetPatchVertices(patchArray, patch);
    ...
    GLMesh::Options options;
    gregoryWire.Initialize(options, (int)vertexBuffer.size(), (int)vertsperedge.size(), &vertsperedge[0], &edgeindices[0], (float const *)&vertexBuffer[0]);
}

//------------------------------------------------------------------------------
// generate display IDs for Far faces
static void createPtexNumbers(OpenSubdiv::Far::PatchTable const &patchTable, std::vector<Vertex> const &vertexBuffer)
{
    ...
    static int regular[4] = {5, 6, 9, 10}, gregory[4] = {0, 1, 2, 3};
    ...
    for (int array = 0; array < (int)patchTable.GetNumPatchArrays(); ++array)
    {
        ...
        for (int patch = 0; patch < (int)patchTable.GetNumPatches(array); ++patch)
        {
            ...
            OpenSubdiv::Far::ConstIndexArray const cvs = patchTable.GetPatchVertices(array, patch);
...
//------------------------------------------------------------------------------
static void createFarGLMesh(Shape *shape, int maxlevel)
{
    ...
    if (g_useStencils)
    {
        ...
        {
            if (Far::StencilTable const *stencilTableWithLocalPoints = Far::StencilTableFactory::AppendLocalPointStencilTable(*refiner, stencilTable, patchTable->GetLocalPointStencilTable()))
            {
                delete stencilTable;
    ...
    glBufferData(GL_ARRAY_BUFFER, 3 * sizeof(float) * vertexBuffer.size(), (GLfloat *)&vertexBuffer[0], GL_STATIC_DRAW);
...
//------------------------------------------------------------------------------
static void createMeshes(ShapeDesc const &desc, int maxlevel)
{
...
static void display()
{
    ...
    g_controlMeshDisplay.Draw(g_controlMeshDisplayVBO, 3 * sizeof(float), g_transformData.ModelViewProjectionMatrix);
    ...
    {
    case kDRAW_VERTICES:
        comp = GLMesh::COMP_VERT;
        break;
    case kDRAW_WIREFRAME:
        comp = GLMesh::COMP_EDGE;
        break;
    case kDRAW_FACES:
        comp = GLMesh::COMP_FACE;
        break;
    default:
        assert(0);
    }
    ...
    if (g_hud.IsVisible())
    {
        ...
        { // display selected patch info
            static char const *patchTypes[13] = {"undefined", "points", "lines", "quads", "tris", "loop", "regular", "single crease", "boundary", "corner", "gregory", "gregory-boundary", "gregory-basis"}, *format0 = "Current Patch : %d/%d (%s - %d CVs)",
            ...
            if (g_Adaptive && g_currentPatch)
            {
                ...
                if (g_FarDrawFVarPatches || g_FarDrawFVarVerts)
                {
                    ...
                    g_hud.DrawString(g_width / 2 - 200, 225, format1, g_currentPatch - 1, g_numPatches - 1, patchTypes[g_currentPatchDesc.GetType()], g_currentPatchDesc.GetNumControlVertices(), patchTypes[g_currentFVarPatchType],
                                     OpenSubdiv::Far::PatchDescriptor::GetNumFVarControlVertices(g_currentFVarPatchType));
                }
                else
                {
                    g_hud.DrawString(g_width / 2 - 200, 225, format0, g_currentPatch - 1, g_numPatches - 1, patchTypes[g_currentPatchDesc.GetType()], g_currentPatchDesc.GetNumControlVertices(), patchTypes[g_currentFVarPatchType]);
                }
            }
        }
...
//------------------------------------------------------------------------------
static void rebuildOsdMeshes() { createMeshes(g_shapes[g_currentShape], g_level); }
...
static void keyboard(GLFWwindow *, int key, int /* scancode */, int event, int /* mods */)
{
    ...
    case ']':
        if (g_currentPatch < g_numPatches)
        {
            ++g_currentPatch;
            rebuildOsdMeshes();
        }
...
static void callbackUseStencils(bool checked, int /* a */)
{
...
static void callbackFarDrawMode(int m) { g_FarDrawMode = m; }

static void callbackDrawIDs(bool checked, int button)
{
    ...
    case 0:
        g_FarDrawVertIDs = checked;
        break;
    case 1:
        g_FarDrawEdgeIDs = checked;
        break;
    case 2:
        g_FarDrawFaceIDs = checked;
        break;
    case 3:
        g_FarDrawPtexIDs = checked;
        break;
    case 4:
        g_FarDrawEdgeSharpness = checked;
        break;
    case 5:
        g_FarDrawGregogyBasis = checked;
        break;

    case 6:
        g_FarDrawFVarVerts = checked;
        break;
    case 7:
        g_FarDrawFVarPatches = checked;
        break;
...
static void callbackScale(float value, int) { g_font->SetFontScale(value); }

static void callbackFVarTess(float value, int)
{
...
static void callbackFVarInterpolation(int b)
{
    ...
    case SdcOptions::FVAR_LINEAR_NONE:
        g_fvarInterpolation = SdcOptions::FVAR_LINEAR_NONE;
        break;
    ...
    case SdcOptions::FVAR_LINEAR_CORNERS_PLUS1:
        g_fvarInterpolation = SdcOptions::FVAR_LINEAR_CORNERS_PLUS1;
        break;

    case SdcOptions::FVAR_LINEAR_CORNERS_PLUS2:
        g_fvarInterpolation = SdcOptions::FVAR_LINEAR_CORNERS_PLUS2;
        break;

    case SdcOptions::FVAR_LINEAR_BOUNDARIES:
        g_fvarInterpolation = SdcOptions::FVAR_LINEAR_BOUNDARIES;
        break;
...
static void initHUD()
{
    ...
    g_hud.AddSlider("Font Scale", 0.0f, 0.1f, 0.01f, -900, -50, 100, false, callbackScale, 0);
    ...
    g_hud.AddSlider("FVar Tess", 1.0f, 10.0f, (float)g_FarDrawFVarPatchTess, 300, 50, 25, true, callbackFVarTess, 0);

    int fvar_pulldown = g_hud.AddPullDown("FVar Interpolation (i)", 300, 90, 250, callbackFVarInterpolation, 'i');
    g_hud.AddPullDownButton(fvar_pulldown, "FVAR_LINEAR_NONE", SdcOptions::FVAR_LINEAR_NONE, g_fvarInterpolation == SdcOptions::FVAR_LINEAR_NONE);
    g_hud.AddPullDownButton(fvar_pulldown, "FVAR_LINEAR_CORNERS_ONLY", SdcOptions::FVAR_LINEAR_CORNERS_ONLY, g_fvarInterpolation == SdcOptions::FVAR_LINEAR_CORNERS_ONLY);
    g_hud.AddPullDownButton(fvar_pulldown, "FVAR_LINEAR_CORNERS_PLUS1", SdcOptions::FVAR_LINEAR_CORNERS_PLUS1, g_fvarInterpolation == SdcOptions::FVAR_LINEAR_CORNERS_PLUS1);
    g_hud.AddPullDownButton(fvar_pulldown, "FVAR_LINEAR_CORNERS_PLUS2", SdcOptions::FVAR_LINEAR_CORNERS_PLUS2, g_fvarInterpolation == SdcOptions::FVAR_LINEAR_CORNERS_PLUS2);
    g_hud.AddPullDownButton(fvar_pulldown, "FVAR_LINEAR_BOUNDARIES", SdcOptions::FVAR_LINEAR_BOUNDARIES, g_fvarInterpolation == SdcOptions::FVAR_LINEAR_BOUNDARIES);
    g_hud.AddPullDownButton(fvar_pulldown, "FVAR_LINEAR_ALL", SdcOptions::FVAR_LINEAR_ALL, g_fvarInterpolation == SdcOptions::FVAR_LINEAR_ALL);
...
//------------------------------------------------------------------------------
static void uninitGL() { glDeleteQueries(2, g_queries); }
