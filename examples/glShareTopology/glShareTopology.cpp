...
// GUI variables
int g_displayStyle = kShaded, g_MDI = 0, g_mbutton[3] = {0, 0, 0}, g_freeze = 0, g_screenSpaceTess = 1, g_patchCull = 1, g_running = 1;
...
GLuint g_transformUB = 0, g_transformBinding = 0, g_tessellationUB = 0, g_tessellationBinding = 0, g_lightingUB = 0, g_lightingBinding = 0;
...
struct SimpleShape
{
    ...
    SimpleShape() {}
    SimpleShape(std::string const &idata, char const *iname, Scheme ischeme) : name(iname), scheme(ischeme), data(idata) {}
};
...
static Effect GetEffect() { return Effect(g_displayStyle, g_screenSpaceTess, g_patchCull); }
...
class ShaderCache : public GLShaderCache<EffectDesc>
{
  public:
    virtual GLDrawConfig *CreateDrawConfig(EffectDesc const &effectDesc)
    {
        ...
        std::string primTypeDefine = (type == Far::PatchDescriptor::QUADS ? "#define PRIM_QUAD\n" : "#define PRIM_TRI\n");
        ...
        {
            // tess control shader
            ss << common << "#define OSD_PATCH_TESS_CONTROL_BSPLINE_SHADER\n" << shaderSource << Osd::GLSLPatchShaderSource::GetTessControlShaderSource(type);
...
static GLenum bindProgram(Effect effect, Far::PatchDescriptor desc, int basePrimitiveID)
{
...
template <typename T> std::string formatWithCommas(T value)
{
    std::stringstream ss;
    ss.imbue(std::locale(""));
    ss << std::fixed << value;
...
//#define ENABLE_MDI
    ...
    if (g_MDI && glMultiDrawElementsIndirect)
    {
        ...
        for (int i = 0; i < (int)batches.size(); ++i)
        {
            ...
            GLenum primType = bindProgram(GetEffect(), batches[i].desc,
                                          /*primitiveIDBase=*/0);
            glMultiDrawElementsIndirect(primType, GL_UNSIGNED_INT, 0, batches[i].count, batches[i].stride);
...
#else
    {
        ...
        for (int i = 0; i < numObjects; ++i)
        {
            ...
            for (int j = 0; j < (int)patchArrays.size(); ++j)
            {
                ...
                GLenum  primType   = bindProgram(GetEffect(), patchArray.desc, patchArray.primitiveIDOffset);
                glDrawElementsBaseVertex(primType, nPatch * patchArray.desc.GetNumControlVertices(), GL_UNSIGNED_INT, indices, baseVertex);
                ++numDrawCalls;
            }
        }
    }
    ...
    if (g_hud.IsVisible())
    {
        ...
        g_hud.DrawString(230, -60, "Vertex + Varying Bufsize   : %s", formatWithCommas(g_vboSize).c_str());
        g_hud.DrawString(230, -40, "Index + PatchParam Bufsize : %s", formatWithCommas(g_iboSize).c_str());
        g_hud.DrawString(230, -20, "Stencil table size         : %s", formatWithCommas(g_scene->GetStencilTableSize()).c_str());
...
static void rebuildTopology()
{
    ...
    if (g_kernel == kCPU)
    {
        g_scene = new Scene<Osd::CpuEvaluator, Osd::CpuGLVertexBuffer, Far::StencilTable>(g_options);
#ifdef OPENSUBDIV_HAS_OPENMP
    }
    else if (g_kernel == kOPENMP)
    {
        g_scene = new Scene<Osd::OmpEvaluator, Osd::CpuGLVertexBuffer, Far::StencilTable>(g_options);
#endif
#ifdef OPENSUBDIV_HAS_TBB
    }
    else if (g_kernel == kTBB)
    {
        g_scene = new Scene<Osd::TbbEvaluator, Osd::CpuGLVertexBuffer, Far::StencilTable>(g_options);
#endif
#ifdef OPENSUBDIV_HAS_CUDA
    }
    else if (g_kernel == kCUDA)
    {
        g_scene = new Scene<Osd::CudaEvaluator, Osd::CudaGLVertexBuffer, Osd::CudaStencilTable>(g_options);
#endif
#ifdef OPENSUBDIV_HAS_OPENCL
    }
    else if (g_kernel == kCL)
    {
        ...
        g_scene = new Scene<Osd::CLEvaluator, Osd::CLGLVertexBuffer, Osd::CLStencilTable, CLDeviceContext>(g_options, &clEvaluatorCache, &g_clDeviceContext);
#endif
#ifdef OPENSUBDIV_HAS_GLSL_TRANSFORM_FEEDBACK
    }
    else if (g_kernel == kGLSL)
    {
        ...
        g_scene = new Scene<Osd::GLXFBEvaluator, Osd::GLVertexBuffer, Osd::GLStencilTableTBO>(g_options, &glXFBEvaluatorCache);
#endif
#ifdef OPENSUBDIV_HAS_GLSL_COMPUTE
    }
    else if (g_kernel == kGLSLCompute)
    {
        ...
        g_scene = new Scene<Osd::GLComputeEvaluator, Osd::GLVertexBuffer, Osd::GLStencilTableSSBO>(g_options, &glComputeEvaluatorCache);
#endif
    }
...
static void keyboard(GLFWwindow *, int key, int /* scancode */, int event, int /* mods */)
{
    ...
    case '.':
        ...
        break;
    case ',':
        g_numObjects = std::max(1, g_numObjects / 2);
        rebuildObjects();
        break;
...
static void callbackDisplayStyle(int b)
{
...
static void initHUD()
{
    ...
    g_hud.AddCheckBox("Frustum Patch Culling (B)", g_patchCull != 0, 10, 130, callbackCheckBox, kHUD_CB_PATCH_CULL, 'b');
    g_hud.AddCheckBox("Freeze (spc)", g_freeze != 0, 10, 150, callbackCheckBox, kHUD_CB_FREEZE, ' ');
    ...
    g_hud.AddSlider("Objects count", 1, 1000, 25, -200, 20, 20, true, callbackSlider, 0);

    {
#if defined(ENABLE_MDI) && defined(GL_ARB_multi_draw_indirect)
        g_hud.AddCheckBox("Multi Draw Indirect (m)", g_MDI != 0, 10, 170, callbackCheckBox, kHUD_CB_MDI, 'm');
#endif
        g_hud.AddCheckBox("Adaptive (`)", g_options.adaptive != 0, 10, 190, callbackCheckBox, kHUD_CB_ADAPTIVE, '`');

        int endcap_pulldown = g_hud.AddPullDown("End cap (E)", 10, 210, 200, callbackEndCap, 'e');
        g_hud.AddPullDownButton(endcap_pulldown, "Regular", SceneBase::kEndCapBSplineBasis, g_options.endCap == SceneBase::kEndCapBSplineBasis);
        g_hud.AddPullDownButton(endcap_pulldown, "Gregory", SceneBase::kEndCapGregoryBasis, g_options.endCap == SceneBase::kEndCapGregoryBasis);
    }
...
//------------------------------------------------------------------------------
static void callbackError(Far::ErrorType err, const char *message)
{
