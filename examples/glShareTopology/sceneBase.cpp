...
SceneBase::SceneBase(Options const &options) : _options(options), _indexBuffer(0), _patchParamTexture(0) {}
...
SceneBase::~SceneBase()
{
    ...
    if (_patchParamTexture)
        glDeleteTextures(1, &_patchParamTexture);
...
void SceneBase::AddTopology(Shape const *shape, int level, bool varying)
{
    ...
    float center[3] = {(pmax[0] + pmin[0]) * 0.5f, (pmax[1] + pmin[1]) * 0.5f, pmin[2]};
...
int SceneBase::createStencilTable(Shape const *shape, int level, bool varying, OpenSubdiv::Far::PatchTable const **patchTableOut)
{
    ...
    Far::TopologyRefiner *refiner = 0;
    {
        ...
        refiner = Far::TopologyRefinerFactory<Shape>::Create(*shape, Far::TopologyRefinerFactory<Shape>::Options(type, options));
        assert(refiner);
    }
    ...
    *patchTableOut = patchTable;
    ...
    {
        if (Far::StencilTable const *vertexStencilsWithLocalPoints = Far::StencilTableFactory::AppendLocalPointStencilTable(*refiner, vertexStencils, patchTable->GetLocalPointStencilTable()))
        {
        ...
        if (varyingStencils)
        {
            if (Far::StencilTable const *varyingStencilsWithLocalPoints = Far::StencilTableFactory::AppendLocalPointStencilTable(*refiner, varyingStencils, patchTable->GetLocalPointVaryingStencilTable()))
            {
    ...
    _stencilTableSize = createMeshRefiner(vertexStencils, varyingStencils, numControlVertices);
...
int SceneBase::AddObjects(int numObjects)
{
...
size_t SceneBase::CreateIndexBuffer()
{
    if (_indexBuffer == 0)
    {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, (int)buffer.size() * sizeof(int), &buffer[0], GL_STATIC_DRAW);

    // patchParam is currently expected to be texture (it can be SSBO)
    GLuint texBuffer = 0;
    glGenBuffers(1, &texBuffer);
    glBindBuffer(GL_ARRAY_BUFFER, texBuffer);
    glBufferData(GL_ARRAY_BUFFER, ppBuffer.size() * sizeof(unsigned int), &ppBuffer[0], GL_STATIC_DRAW);
...
void SceneBase::buildBatches()
{
    ...
    for (int i = 0; i < numObjects; ++i)
    {
        ...
        SceneBase::PatchArrayVector const &patchArrays = _topologies[_objects[i].topologyIndex].patchArrays;
    ...
    for (StagingBatches::iterator it = stagingBatches.begin(); it != stagingBatches.end(); ++it)
    {
        ...
        glBufferData(GL_DRAW_INDIRECT_BUFFER, it->second.size() * sizeof(int), &it->second[0], GL_STATIC_DRAW);
