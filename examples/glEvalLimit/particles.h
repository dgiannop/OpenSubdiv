...
class STParticles
{
  ...
  public:
    ...
    struct FaceInfo
    {
        ...
        FaceInfo() : adjedges(0), flags(0) { adjfaces[0] = adjfaces[1] = adjfaces[2] = adjfaces[3] = -1; }

        FaceInfo(int adjfaces_[4], int adjedges_[4], bool isSubface = false) : flags(isSubface ? flag_subface : 0)
        {
        ...
        void setadjedges(int e0, int e1, int e2, int e3) { adjedges = (e0 & 3) | ((e1 & 3) << 2) | ((e2 & 3) << 4) | ((e3 & 3) << 6); }
    ...
    STParticles(Refiner const &refiner, PatchTable const *patchTable, int nparticles, bool centered = false);
    ...
    void SetSpeed(float speed) { _speed = std::max(-1.0f, std::min(1.0f, speed)); }
    ...
    std::vector<OpenSubdiv::Osd::PatchCoord> GetPatchCoords() const { return _patchCoords; }
