...
void UpdateParticle(float speed, STParticles::Position *p, float *dp, Osd::PatchCoord *patchCoord, int regFaceSize, std::vector<STParticles::FaceInfo> const &adjacency, Far::PatchMap const *patchMap);
...
{
  public:
    TbbUpdateKernel(float speed, STParticles::Position *positions, float *velocities, Osd::PatchCoord *patchCoords, int regFaceSize, std::vector<STParticles::FaceInfo> const &adjacency, Far::PatchMap const *patchMap)
        : _speed(speed), _positions(positions), _velocities(velocities), _patchCoords(patchCoords), _regFaceSize(regFaceSize), _adjacency(adjacency), _patchMap(patchMap)
    {
    }
...
STParticles::STParticles(Refiner const &refiner, PatchTable const *patchTable, int nParticles, bool centered) : _speed(1.0f), _regFaceSize(Sdc::SchemeTypeTraits::GetRegularFaceSize(refiner.GetSchemeType()))
{
    ...
    { // initialize positions
        ...
        {
            pos->ptexIndex = std::min((int)(((float)rand() / (float)RAND_MAX) * nPtexFaces), nPtexFaces - 1);
            if (_regFaceSize == 3)
            {
    ...
    { // initialize velocities
        ...
        for (int i = 0; i < nParticles; ++i)
        {
            ...
            float s = 2.0f * (float)rand() / (float)RAND_MAX - 1.0f, t = 2.0f * (float)rand() / (float)RAND_MAX - 1.0f, l = sqrtf(s * s + t * t);
...
inline void RotateQuad(int rot, STParticles::Position *p, float *dp)
{
    switch (rot & 3)
    {
    default:
...
inline void TrimQuad(STParticles::Position *p)
{
    if (p->s < 0.0f)
        p->s = 1.0f + p->s;
    if (p->s >= 1.0f)
...
inline void ClampQuad(STParticles::Position *p)
{
    if (p->s < 0.0f)
    {
        p->s = 0.0f;
...
inline void BounceQuad(int edge, STParticles::Position *p, float *dp)
{
    switch (edge)
    {
    case 0:
...
void WarpQuad(std::vector<STParticles::FaceInfo> const &adjacency, int edge, STParticles::Position *p, float *dp)
{
...
void ConstrainQuad(STParticles::Position *p, float *dp, std::vector<STParticles::FaceInfo> const &adjacency)
{
...
inline void ClampTri(STParticles::Position *p)
{
    if (p->s < 0.0f)
    {
        p->s = 0.0f;
...
inline void BounceTri(int edge, STParticles::Position *p, float *dp)
{
    switch (edge)
    {
    case 0:
    ...
    // if they cross 2 boundaries
    ClampTri(p);
    assert((p->s >= 0.0f) && (p->s <= 1.0f) && (p->t >= 0.0f) && (p->t <= 1.0f) && ((p->s + p->t) <= 1.0f));
}

void WarpTri(std::vector<STParticles::FaceInfo> const &, int edge, STParticles::Position *p, float *dp)
{
...
void ConstrainTri(STParticles::Position *p, float *dp, std::vector<STParticles::FaceInfo> const &adjacency)
{

    // make sure particles can't skip more than 1 face boundary at a time
    assert((p->s > -2.0f) && (p->s < 2.0f) && (p->t > -2.0f) && (p->t < 2.0f) && ((p->s + p->t) > -2.0f) && ((p->s + p->t) < 2.0f));
...
void UpdateParticle(float speed, STParticles::Position *p, float *dp, Osd::PatchCoord *patchCoord, int regFaceSize, std::vector<STParticles::FaceInfo> const &adjacency, Far::PatchMap const *patchMap)
{
    ...
    Far::PatchTable::PatchHandle const *handle = patchMap->FindPatch(p->ptexIndex, p->s, p->t);
    if (handle)
    {
...
#ifdef OPENSUBDIV_HAS_TBB
    TbbUpdateKernel         kernel(speed, &_positions[0], &_velocities[0], &_patchCoords[0], _regFaceSize, _adjacency, _patchMap);
...
std::ostream &operator<<(std::ostream &os, STParticles::FaceInfo const &f)
{
    ...
    os << "  adjface: " << f.adjfaces[0] << ' ' << f.adjfaces[1] << ' ' << f.adjfaces[2] << ' ' << f.adjfaces[3] << "  adjedge: " << f.adjedge(0) << ' ' << f.adjedge(1) << ' ' << f.adjedge(2) << ' ' << f.adjedge(3) << "  flags:";
