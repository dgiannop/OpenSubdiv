...
// camera
float g_rotate[2] = {0, 0}, g_prev_x = 0, g_prev_y = 0, g_dolly = 5, g_pan[2] = {0, 0}, g_center[3] = {0, 0, 0}, g_size = 0;
...
static ID3D11Texture2D *createHDRTexture(ID3D11Device *device, char const *hdrFile)
{
...
class ShaderCache : public D3D11ShaderCache<EffectDesc>
{
  public:
    virtual D3D11DrawConfig *CreateDrawConfig(EffectDesc const &effectDesc)
    {
        ...
        // input layout
        const D3D11_INPUT_ELEMENT_DESC hInElementDesc[] = {{"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0}, {"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 4 * 3, D3D11_INPUT_PER_VERTEX_DATA, 0}};

        // vertex shader
        ss << common << g_shaderSource << Osd::HLSLPatchShaderSource::GetVertexShaderSource(type);
        if (effectDesc.desc.IsAdaptive())
        {
            config->CompileVertexShader("vs_5_0", "vs_main_patches", ss.str(), &g_pInputLayout, hInElementDesc, ARRAYSIZE(hInElementDesc), g_pd3dDevice);
        }
        else
        {
            config->CompileVertexShader("vs_5_0", "vs_main", ss.str(), &g_pInputLayout, hInElementDesc, ARRAYSIZE(hInElementDesc), g_pd3dDevice);
        }
        ss.str("");
        ...
        {
            // hull shader
            ss << common << g_shaderSource << Osd::HLSLPatchShaderSource::GetHullShaderSource(type);
            config->CompileHullShader("hs_5_0", "hs_main_patches", ss.str(), g_pd3dDevice);
            ss.str("");

            // domain shader
            ss << common << g_shaderSource << Osd::HLSLPatchShaderSource::GetDomainShaderSource(type);
            config->CompileDomainShader("ds_5_0", "ds_main_patches", ss.str(), g_pd3dDevice);
            ss.str("");
        }

        // geometry shader
        ss << common << g_shaderSource;
        config->CompileGeometryShader("gs_5_0", "gs_main", ss.str(), g_pd3dDevice);
        ss.str("");

        // pixel shader
        ss << common << g_shaderSource;
        config->CompilePixelShader("ps_5_0", "ps_main", ss.str(), g_pd3dDevice);
        ss.str("");
...
//------------------------------------------------------------------------------
D3D11PtexMipmapTexture *createPtex(const char *filename, int memLimit)
{
...
#define PTEX_CACHE_SIZE (512 * 1024 * 1024)
    ...
    D3D11PtexMipmapTexture *osdPtex = D3D11PtexMipmapTexture::Create(g_pd3dDeviceContext, ptex, g_maxMipmapLevels, targetMemory);
...
void createOsdMesh(int level, int kernel)
{
    ...
    if (g_kernel == kCPU)
    {
        g_mesh = new Osd::Mesh<Osd::CpuD3D11VertexBuffer, Far::StencilTable, Osd::CpuEvaluator, Osd::D3D11PatchTable, ID3D11DeviceContext>(refiner, numVertexElements, numVaryingElements, level, bits, NULL, g_pd3dDeviceContext);

#ifdef OPENSUBDIV_HAS_OPENMP
    }
    else if (kernel == kOPENMP)
    {
        g_mesh = new Osd::Mesh<Osd::CpuD3D11VertexBuffer, Far::StencilTable, Osd::OmpEvaluator, Osd::D3D11PatchTable, ID3D11DeviceContext>(refiner, numVertexElements, numVaryingElements, level, bits, NULL, g_pd3dDeviceContext);
#endif
#ifdef OPENSUBDIV_HAS_TBB
    }
    else if (kernel == kTBB)
    {
        g_mesh = new Osd::Mesh<Osd::CpuD3D11VertexBuffer, Far::StencilTable, Osd::TbbEvaluator, Osd::D3D11PatchTable, ID3D11DeviceContext>(refiner, numVertexElements, numVaryingElements, level, bits, NULL, g_pd3dDeviceContext);
#endif
#ifdef OPENSUBDIV_HAS_OPENCL
    }
    else if (kernel == kCL)
    {
        ...
        g_mesh = new Osd::Mesh<Osd::CLD3D11VertexBuffer, Osd::CLStencilTable, Osd::CLEvaluator, Osd::D3D11PatchTable, CLD3D11DeviceContext>(refiner, numVertexElements, numVaryingElements, level, bits, &clEvaluatorCache, &g_clDeviceContext);
#endif
#ifdef OPENSUBDIV_HAS_CUDA
    }
    else if (g_kernel == kCUDA)
    {
        g_mesh = new Osd::Mesh<Osd::CudaD3D11VertexBuffer, Osd::CudaStencilTable, Osd::CudaEvaluator, Osd::D3D11PatchTable, ID3D11DeviceContext>(refiner, numVertexElements, numVaryingElements, level, bits, NULL, g_pd3dDeviceContext);
#endif
    }
    else if (g_kernel == kDirectCompute)
    {
        ...
        g_mesh =
            new Osd::Mesh<Osd::D3D11VertexBuffer, Osd::D3D11StencilTable, Osd::D3D11ComputeEvaluator, Osd::D3D11PatchTable, ID3D11DeviceContext>(refiner, numVertexElements, numVaryingElements, level, bits, &d3d11ComputeEvaluatorCache, g_pd3dDeviceContext);
    }
    else
    {
...
//------------------------------------------------------------------------------
static void bindProgram(Effect effect, OpenSubdiv::Osd::PatchArray const &patch)
{
    ...
    {
        __declspec(align(16)) struct CB_PER_FRAME_CONSTANTS
        {
    ...
    {
        __declspec(align(16)) struct Tessellation
        {
            float TessLevel;
            int   PrimitiveIdBase;
        };
    ...
    // Update config state
    {
        __declspec(align(16)) struct Config
        {
...
static void drawModel()
{
    ...
    g_pd3dDeviceContext->IASetIndexBuffer(g_mesh->GetPatchTable()->GetPatchIndexBuffer(), DXGI_FORMAT_R32_UINT, 0);
    ...
    for (int i = 0; i < (int)patches.size(); ++i)
    {
        ...
        g_pd3dDeviceContext->DrawIndexed(patch.GetNumPatches() * desc.GetNumControlVertices(), patch.GetIndexBase(), 0);
    }
}
...
static void mouse(int button, int state, int x, int y)
{
    ...
    if (button == 0 && state == 1 && g_hud->MouseClick(x, y))
        return;
...
static void keyboard(char key)
{
    ...
    if (g_hud->KeyDown((int)tolower(key)))
        return;
...
//                deviceFlags |= D3D11_CREATE_DEVICE_DEBUG;
#endif
        hr = D3D11CreateDeviceAndSwapChain(NULL, hDriverType, NULL, deviceFlags, NULL, 0, D3D11_SDK_VERSION, &hDXGISwapChainDesc, &g_pSwapChain, &g_pd3dDevice, &hFeatureLevel, &g_pd3dDeviceContext);
        if (SUCCEEDED(hr))
        {
            break;
        }
    ...
    __declspec(align(16)) struct Lighting
    {
        struct Light
        {
            float position[4];
        ...
        {{{0.5, 0.2f, 1.0f, 0.0f}, {0.1f, 0.1f, 0.1f, 1.0f}, {0.7f, 0.7f, 0.7f, 1.0f}, {0.8f, 0.8f, 0.8f, 1.0f}},
...
static bool updateRenderTarget(HWND hWnd)
{
...
static std::vector<std::string> tokenize(std::string const &src)
{

    std::vector<std::string> result;

    std::stringstream input(src);
    std::copy(std::istream_iterator<std::string>(input), std::istream_iterator<std::string>(), std::back_inserter<std::vector<std::string>>(result));
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
    ...
    HWND hWnd = CreateWindow(szWindowClass, windowTitle, WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, rect.right - rect.left, rect.bottom - rect.top, NULL, NULL, hInstance, NULL);
    ...
    if (diffuseEnvironmentMap)
    {
        ...
        g_diffuseEnvironmentMap = createHDRTexture(g_pd3dDevice, diffuseEnvironmentMap);
    ...
    if (specularEnvironmentMap)
    {
        ...
        g_specularEnvironmentMap = createHDRTexture(g_pd3dDevice, specularEnvironmentMap);
    ...
    if (g_diffuseEnvironmentMap || g_specularEnvironmentMap)
    {
        ...
        g_sky = new Sky(g_pd3dDevice, g_specularEnvironmentMap != 0 ? g_specularEnvironmentMap : g_diffuseEnvironmentMap);
    }
