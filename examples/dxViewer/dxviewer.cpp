...
// GUI variables
int g_freeze = 0, g_shadingMode = kShadingPatchType, g_displayStyle = kDisplayStyleWireOnShaded, g_adaptive = 1, g_endCap = kEndCapGregoryBasis, g_smoothCornerPatch = 1, g_singleCreasePatch = 1, g_infSharpPatch = 1, g_drawNormals = 0,
    g_mbutton[3] = {0, 0, 0};
...
// geometry
std::vector<float> g_orgPositions, g_positions;
...
//------------------------------------------------------------------------------
static void createOsdMesh(ShapeDesc const &shapeDesc, int level, int kernel, Scheme scheme = kCatmark)
{
    ...
    {
        g_legacyGregoryPatchTable = Osd::D3D11LegacyGregoryPatchTable::Create(g_mesh->GetFarPatchTable(), g_pd3dDeviceContext);
    }
...
class ShaderCache : public D3D11ShaderCache<EffectDesc>
{
  public:
    virtual D3D11DrawConfig *CreateDrawConfig(EffectDesc const &effectDesc)
    {
        ...
        // display styles
        std::string gs_entry = (type == Far::PatchDescriptor::QUADS ? "gs_quad" : "gs_triangle");
        ...
        // vertex shader
        ss << common << shaderSource << Osd::HLSLPatchShaderSource::GetVertexShaderSource(type);
        if (effectDesc.desc.IsAdaptive())
        {
        ...
        {
            // hull shader
            ss << common << shaderSource << Osd::HLSLPatchShaderSource::GetHullShaderSource(type);
            config->CompileHullShader("hs_5_0", "hs_main_patches", ss.str(), g_pd3dDevice);
            ss.str("");

            // domain shader
            ss << common << shaderSource << Osd::HLSLPatchShaderSource::GetDomainShaderSource(type);
            config->CompileDomainShader("ds_5_0", "ds_main_patches", ss.str(), g_pd3dDevice);
            ss.str("");
        }

        // geometry shader
        ss << common << shaderSource;
        config->CompileGeometryShader("gs_5_0", gs_entry, ss.str(), g_pd3dDevice);
        ss.str("");
...
static void bindProgram(Effect effect, OpenSubdiv::Osd::PatchArray const &patch)
{
    ...
    // Update tessellation state
    {
        ...
        pData->GregoryQuadOffsetBase = g_legacyGregoryPatchTable ? g_legacyGregoryPatchTable->GetQuadOffsetsBase(patch.GetDescriptor().GetType()) : 0;
    ...
    if (g_legacyGregoryPatchTable)
    {
        ...
        ID3D11ShaderResourceView *vertexValenceSRV = g_legacyGregoryPatchTable->GetVertexValenceSRV();
        ID3D11ShaderResourceView *quadOffsetsSRV   = g_legacyGregoryPatchTable->GetQuadOffsetsSRV();
...
static void display()
{
    ...
    if (g_legacyGregoryPatchTable)
    {
        g_legacyGregoryPatchTable->UpdateVertexBuffer(buffer, g_mesh->GetNumVertices(), 6, g_pd3dDeviceContext);
    }
    ...
    if (g_hud->IsVisible())
    {
        ...
        if (g_displayPatchCounts)
        {
            ...
            if (g_legacyGregoryEnabled)
            {
                ...
                y += 20;
                g_hud->DrawString(x, y, "Boundary Gregory : %d", patchCount[Descriptor::GREGORY_BOUNDARY]);
                y += 20;
            }
...
//------------------------------------------------------------------------------
static void rebuildOsdMesh() { createOsdMesh(g_defaultShapes[g_currentShape], g_level, g_kernel, g_defaultShapes[g_currentShape].scheme); }
...
static void callbackDisplayNormal(bool checked, int n) { g_drawNormals = checked; }

static void callbackShadingMode(int b) { g_shadingMode = b; }
...
static void initHUD()
{
    ...
    g_hud->AddPullDownButton(shading_pulldown, "Patch Depth", kShadingPatchDepth, g_shadingMode == kShadingPatchCoord);
    ...
    g_hud->AddCheckBox("Adaptive (`)", true, 10, 190, callbackCheckBox, kHUD_CB_ADAPTIVE, '`');
...
static bool initD3D11(HWND hWnd)
{
    ...
    for (UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++)
    {
        ...
        hr          = D3D11CreateDeviceAndSwapChain(NULL, hDriverType, NULL, 0, NULL, 0, D3D11_SDK_VERSION, &hDXGISwapChainDesc, &g_pSwapChain, &g_pd3dDevice, &hFeatureLevel, &g_pd3dDeviceContext);
        if (SUCCEEDED(hr))
        {
            break;
        }
