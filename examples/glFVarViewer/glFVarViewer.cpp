...
OpenSubdiv::Sdc::Options::FVarLinearInterpolation g_fvarInterp = OpenSubdiv::Sdc::Options::FVAR_LINEAR_ALL;

// GUI variables
int g_freeze = 0, g_uvCullBackface = 0, g_displayStyle = kWireShaded, g_adaptive = 1, g_smoothCornerPatch = 1, g_singleCreasePatch = 1, g_infSharpPatch = 1, g_mbutton[3] = {0, 0, 0}, g_mouseUvView = 0, g_running = 1;

float g_moveScale = 0.0f;

float g_rotate[2] = {0, 0}, g_dolly = 5, g_pan[2] = {0, 0}, g_center[3] = {0, 0, 0}, g_size = 0, g_uvPan[2] = {0, 0}, g_uvScale = 1.0;

bool g_yup = false;

int g_prev_x = 0, g_prev_y = 0;

int g_width = 1600, g_height = 800;

GLhud                g_hud;
GLControlMeshDisplay g_controlMeshDisplay;

// geometry
std::vector<float> g_orgPositions, g_positions, g_normals;
...
GLuint g_transformUB = 0, g_transformBinding = 0, g_tessellationUB = 0, g_tessellationBinding = 0, g_fvarArrayDataUB = 0, g_fvarArrayDataBinding = 0;
...
struct FVarData
{
    ...
    void Create(OpenSubdiv::Far::TopologyRefiner const *refiner, OpenSubdiv::Far::PatchTable const *patchTable, std::vector<float> const &fvarSrcData, int fvarWidth, int fvarChannel = 0)
    {
        ...
        Far::StencilTable const *fvarStencils = Far::StencilTableFactory::Create(*refiner, soptions);

        if (Far::StencilTable const *fvarStencilsWithLocalPoints = Far::StencilTableFactory::AppendLocalPointStencilTableFaceVarying(*refiner, fvarStencils, patchTable->GetLocalPointFaceVaryingStencilTable(), fvarChannel))
        {
            delete fvarStencils;
        ...
        int numFVarPoints    = numSrcFVarPoints + fvarStencils->GetNumStencils();

        Osd::CpuVertexBuffer *fvarBuffer = Osd::CpuVertexBuffer::Create(fvarWidth, numFVarPoints);
        fvarBuffer->UpdateData(&fvarSrcData[0], 0, numSrcFVarPoints);

        Osd::BufferDescriptor srcDesc(0, fvarWidth, fvarWidth);
        Osd::BufferDescriptor dstDesc(numSrcFVarPoints * fvarWidth, fvarWidth, fvarWidth);

        Osd::CpuEvaluator::EvalStencils(fvarBuffer, srcDesc, fvarBuffer, dstDesc, fvarStencils);

        Far::ConstIndexArray indices        = patchTable->GetFVarValues();
        const float *        fvarSrcDataPtr = !refiner->IsUniform() ? fvarBuffer->BindCpuBuffer() : fvarBuffer->BindCpuBuffer() + numSrcFVarPoints * fvarWidth;
...
#define GLSL_VERSION_DEFINE "#version 150\n"
    ...
    g_defaultProgram.uniformModelViewProjectionMatrix = glGetUniformLocation(program, "ModelViewProjectionMatrix");
...
//------------------------------------------------------------------------------
static void calcNormals(OpenSubdiv::Far::TopologyRefiner const &refiner, std::vector<float> const &pos, std::vector<float> &normals)
{
...
static void rebuildMesh()
{
    ...
    g_fvarData.Create(refiner, g_mesh->GetFarPatchTable(), shape->uvs, shape->GetFVarWidth());
...
static Effect GetEffect(bool uvDraw = false) { return Effect(g_displayStyle, uvDraw); }
...
static void display()
{
    ...
    g_controlMeshDisplay.Draw(vbo, 3 * sizeof(float), g_transformData.ModelViewProjectionMatrix);
...
static void callbackControlEdges(bool checked, int /* a */)
{
...
static void callbackControlVertices(bool checked, int /* a */)
{
...
static void callbackUVCullBackface(bool checked, int /* a */)
{
...
static void callbackAdaptive(bool checked, int /* a */)
{
...
static void callbackSmoothCornerPatch(bool checked, int /* a */)
{
...
static void callbackSingleCreasePatch(bool checked, int /* a */)
{
...
static void callbackInfSharpPatch(bool checked, int /* a */)
{
...
static void callbackFVarInterp(int b)
{
    ...
    case SdcOptions::FVAR_LINEAR_NONE:
        g_fvarInterp = SdcOptions::FVAR_LINEAR_NONE;
        break;

    case SdcOptions::FVAR_LINEAR_CORNERS_ONLY:
        g_fvarInterp = SdcOptions::FVAR_LINEAR_CORNERS_ONLY;
        break;

    case SdcOptions::FVAR_LINEAR_CORNERS_PLUS1:
        g_fvarInterp = SdcOptions::FVAR_LINEAR_CORNERS_PLUS1;
        break;

    case SdcOptions::FVAR_LINEAR_CORNERS_PLUS2:
        g_fvarInterp = SdcOptions::FVAR_LINEAR_CORNERS_PLUS2;
        break;

    case SdcOptions::FVAR_LINEAR_BOUNDARIES:
        g_fvarInterp = SdcOptions::FVAR_LINEAR_BOUNDARIES;
        break;

    case SdcOptions::FVAR_LINEAR_ALL:
        g_fvarInterp = SdcOptions::FVAR_LINEAR_ALL;
        break;
    }
    rebuildMesh();
}
...
static void initHUD()
{
    ...
    g_hud.AddCheckBox("Control edges (H)", g_controlMeshDisplay.GetEdgesDisplay(), 10, 60, callbackControlEdges, 0, 'h');
    g_hud.AddCheckBox("Control vertices (J)", g_controlMeshDisplay.GetVerticesDisplay(), 10, 80, callbackControlVertices, 0, 'j');
    g_hud.AddCheckBox("UV Backface Culling (B)", g_uvCullBackface != 0, 10, 100, callbackUVCullBackface, 0, 'b');

    int shading_pulldown = g_hud.AddPullDown("Display Style (W)", 400, 10, 250, callbackDisplayStyle, 'w');
    ...
    {
        g_hud.AddCheckBox("Adaptive (`)", g_adaptive != 0, 10, 140, callbackAdaptive, 0, '`');

        g_hud.AddCheckBox("Smooth Corner Patch (O)", g_smoothCornerPatch != 0, 10, 160, callbackSmoothCornerPatch, 0, 'o');
        g_hud.AddCheckBox("Single Crease Patch (S)", g_singleCreasePatch != 0, 10, 180, callbackSingleCreasePatch, 0, 's');
        g_hud.AddCheckBox("Inf Sharp Patch (I)", g_infSharpPatch != 0, 10, 200, callbackInfSharpPatch, 0, 'i');

        int endcap_pulldown = g_hud.AddPullDown("End cap (E)", 10, 220, 200, callbackEndCap, 'e');
    ...
    int fvar_interp_pulldown = g_hud.AddPullDown("Linear Interpolation (L)", 10, 10, 250, callbackFVarInterp, 'l');
    g_hud.AddPullDownButton(fvar_interp_pulldown, "None (edge only)", SdcOptions::FVAR_LINEAR_NONE, g_fvarInterp == SdcOptions::FVAR_LINEAR_NONE);
    g_hud.AddPullDownButton(fvar_interp_pulldown, "Corners Only", SdcOptions::FVAR_LINEAR_CORNERS_ONLY, g_fvarInterp == SdcOptions::FVAR_LINEAR_CORNERS_ONLY);
    g_hud.AddPullDownButton(fvar_interp_pulldown, "Corners 1 (edge corner)", SdcOptions::FVAR_LINEAR_CORNERS_PLUS1, g_fvarInterp == SdcOptions::FVAR_LINEAR_CORNERS_PLUS1);
    g_hud.AddPullDownButton(fvar_interp_pulldown, "Corners 2 (edge corner prop)", SdcOptions::FVAR_LINEAR_CORNERS_PLUS2, g_fvarInterp == SdcOptions::FVAR_LINEAR_CORNERS_PLUS2);
    g_hud.AddPullDownButton(fvar_interp_pulldown, "Boundaries (always sharp)", SdcOptions::FVAR_LINEAR_BOUNDARIES, g_fvarInterp == SdcOptions::FVAR_LINEAR_BOUNDARIES);
    g_hud.AddPullDownButton(fvar_interp_pulldown, "All (bilinear)", SdcOptions::FVAR_LINEAR_ALL, g_fvarInterp == SdcOptions::FVAR_LINEAR_ALL);
