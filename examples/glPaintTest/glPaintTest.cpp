...
int g_running = 1, g_wire = 2, g_displayColor = 1, g_displayDisplacement = 0, g_mbutton[3] = {0, 0, 0};

int g_brushSize = 100;
int g_frame     = 0;

GLuint g_ptexPages = 0, g_ptexLayouts = 0, g_ptexTexels = 0;
...
static void checkGLErrors(std::string const &where = "")
{
    ...
    {
        std::cerr << "GL error: " << (where.empty() ? "" : where + " ") << err << "\n";
    }
}
...
static void createOsdMesh()
{
    ...
    g_mesh = new OpenSubdiv::Osd::Mesh<OpenSubdiv::Osd::CpuGLVertexBuffer, OpenSubdiv::Far::StencilTable, OpenSubdiv::Osd::CpuEvaluator, OpenSubdiv::Osd::GLPatchTable>(refiner, 3, 0, g_level, bits);
    ...
    // -------- create ptex
    if (g_ptexPages)
        glDeleteTextures(1, &g_ptexPages);
    if (g_ptexLayouts)
        glDeleteTextures(1, &g_ptexLayouts);
    if (g_ptexTexels)
        glDeleteTextures(1, &g_ptexTexels);
    ...
    g_ptexPages = genTextureBuffer(GL_R32I, numPtexFaces * sizeof(GLint), &pages[0]);

    g_ptexLayouts = genTextureBuffer(GL_RGBA32F, numPtexFaces * 4 * sizeof(GLfloat), &layouts[0]);
    ...
    // allocate ptex
    glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_R32F, pageSize, pageSize, numPtexFaces, 0, GL_RED, GL_FLOAT, &texels[0]);
...
class ShaderCache : public GLShaderCache<EffectDesc>
{
  public:
    virtual GLDrawConfig *CreateDrawConfig(EffectDesc const &effectDesc)
    {
        ...
        // vertex shader
        ss << common << (effectDesc.desc.IsAdaptive() ? "" : "#define VERTEX_SHADER\n") << (effectDesc.effect.paint ? paintShaderSource : shaderSource) << Osd::GLSLPatchShaderSource::GetVertexShaderSource(type);
        ...
        {
            // tess control shader
            ss << common << (effectDesc.effect.paint ? paintShaderSource : shaderSource) << Osd::GLSLPatchShaderSource::GetTessControlShaderSource(type);
            config->CompileAndAttachShader(GL_TESS_CONTROL_SHADER, ss.str());
            ss.str("");

            // tess eval shader
            ss << common << (effectDesc.effect.paint ? paintShaderSource : shaderSource) << Osd::GLSLPatchShaderSource::GetTessEvalShaderSource(type);
...
static GLuint bindProgram(Effect effect, OpenSubdiv::Osd::PatchArray const &patch)
{
...
//------------------------------------------------------------------------------
static void motion(GLFWwindow *w, double dx, double dy)
{
...
//------------------------------------------------------------------------------
static void mouse(GLFWwindow *w, int button, int state, int /* mods */)
{
...
static void callbackDisplay(bool /* checked */, int n)
{

    if (n == 0)
        g_displayColor = !g_displayColor;
    else if (n == 1)
        g_displayDisplacement = !g_displayDisplacement;
}
...
static void callbackBrushSize(float value, int /* data */) { g_brushSize = (int)value; }
...
static void initHUD()
{
    ...
    g_hud.AddSlider("Brush size", 10.0f, 500.0f, (float)g_brushSize, 350, -60, 40, true, callbackBrushSize, 0);
...
static void initGL()
{
    ...
    for (int yy = 0; yy < reso; ++yy)
    {
        ...
        {
            float r = sqrtf((xx - reso * 0.5f) * (xx - reso * 0.5f) + (yy - reso * 0.5f) * (yy - reso * 0.5f)) / (reso * 0.5f);
