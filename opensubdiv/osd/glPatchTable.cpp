...
GLPatchTable::GLPatchTable() : _patchIndexBuffer(0), _patchParamBuffer(0), _patchIndexTexture(0), _patchParamTexture(0)
{
...
GLPatchTable::~GLPatchTable()
{
    ...
    if (_patchParamBuffer)
        glDeleteBuffers(1, &_patchParamBuffer);
    if (_patchIndexTexture)
        glDeleteTextures(1, &_patchIndexTexture);
    if (_patchParamTexture)
        glDeleteTextures(1, &_patchParamTexture);
    if (_varyingIndexBuffer)
        glDeleteBuffers(1, &_varyingIndexBuffer);
    if (_varyingIndexTexture)
        glDeleteTextures(1, &_varyingIndexTexture);
    for (int fvc = 0; fvc < (int)_fvarIndexBuffers.size(); ++fvc)
    {
        if (_fvarIndexBuffers[fvc])
            glDeleteBuffers(1, &_fvarIndexBuffers[fvc]);
    }
    for (int fvc = 0; fvc < (int)_fvarIndexTextures.size(); ++fvc)
    {
        if (_fvarIndexTextures[fvc])
            glDeleteTextures(1, &_fvarIndexTextures[fvc]);
    }
}

GLPatchTable *GLPatchTable::Create(Far::PatchTable const *farPatchTable, void * /*deviceContext*/)
{
...
bool GLPatchTable::allocate(Far::PatchTable const *farPatchTable)
{
    ...
    // copy patch array
    _patchArrays.assign(patchTable.GetPatchArrayBuffer(), patchTable.GetPatchArrayBuffer() + numPatchArrays);

    // copy index buffer
    glBindBuffer(GL_ARRAY_BUFFER, _patchIndexBuffer);
    glBufferData(GL_ARRAY_BUFFER, indexSize * sizeof(GLint), patchTable.GetPatchIndexBuffer(), GL_STATIC_DRAW);

    // copy patchparam buffer
    glBindBuffer(GL_ARRAY_BUFFER, _patchParamBuffer);
    glBufferData(GL_ARRAY_BUFFER, patchParamSize * sizeof(PatchParam), patchTable.GetPatchParamBuffer(), GL_STATIC_DRAW);
    ...
    // varying
    _varyingPatchArrays.assign(patchTable.GetVaryingPatchArrayBuffer(), patchTable.GetVaryingPatchArrayBuffer() + numPatchArrays);

    glGenBuffers(1, &_varyingIndexBuffer);
    glBindBuffer(GL_ARRAY_BUFFER, _varyingIndexBuffer);
    glBufferData(GL_ARRAY_BUFFER, patchTable.GetVaryingPatchIndexSize() * sizeof(GLint), patchTable.GetVaryingPatchIndexBuffer(), GL_STATIC_DRAW);
    ...
    {
        _fvarPatchArrays[fvc].assign(patchTable.GetFVarPatchArrayBuffer(fvc), patchTable.GetFVarPatchArrayBuffer(fvc) + numPatchArrays);

        glGenBuffers(1, &_fvarIndexBuffers[fvc]);
        glBindBuffer(GL_ARRAY_BUFFER, _fvarIndexBuffers[fvc]);
        glBufferData(GL_ARRAY_BUFFER, patchTable.GetFVarPatchIndexSize(fvc) * sizeof(GLint), patchTable.GetFVarPatchIndexBuffer(fvc), GL_STATIC_DRAW);
        ...
        glBufferData(GL_ARRAY_BUFFER, patchTable.GetFVarPatchParamSize(fvc) * sizeof(PatchParam), patchTable.GetFVarPatchParamBuffer(fvc), GL_STATIC_DRAW);
