...
D3D11LegacyGregoryPatchTable::D3D11LegacyGregoryPatchTable() : _vertexValenceBuffer(0), _quadOffsetsBuffer(0), _vertexSRV(0), _vertexValenceSRV(0), _quadOffsetsSRV(0) { _quadOffsetsBase[0] = _quadOffsetsBase[1] = 0; }

D3D11LegacyGregoryPatchTable::~D3D11LegacyGregoryPatchTable()
{
    if (_vertexValenceBuffer)
        _vertexValenceBuffer->Release();
    if (_quadOffsetsBuffer)
        _quadOffsetsBuffer->Release();
    if (_vertexSRV)
        _vertexSRV->Release();
    if (_vertexValenceSRV)
        _vertexValenceSRV->Release();
    if (_quadOffsetsSRV)
        _quadOffsetsSRV->Release();
}

D3D11LegacyGregoryPatchTable *D3D11LegacyGregoryPatchTable::Create(Far::PatchTable const *farPatchTable, ID3D11DeviceContext *pd3d11DeviceContext)
{
    ...
    if (!valenceTable.empty())
    {
        ...
        HRESULT hr       = pd3d11Device->CreateBuffer(&bd, &initData, &result->_vertexValenceBuffer);
        if (FAILED(hr))
        {
            delete result;
            return NULL;
        ...
        hr                       = pd3d11Device->CreateShaderResourceView(result->_vertexValenceBuffer, &srvd, &result->_vertexValenceSRV);
        if (FAILED(hr))
        {
            delete result;
            return NULL;
    ...
    if (!quadOffsetsTable.empty())
    {
        ...
        HRESULT hr       = pd3d11Device->CreateBuffer(&bd, &initData, &result->_quadOffsetsBuffer);
        if (FAILED(hr))
        {
            delete result;
            return NULL;
        ...
        hr                       = pd3d11Device->CreateShaderResourceView(result->_quadOffsetsBuffer, &srvd, &result->_quadOffsetsSRV);
        if (FAILED(hr))
        {
            delete result;
            return NULL;
...
void D3D11LegacyGregoryPatchTable::UpdateVertexBuffer(ID3D11Buffer *vbo, int numVertices, int numVertexElements, ID3D11DeviceContext *pd3d11DeviceContext)
{
    ID3D11Device *pd3d11Device = NULL;
    pd3d11DeviceContext->GetDevice(&pd3d11Device);
    assert(pd3d11Device);
    ...
    HRESULT hr               = pd3d11Device->CreateShaderResourceView(vbo, &srvd, &_vertexSRV);
    if (FAILED(hr))
    {
        return;
    }
