...
D3D11PatchTable::D3D11PatchTable() : _indexBuffer(0), _patchParamBuffer(0), _patchParamBufferSRV(0) {}
...
D3D11PatchTable::~D3D11PatchTable()
{
    ...
    if (_patchParamBuffer)
        _patchParamBuffer->Release();
    if (_patchParamBufferSRV)
        _patchParamBufferSRV->Release();
}

D3D11PatchTable *D3D11PatchTable::Create(Far::PatchTable const *farPatchTable, ID3D11DeviceContext *pd3d11DeviceContext)
{
...
bool D3D11PatchTable::allocate(Far::PatchTable const *farPatchTable, ID3D11DeviceContext *pd3d11DeviceContext)
{
    ...
    hr = pd3d11DeviceContext->Map(_indexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    if (FAILED(hr))
    {
        return false;
    }
    unsigned int *indexBuffer = (unsigned int *)mappedResource.pData;
    memcpy(indexBuffer, patchTable.GetPatchIndexBuffer(), indexSize * sizeof(unsigned int));
    ...
    hr                       = pd3d11Device->CreateShaderResourceView(_patchParamBuffer, &srvd, &_patchParamBufferSRV);
    if (FAILED(hr))
    {
        return false;
    }
    hr = pd3d11DeviceContext->Map(_patchParamBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    if (FAILED(hr))
    {
        return false;
    }
    unsigned int *dst = (unsigned int *)mappedResource.pData;
    memcpy(dst, patchTable.GetPatchParamBuffer(), patchParamSize * sizeof(PatchParam));
