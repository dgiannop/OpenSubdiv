...
{
  public:
    template <typename DEVICE_CONTEXT> static D3D11StencilTable *Create(Far::StencilTable const *stencilTable, DEVICE_CONTEXT context) { return new D3D11StencilTable(stencilTable, context->GetDeviceContext()); }

    static D3D11StencilTable *Create(Far::StencilTable const *stencilTable, ID3D11DeviceContext *deviceContext) { return new D3D11StencilTable(stencilTable, deviceContext); }
...
class D3D11ComputeEvaluator
{
  public:
    ...
    static D3D11ComputeEvaluator *Create(BufferDescriptor const &srcDesc, BufferDescriptor const &dstDesc, BufferDescriptor const &duDesc, BufferDescriptor const &dvDesc, BufferDescriptor const &duuDesc, BufferDescriptor const &duvDesc,
    ...
    static bool EvalStencils(SRC_BUFFER *srcBuffer, BufferDescriptor const &srcDesc, DST_BUFFER *dstBuffer, BufferDescriptor const &dstDesc, STENCIL_TABLE const *stencilTable, D3D11ComputeEvaluator const *instance, ID3D11DeviceContext *deviceContext)
    {
        if (instance)
        {
            return instance->EvalStencils(srcBuffer, srcDesc, dstBuffer, dstDesc, stencilTable, deviceContext);
        }
        else
        {
            ...
            if (instance)
            {
                bool r = instance->EvalStencils(srcBuffer, srcDesc, dstBuffer, dstDesc, stencilTable, deviceContext);
                delete instance;
                return r;
            }
            return false;
    ...
    {
        return EvalStencils(srcBuffer->BindD3D11UAV(deviceContext), srcDesc, dstBuffer->BindD3D11UAV(deviceContext), dstDesc, stencilTable->GetSizesSRV(), stencilTable->GetOffsetsSRV(), stencilTable->GetIndicesSRV(), stencilTable->GetWeightsSRV(),
                            /* start = */ 0,
                            /* end   = */ stencilTable->GetNumStencils(), deviceContext);
    }

    /// Dispatch the DX compute kernel on GPU asynchronously.
    /// returns false if the kernel hasn't been compiled yet.
    bool EvalStencils(ID3D11UnorderedAccessView *srcSRV, BufferDescriptor const &srcDesc, ID3D11UnorderedAccessView *dstUAV, BufferDescriptor const &dstDesc, ID3D11ShaderResourceView *sizesSRV, ID3D11ShaderResourceView *offsetsSRV,
