...
struct PatchCoord
{
    ...
    ///
    PatchCoord(Far::PatchTable::PatchHandle handleArg, float sArg, float tArg) : handle(handleArg), s(sArg), t(tArg) {}
...
struct PatchArray
{
    ...
    PatchArray(Far::PatchDescriptor desc_in, int numPatches_in, int indexBase_in, int primitiveIdBase_in)
        : regDesc(desc_in), desc(desc_in), numPatches(numPatches_in), indexBase(indexBase_in), stride(desc_in.GetNumControlVertices()), primitiveIdBase(primitiveIdBase_in)
    {
    }
    ...
    PatchArray(Far::PatchDescriptor regDesc_in, Far::PatchDescriptor irregDesc_in, int numPatches_in, int indexBase_in, int primitiveIdBase_in)
        : regDesc(regDesc_in), desc(irregDesc_in), numPatches(numPatches_in), indexBase(indexBase_in), stride(std::max(regDesc_in.GetNumControlVertices(), irregDesc_in.GetNumControlVertices())), primitiveIdBase(primitiveIdBase_in)
    {
    }

    Far::PatchDescriptor const &GetDescriptor() const { return desc; }
    Far::PatchDescriptor const &GetDescriptorRegular() const { return regDesc; }
    Far::PatchDescriptor const &GetDescriptorIrregular() const { return desc; }

    int GetPatchType() const { return desc.GetType(); }
    int GetPatchTypeRegular() const { return regDesc.GetType(); }
    int GetPatchTypeIrregular() const { return desc.GetType(); }

    int GetNumPatches() const { return numPatches; }
    int GetIndexBase() const { return indexBase; }
    int GetStride() const { return stride; }
    int GetPrimitiveIdBase() const { return primitiveIdBase; }
