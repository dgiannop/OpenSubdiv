...
class D3D11LegacyGregoryPatchTable : private NonCopyable<D3D11LegacyGregoryPatchTable>
{
  public:
    ~D3D11LegacyGregoryPatchTable();

    template <typename DEVICE_CONTEXT> static D3D11LegacyGregoryPatchTable *Create(Far::PatchTable const *farPatchTable, DEVICE_CONTEXT context) { return Create(farPatchTable, context->GetDeviceContext()); }
    ...
    ID3D11ShaderResourceView *GetVertexSRV() const { return _vertexSRV; }

    ID3D11ShaderResourceView *GetVertexValenceSRV() const { return _vertexValenceSRV; }

    ID3D11ShaderResourceView *GetQuadOffsetsSRV() const { return _quadOffsetsSRV; }
