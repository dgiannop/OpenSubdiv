...
GLVertexBuffer::GLVertexBuffer(int numElements, int numVertices) : _numElements(numElements), _numVertices(numVertices), _vbo(0)
{
...
GLVertexBuffer *GLVertexBuffer::Create(int numElements, int numVertices, void *)
{
...
void GLVertexBuffer::UpdateData(const float *src, int startVertex, int numVertices, void * /*deviceContext*/)
{
    int size = numVertices * _numElements * (int)sizeof(float);
#if defined(GL_ARB_direct_state_access)
    if (OSD_OPENGL_HAS(ARB_direct_state_access))
    {
        glNamedBufferSubData(_vbo, startVertex * _numElements * sizeof(float), size, src);
    }
    else
#endif
    {
