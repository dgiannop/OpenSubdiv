...
char const *Level::getTopologyErrorString(TopologyError errCode)
{
    switch (errCode)
    {
...
bool Level::validateTopology(ValidationCallback callback, void const *clientData) const
{
    //
    ...
    for (int fIndex = 0; fIndex < getNumFaces(); ++fIndex)
    {
        ...
        for (int i = 0; i < fVertCount; ++i)
        {
            ...
            {
                REPORT(TOPOLOGY_FAILED_CORRELATION_FACE_VERT, "face %d correlation of vert %d failed", fIndex, i);
    ...
    for (int fIndex = 0; fIndex < getNumFaces(); ++fIndex)
    {
        ...
        for (int i = 0; i < fEdgeCount; ++i)
        {
            ...
            {
                REPORT(TOPOLOGY_FAILED_CORRELATION_FACE_EDGE, "face %d correlation of edge %d failed", fIndex, i);
                if (returnOnFirstError)
                    return false;
                isValid = false;
            }
    ...
    for (int eIndex = 0; eIndex < getNumEdges(); ++eIndex)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            {
                REPORT(TOPOLOGY_FAILED_CORRELATION_FACE_VERT, "edge %d correlation of vert %d failed", eIndex, i);
                if (returnOnFirstError)
                    return false;
                isValid = false;
            }
    ...
    {
        if (_vertTags[vIndex]._incomplete || _vertTags[vIndex]._nonManifold)
            continue;
        ...
        {
            REPORT(TOPOLOGY_FAILED_ORIENTATION_INCIDENT_FACES_EDGES, "vertex %d cannot orient incident faces and edges", vIndex);
            if (returnOnFirstError)
                return false;
            isValid = false;
        }
        for (int i = 0; i < vFaces.size(); ++i)
        {
            ...
            {
                REPORT(TOPOLOGY_FAILED_ORIENTATION_INCIDENT_FACE, "vertex %d orientation failure at incident face %d", vIndex, i);
                if (returnOnFirstError)
                    return false;
                isValid = false;
                break;
        ...
        for (int i = 0; i < vEdges.size(); ++i)
        {
            ...
            {
                REPORT(TOPOLOGY_FAILED_ORIENTATION_INCIDENT_EDGE, "vertex %d orientation failure at incident edge %d", vIndex, i);
                if (returnOnFirstError)
                    return false;
                isValid = false;
                break;
    ...
    for (int eIndex = 0; eIndex < getNumEdges(); ++eIndex)
    {
        ...
        {
            REPORT(TOPOLOGY_DEGENERATE_EDGE, "Error in eIndex = %d:  degenerate edge not tagged marked non-manifold", eIndex);
            if (returnOnFirstError)
                return false;
            isValid = false;
        }
        ...
        {
            REPORT(TOPOLOGY_NON_MANIFOLD_EDGE, "edge %d with %d incident faces not tagged non-manifold", eIndex, eFaces.size());
            if (returnOnFirstError)
                return false;
            isValid = false;
        }
...
namespace
{
template <typename INT_TYPE> void printIndexArray(ConstArray<INT_TYPE> const &array)
{
...
const char *ruleString(Sdc::Crease::Rule rule)
{
    switch (rule)
    {
    case Sdc::Crease::RULE_UNKNOWN:
...
void Level::print(const Refinement *pRefinement) const
{
...
//
bool Level::doesVertexFVarTopologyMatch(Index vIndex, int fvarChannel) const { return getFVarLevel(fvarChannel).valueTopologyMatches(getFVarLevel(fvarChannel).getVertexValueOffset(vIndex)); }
bool Level::doesEdgeFVarTopologyMatch(Index eIndex, int fvarChannel) const { return getFVarLevel(fvarChannel).edgeTopologyMatches(eIndex); }
bool Level::doesFaceFVarTopologyMatch(Index fIndex, int fvarChannel) const { return !getFVarLevel(fvarChannel).getFaceCompositeValueTag(fIndex).isMismatch(); }

void Level::getFaceVTags(Index fIndex, VTag vTags[], int fvarChannel) const
{
    ConstIndexArray fVerts = getFaceVertices(fIndex);
    if (fvarChannel < 0)
    {
...
void Level::getFaceETags(Index fIndex, ETag eTags[], int fvarChannel) const
{
    ConstIndexArray fEdges = getFaceEdges(fIndex);
    if (fvarChannel < 0)
    {
...
Level::VTag Level::VTag::BitwiseOr(VTag const vTags[], int size)
{
...
Level::ETag Level::ETag::BitwiseOr(ETag const eTags[], int size)
{
...
Level::VTag Level::getFaceCompositeVTag(ConstIndexArray &fVerts) const
{
...
Level::VTag Level::getFaceCompositeVTag(Index fIndex, int fvarChannel) const
{
    ConstIndexArray fVerts = getFaceVertices(fIndex);
    if (fvarChannel < 0)
    {
...
Level::VTag Level::getVertexCompositeFVarVTag(Index vIndex, int fvarChannel) const
{
    FVarLevel const &fvarLevel = getFVarLevel(fvarChannel);
...
namespace
{
template <typename INT_TYPE> inline INT_TYPE fastMod4(INT_TYPE value) { return (value & 0x3); }

template <class ARRAY_OF_TYPE, class TYPE> inline TYPE otherOfTwo(ARRAY_OF_TYPE const &arrayOfTwo, TYPE const &value) { return arrayOfTwo[value == arrayOfTwo[0]]; }
} // namespace
...
//  the last face.
//
int Level::gatherQuadRegularRingAroundVertex(Index vIndex, int ringPoints[], int fvarChannel) const
{
...
int Level::gatherQuadRegularPartialRingAroundVertex(Index vIndex, VSpan const &span, int ringPoints[], int fvarChannel) const
{
    Level const &level = *this;
    ...
    for (int i = 0; i < nFaces; ++i)
    {
        ...
        ConstIndexArray fPoints = (fvarChannel < 0) ? level.getFaceVertices(vFaces[fIncident]) : level.getFaceFVarValues(vFaces[fIncident], fvarChannel);
...
//        |     |
//
int Level::gatherQuadLinearPatchPoints(Index thisFace, Index patchPoints[], int rotation, int fvarChannel) const
{
    Level const &level = *this;
    ...
    ConstIndexArray facePoints = (fvarChannel < 0) ? level.getFaceVertices(thisFace) : level.getFaceFVarValues(thisFace, fvarChannel);
...
//         |     |     |     |
//
int Level::gatherQuadRegularInteriorPatchPoints(Index thisFace, Index patchPoints[], int rotation, int fvarChannel) const
{
    Level const &level = *this;
    ...
    ConstIndexArray facePoints = (fvarChannel < 0) ? thisFaceVerts : level.getFaceFVarValues(thisFace, fvarChannel);
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        facePoints = (fvarChannel < 0) ? level.getFaceVertices(intFace) : level.getFaceFVarValues(intFace, fvarChannel);
...
//         |     |     |     |
//
int Level::gatherQuadRegularBoundaryPatchPoints(Index face, Index patchPoints[], int boundaryEdgeInFace, int fvarChannel) const
{
    Level const &level = *this;
    ...
    {
        if (face == v0Faces[i])
            boundaryFaceInV0Faces = i;
        if (face == v1Faces[i])
            boundaryFaceInV1Faces = i;
    }
    ...
    //
    ConstIndexArray thisFacePoints, prevFacePoints, intV0FacePoints, intV1FacePoints, nextFacePoints;
...
//      |     |     |
//
int Level::gatherQuadRegularCornerPatchPoints(Index face, Index patchPoints[], int cornerVertInFace, int fvarChannel) const
{
    Level const &level = *this;
    ...
    //  and copy them into the patch points:
    //
    ConstIndexArray thisFacePoints, prevFacePoints, intFacePoints, nextFacePoints;
...
//             6           7           8
*/
int Level::gatherTriRegularInteriorPatchPoints(Index fIndex, Index points[], int rotation) const
{
    ConstIndexArray fVerts = getFaceVertices(fIndex);
    ConstIndexArray fEdges = getFaceEdges(fIndex);
...
//                   0           1
*/
int Level::gatherTriRegularBoundaryEdgePatchPoints(Index fIndex, Index points[], int boundaryFaceEdge) const
{
    ConstIndexArray fVerts = getFaceVertices(fIndex);
...
//             5           6           7
*/
int Level::gatherTriRegularBoundaryVertexPatchPoints(Index fIndex, Index points[], int boundaryFaceVert) const
{
    ConstIndexArray fVerts = getFaceVertices(fIndex);
    ConstIndexArray fEdges = getFaceEdges(fIndex);
...
//             3           4           5
*/
int Level::gatherTriRegularCornerVertexPatchPoints(Index fIndex, Index points[], int cornerFaceVert) const
{
    ConstIndexArray fVerts = getFaceVertices(fIndex);
...
//                         3
*/
int Level::gatherTriRegularCornerEdgePatchPoints(Index fIndex, Index points[], int cornerFaceEdge) const
{
    ConstIndexArray fVerts = getFaceVertices(fIndex);
...
bool Level::isSingleCreasePatch(Index face, float *sharpnessOut, int *sharpEdgeInFaceOut) const
{
    ...
    if (!(allCornersTag._rule & Sdc::Crease::RULE_CREASE) || (allCornersTag._rule & Sdc::Crease::RULE_CORNER) || (allCornersTag._rule & Sdc::Crease::RULE_DART) || allCornersTag._boundary || allCornersTag._xordinary || allCornersTag._nonManifold)
    {
        return false;
    }
    ...
    //
    int creaseCornerMask = ((getVertexTag(fVerts[0])._rule == Sdc::Crease::RULE_CREASE) << 0) | ((getVertexTag(fVerts[1])._rule == Sdc::Crease::RULE_CREASE) << 1) | ((getVertexTag(fVerts[2])._rule == Sdc::Crease::RULE_CREASE) << 2) |
                           ((getVertexTag(fVerts[3])._rule == Sdc::Crease::RULE_CREASE) << 3);
    static const int sharpEdgeFromCreaseMask[16] = {-1, -1, -1, 0, -1, -1, 1, -1, -1, 3, -1, -1, 2, -1, -1, -1};
    ...
    if (!isSharpnessEqual(getEdgeSharpness(vAEdges[0]), getEdgeSharpness(vAEdges[2])) || !isSharpnessEqual(getEdgeSharpness(vAEdges[1]), getEdgeSharpness(vAEdges[3])) || !isSharpnessEqual(getEdgeSharpness(vBEdges[0]), getEdgeSharpness(vBEdges[2])) ||
...
inline DynamicRelation::DynamicRelation(IndexVector &countAndOffsets, IndexVector &indices, int membersPerComp) : _compCount(0), _memberCountPerComp(membersPerComp), _countsAndOffsets(countAndOffsets), _regIndices(indices)
{
...
inline IndexArray DynamicRelation::getCompMembers(Index compIndex)
{
...
inline void DynamicRelation::appendCompMember(Index compIndex, Index memberValue)
{
    int count  = _countsAndOffsets[2 * compIndex];
    int offset = _countsAndOffsets[2 * compIndex + 1];
...
inline void DynamicRelation::appendComponent()
{
...
int DynamicRelation::compressMemberIndices()
{
...
//
inline Index Level::findEdge(Index v0Index, Index v1Index, ConstIndexArray v0Edges) const
{
    if (v0Index != v1Index)
    {
...
bool Level::completeTopologyFromFaceVertices()
{
    //
...
void Level::populateLocalIndices()
{
    //
...
void Level::orientIncidentComponents()
{
...
namespace
{
inline int findInArray(ConstIndexArray array, Index value) { return (int)(std::find(array.begin(), array.end(), value) - array.begin()); }
} // namespace

bool Level::orderVertexFacesAndEdges(Index vIndex, Index *vFacesOrdered, Index *vEdgesOrdered) const
{
    ...
    if ((fCount == 0) || (eCount < 2) || ((eCount - fCount) > 1))
        return false;
    ...
    while (eCountOrdered < eCount)
    {
        ...
        if (fCountOrdered < fCount)
        {
            ...
            if (eFaces.size() == 0)
                return false;
            if ((eFaces.size() == 1) && (eFaces[0] == fStart))
                return false;
...
bool Level::orderVertexFacesAndEdges(Index vIndex)
{
    IndexArray vFaces = this->getVertexFaces(vIndex);
    IndexArray vEdges = this->getVertexEdges(vIndex);
...
//
int Level::createFVarChannel(int fvarValueCount, Sdc::Options const &fvarOptions)
{
...
void Level::destroyFVarChannel(int channel)
{
    delete _fvarChannels[channel];
    _fvarChannels.erase(_fvarChannels.begin() + channel);
}

int Level::getNumFVarValues(int channel) const { return _fvarChannels[channel]->getNumValues(); }

Sdc::Options Level::getFVarOptions(int channel) const { return _fvarChannels[channel]->getOptions(); }

ConstIndexArray Level::getFaceFVarValues(Index faceIndex, int channel) const { return _fvarChannels[channel]->getFaceValues(faceIndex); }

IndexArray Level::getFaceFVarValues(Index faceIndex, int channel) { return _fvarChannels[channel]->getFaceValues(faceIndex); }

void Level::completeFVarChannelTopology(int channel, int regBoundaryValence) { return _fvarChannels[channel]->completeTopologyFromFaceValues(regBoundaryValence); }
