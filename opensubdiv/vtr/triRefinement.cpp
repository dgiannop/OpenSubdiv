...
//
TriRefinement::TriRefinement(Level const &parentArg, Level &childArg, Sdc::Options const &optionsArg) : Refinement(parentArg, childArg, optionsArg)
{
...
//
void TriRefinement::allocateParentChildIndices()
{
    //
    ...
    _faceChildFaceCountsAndOffsets = IndexArray(&_localFaceChildFaceCountsAndOffsets[0], (int)_localFaceChildFaceCountsAndOffsets.size());
...
//
void TriRefinement::populateFaceVertexRelation()
{
...
void TriRefinement::populateFaceVertexCountsAndOffsets()
{
...
void TriRefinement::populateFaceVerticesFromParentFaces()
{
...
//      - child faces only originate from parent faces
//
void TriRefinement::populateFaceEdgeRelation()
{
...
void TriRefinement::populateFaceEdgesFromParentFaces()
{
    for (Index pFace = 0; pFace < _parent->getNumFaces(); ++pFace)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            bool edgeReversedWrtFace = (pEdgeVerts[0] != pEdgeVerts[1]) && (pFaceVerts[i] != pEdgeVerts[0]);
...
//
void TriRefinement::populateEdgeVertexRelation()
{
...
void TriRefinement::populateEdgeVerticesFromParentFaces()
{
    for (Index pFace = 0; pFace < _parent->getNumFaces(); ++pFace)
    {
...
void TriRefinement::populateEdgeVerticesFromParentEdges()
{
...
//
void TriRefinement::populateEdgeFaceRelation()
{
    //
    ...
    //
    int childEdgeFaceIndexSizeEstimate = (int)_faceChildEdgeIndices.size() * 2 + (int)_parent->_edgeFaceIndices.size() * 2;
...
void TriRefinement::populateEdgeFacesFromParentFaces()
{
    for (Index pFace = 0; pFace < _parent->getNumFaces(); ++pFace)
    {
...
void TriRefinement::populateEdgeFacesFromParentEdges()
{
    for (Index pEdge = 0; pEdge < _parent->getNumEdges(); ++pEdge)
    {
        ...
        for (int j = 0; j < 2; ++j)
        {
            ...
            for (int i = 0; i < pEdgeFaces.size(); ++i)
            {
                ...
                if (childInFace == pFaceVerts.size())
                    childInFace = 0;
...
//
void TriRefinement::populateVertexFaceRelation()
{
    //
    ...
    //
    int childVertFaceIndexSizeEstimate = (int)_parent->_edgeFaceIndices.size() * 3 + (int)_parent->_vertFaceIndices.size();
...
void TriRefinement::populateVertexFacesFromParentEdges()
{
    for (Index pEdge = 0; pEdge < _parent->getNumEdges(); ++pEdge)
    {
...
//          - note this imposes ordering constraints and inhibits concurrency
//
void TriRefinement::populateVertexEdgeRelation()
{
    //
    ...
    //
    int childVertEdgeIndexSizeEstimate = (int)_parent->_edgeFaceIndices.size() * 2 + _parent->getNumEdges() * 2 + (int)_parent->_vertEdgeIndices.size();
...
void TriRefinement::populateVertexEdgesFromParentEdges()
{
    for (Index pEdge = 0; pEdge < _parent->getNumEdges(); ++pEdge)
    {
        Index cVert = _edgeChildVertIndex[pEdge];
        ...
        Index cEdgeOfEdge0 = INDEX_INVALID, cEdgeOfEdge1 = INDEX_INVALID;
...
void TriRefinement::populateVertexEdgesFromParentVertices()
{
    for (Index pVert = 0; pVert < _parent->getNumVertices(); ++pVert)
    {
        Index cVert = _vertChildVertIndex[pVert];
...
void TriRefinement::markSparseFaceChildren()
{
    ...
    for (Index pFace = 0; pFace < parent().getNumFaces(); ++pFace)
    {
        ...
        {
            int marked = _parentVertexTag[fVerts[0]]._selected + _parentVertexTag[fVerts[1]]._selected + _parentVertexTag[fVerts[2]]._selected;
