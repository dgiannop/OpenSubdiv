...
template <class T> class HbrHierarchicalEdit
{
  ...
  protected:
    HbrHierarchicalEdit(int _faceid, int _nsubfaces, unsigned char *_subfaces) : faceid(_faceid), nsubfaces(_nsubfaces)
    {
    ...
    HbrHierarchicalEdit(int _faceid, int _nsubfaces, int *_subfaces) : faceid(_faceid), nsubfaces(_nsubfaces)
    {
        subfaces = new unsigned char[_nsubfaces];
        for (int i = 0; i < nsubfaces; ++i)
        {
...
{
  public:
    bool operator()(const HbrHierarchicalEdit<T> *path1, const HbrHierarchicalEdit<T> *path2) const { return (*path1 < *path2); }
};
...
template <class T> bool HbrHierarchicalEdit<T>::IsRelevantToFace(HbrFace<T> *face) const
{
