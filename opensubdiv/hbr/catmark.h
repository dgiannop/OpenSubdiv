...
{
  public:
    HbrCatmarkSubdivision<T>() : HbrSubdivision<T>(), triangleSubdivision(k_Normal) {}

    HbrCatmarkSubdivision<T>(const HbrCatmarkSubdivision<T> &old) : HbrSubdivision<T>(), triangleSubdivision(old.triangleSubdivision) {}

    virtual HbrSubdivision<T> *Clone() const { return new HbrCatmarkSubdivision<T>(*this); }
...
template <class T> void HbrCatmarkSubdivision<T>::transferFVarToChild(HbrMesh<T> *mesh, HbrFace<T> *face, HbrFace<T> *child, int index)
{
    ...
    for (int fvaritem = 0; fvaritem < fvarcount; ++fvaritem)
    {
        ...
        if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone || (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryAlwaysSharp && fvarmask >= 1) || v->GetSharpness() > HbrVertex<T>::k_Smooth || infcorner)
        {
        ...
        if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone || edge->GetFVarSharpness(fvaritem) || edge->IsBoundary())
        {
            // Sharp edge rule
...
template <class T> void HbrCatmarkSubdivision<T>::transferEditsToChild(HbrFace<T> *face, HbrFace<T> *child, int index)
{
    // Hand down hole tag
    ...
    if (HbrHierarchicalEdit<T> **edits = face->GetHierarchicalEdits())
    {
        while (HbrHierarchicalEdit<T> *edit = *edits)
        {
            if (!edit->IsRelevantToFace(face))
                break;
            if (edit->GetNSubfaces() > face->GetDepth() && (edit->GetSubface(face->GetDepth()) == index))
            {
...
template <class T> void HbrCatmarkSubdivision<T>::Refine(HbrMesh<T> *mesh, HbrFace<T> *face)
{
...
template <class T> HbrFace<T> *HbrCatmarkSubdivision<T>::RefineFaceAtVertex(HbrMesh<T> *mesh, HbrFace<T> *face, HbrVertex<T> *vertex)
{
#ifdef HBR_DEBUG
...
template <class T> void HbrCatmarkSubdivision<T>::GuaranteeNeighbor(HbrMesh<T> *mesh, HbrHalfedge<T> *edge)
{
    if (edge->GetOpposite())
    {
        return;
...
template <class T> void HbrCatmarkSubdivision<T>::GuaranteeNeighbors(HbrMesh<T> *mesh, HbrVertex<T> *vertex)
{
#ifdef HBR_DEBUG
...
template <class T> bool HbrCatmarkSubdivision<T>::HasLimit(HbrMesh<T> *mesh, HbrFace<T> *face)
{
    if (face->IsHole())
        return false;
...
template <class T> bool HbrCatmarkSubdivision<T>::HasLimit(HbrMesh<T> *mesh, HbrHalfedge<T> *edge)
{
    ...
    if (!HasLimit(mesh, edge->GetOrgVertex()) || !HasLimit(mesh, edge->GetDestVertex()))
        return false;

    return !edge->IsBoundary();
}

template <class T> bool HbrCatmarkSubdivision<T>::HasLimit(HbrMesh<T> * /* mesh */, HbrVertex<T> *vertex)
{
...
template <class T> HbrVertex<T> *HbrCatmarkSubdivision<T>::Subdivide(HbrMesh<T> *mesh, HbrFace<T> *face)
{
...
template <class T> HbrVertex<T> *HbrCatmarkSubdivision<T>::Subdivide(HbrMesh<T> *mesh, HbrHalfedge<T> *edge)
{
...
template <class T> HbrVertex<T> *HbrCatmarkSubdivision<T>::Subdivide(HbrMesh<T> *mesh, HbrVertex<T> *vertex)
{
    ...
    for (int i = 0; i < passes; ++i)
    {
        ...
        case HbrVertex<T>::k_Dart:
        {
            ...
            // subdivided vertices
            HbrSubdivision<T>::AddSurroundingVerticesWithWeight(mesh, vertex, weights[i] * invvalencesquared, &data);
        ...
        case HbrVertex<T>::k_Crease:
        {
            ...
            // sharp edges
            HbrSubdivision<T>::AddCreaseEdgesWithWeight(mesh, vertex, i == 1, weights[i] * 0.125f, &data);
            break;
        }
        case HbrVertex<T>::k_Corner:
        default:
