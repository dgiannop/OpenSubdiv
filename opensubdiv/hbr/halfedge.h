...
template <class T> class HbrHalfedge
{
  private:
    HbrHalfedge()
        : opposite(0), incidentVertex(-1), vchild(-1), sharpness(0.0f)
#ifdef HBRSTITCH
          ,
  ...
  public:
    ...
    {
        const size_t edgesize = (m_index == 4) ? (sizeof(HbrHalfedge<T>) + sizeof(HbrFace<T> *)) : sizeof(HbrHalfedge<T>);
        if (firstedge)
        {
            return (HbrHalfedge<T> *)((char *)this + (GetFace()->GetNumVertices() - 1) * edgesize);
        }
    ...
    HbrVertex<T> *GetVertex() const { return GetMesh()->GetVertex(incidentVertex); }

    // Returns the incident vertex
    HbrVertex<T> *GetVertex(HbrMesh<T> *mesh) const { return mesh->GetVertex(incidentVertex); }

    // Returns the incident vertex
    int GetVertexID() const { return incidentVertex; }

    // Returns the source vertex
    HbrVertex<T> *GetOrgVertex() const { return GetVertex(); }

    // Returns the source vertex
    HbrVertex<T> *GetOrgVertex(HbrMesh<T> *mesh) const { return GetVertex(mesh); }

    // Returns the source vertex id
    int GetOrgVertexID() const { return incidentVertex; }
    ...
    HbrFace<T> *GetFace() const
    {
        ...
        {
            return (HbrFace<T> *)((char *)this - (m_index) * sizeof(HbrHalfedge<T>) - offsetof(HbrFace<T>, edges));
        }
    }
    ...
    void ClearMask()
    {
        GetOrgVertex()->ClearMask();
        GetDestVertex()->ClearMask();
    }
    ...
    StitchEdge *GetRayStitchEdge(int i) { return GetStitchEdge(i + 2); }
    ...
    void SplitRayStitchEdge(int i) { SplitStitchEdge(i + 2); }
    ...
    {
        if (stitchdatavalid)
            return GetMesh()->GetStitchData(this);
        else
            return 0;
    }
    ...
    void ClearStitchCCW(bool raytraced)
    {
        if (raytraced)
        {
            raystitchccw = 0;
            if (opposite)
                opposite->raystitchccw = 0;
        }
        else
        {
            stitchccw = 0;
            if (opposite)
                opposite->stitchccw = 0;
        }
    }
    ...
    void ToggleStitchCCW(bool raytraced)
    {
        if (raytraced)
        {
            ...
            if (opposite)
                opposite->raystitchccw = raystitchccw;
        }
        else
        {
            stitchccw = 1 - stitchccw;
            if (opposite)
                opposite->stitchccw = stitchccw;
        }
    }
  ...
  private:
    ...
    int getIndex() const
    {
        ...
        else
        {
            ...
            return int(((char *)this - incidentFace->extraedges) / (sizeof(HbrHalfedge<T>) + sizeof(HbrFace<T> *)));
        }
    }
...
#ifdef HBRSTITCH
    StitchEdge **getStitchEdges() { return GetFace()->stitchEdges + GetMesh()->GetStitchCount() * getIndex(); }
#endif
...
template <class T> void HbrHalfedge<T>::Initialize(HbrHalfedge<T> *opposite, int index, HbrVertex<T> *origin, unsigned int *fvarbits, HbrFace<T> *face)
{
    ...
    if (opposite)
    {
        sharpness = opposite->sharpness;
#ifdef HBRSTITCH
        StitchEdge **stitchEdges = face->stitchEdges + mesh->GetStitchCount() * index;
...
template <class T> HbrHalfedge<T>::~HbrHalfedge() { Clear(); }

template <class T> void HbrHalfedge<T>::Clear()
{
    if (opposite)
    {
        opposite->opposite = 0;
...
template <class T> HbrVertex<T> *HbrHalfedge<T>::Subdivide()
{
    ...
    if (opposite && opposite->vchild != -1)
        return mesh->GetVertex(opposite->vchild);
...
template <class T> void HbrHalfedge<T>::GuaranteeNeighbor()
{
    HbrMesh<T> *mesh = GetMesh();
...
// on the edge.
template <class T> bool HbrHalfedge<T>::GetFVarInfiniteSharp(int datum)
{
    ...
    for (i = 0; i < nv; ++i)
    {
        if (e->GetOrgVertex() == GetOrgVertex())
            lorg = i;
        if (e->GetOrgVertex() == GetDestVertex())
            ldst = i;
        e = e->GetNext();
    }
    ...
    if (!right->GetFVarData(rorg).Compare(left->GetFVarData(lorg), startindex, width, 0.001f) || !right->GetFVarData(rdst).Compare(left->GetFVarData(ldst), startindex, width, 0.001f))
    {
        bits = ~(0x2 << shift);
        fvarinfsharp[intindex] &= bits;
        if (opposite)
...
template <class T> bool HbrHalfedge<T>::IsFVarInfiniteSharpAnywhere()
{
    ...
    {
        if (GetFVarInfiniteSharp(i))
            return true;
    }
    return false;
}

template <class T> float HbrHalfedge<T>::GetFVarSharpness(int datum, bool ignoreGeometry)
{
    if (GetFVarInfiniteSharp(datum))
...
{
  public:
    bool operator()(const HbrHalfedge<T> *a, HbrHalfedge<T> *b) const { return (a->GetFace()->GetPath() < b->GetFace()->GetPath()); }
};
