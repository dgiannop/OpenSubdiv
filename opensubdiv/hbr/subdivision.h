...
{
  public:
    HbrSubdivision<T>() : creaseSubdivision(k_CreaseNormal) {}
  ...
  private:
    ...
    {
      public:
        SmoothSubdivisionVertexOperator(T *data, bool meshHasEdits, float weight) : m_data(data), m_meshHasEdits(meshHasEdits), m_weight(weight) {}
    ...
    {
      public:
        CreaseSubdivisionHalfedgeOperator(HbrVertex<T> *vertex, T *data, bool meshHasEdits, bool next, float weight) : m_vertex(vertex), m_data(data), m_meshHasEdits(meshHasEdits), m_next(next), m_weight(weight), m_count(0) {}
  ...
  private:
    ...
    {
      public:
        RefineFaceAtVertexOperator(HbrSubdivision<T> *subdivision, HbrMesh<T> *mesh, HbrVertex<T> *vertex) : m_subdivision(subdivision), m_mesh(mesh), m_vertex(vertex) {}
        virtual void operator()(HbrFace<T> &face) { m_subdivision->RefineFaceAtVertex(m_mesh, &face, m_vertex); }
...
template <class T> void HbrSubdivision<T>::RefineAtVertex(HbrMesh<T> *mesh, HbrVertex<T> *vertex)
{
...
template <class T> void HbrSubdivision<T>::SubdivideCreaseWeight(HbrHalfedge<T> *edge, HbrVertex<T> *vertex, HbrHalfedge<T> *subedge)
{
    ...
    else if (creaseSubdivision == HbrSubdivision<T>::k_CreaseChaikin)
    {
        ...
        {
          public:
            ChaikinEdgeCreaseOperator(HbrHalfedge<T> const *edge, float &childsharp, int &count) : m_edge(edge), m_childsharp(childsharp), m_count(count) {}
...
template <class T> void HbrSubdivision<T>::AddSurroundingVerticesWithWeight(HbrMesh<T> *mesh, HbrVertex<T> *vertex, float weight, T *data)
{
    SmoothSubdivisionVertexOperator op(data, mesh->HasVertexEdits(), weight);
    vertex->ApplyOperatorSurroundingVertices(op);
}

template <class T> void HbrSubdivision<T>::AddCreaseEdgesWithWeight(HbrMesh<T> *mesh, HbrVertex<T> *vertex, bool next, float weight, T *data)
{
