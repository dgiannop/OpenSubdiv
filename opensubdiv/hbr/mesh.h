...
template <class T> class HbrMesh
{
  public:
    ...
    template <typename OutputIterator> void GetVertices(OutputIterator vertices) const;
    ...
    const std::vector<HbrHierarchicalEdit<T> *> &GetHierarchicalEdits() const { return hierarchicalEdits; }

    // Return the hierarchical edits associated with the mesh at an
    // offset
    HbrHierarchicalEdit<T> **GetHierarchicalEditsAtOffset(int offset) { return &hierarchicalEdits[offset]; }
    ...
    HbrFaceChildren<T> *NewFaceChildren() { return m_faceChildrenAllocator.Allocate(); }

    // Recycle face children block used by HbrFace
    void DeleteFaceChildren(HbrFaceChildren<T> *facechildren) { m_faceChildrenAllocator.Deallocate(facechildren); }

#ifdef HBRSTITCH
    void *GetStitchData(const HbrHalfedge<T> *edge) const
    {
        typename TgHashMap<const HbrHalfedge<T> *, void *>::const_iterator i = stitchData.find(edge);
        if (i != stitchData.end())
        {
            return i->second;
        }
    ...
    void SetStitchData(const HbrHalfedge<T> *edge, void *data) { stitchData[edge] = data; }
#endif
...
#ifdef HBR_ADAPTIVE
  public:
    std::vector<std::pair<int, int>> const &GetSplitVertices() const { return m_splitVertices; }

  protected:
    friend class HbrVertex<T>;

    void addSplitVertex(int splitIdx, int orgIdx) { m_splitVertices.push_back(std::pair<int, int>(splitIdx, orgIdx)); }
...
HbrMesh<T>::HbrMesh(HbrSubdivision<T> *s, int _fvarcount, const int *_fvarindices, const int *_fvarwidths, int _totalfvarwidth
    ...
    : subdivision(s), fvarcount(_fvarcount), fvarindices(_fvarindices), fvarwidths(_fvarwidths), totalfvarwidth(_totalfvarwidth),
#ifdef HBRSTITCH
      stitchCount(_stitchCount),
#endif
      nvertices(0), nfaces(0), maxVertexID(0), maxFaceID(0), maxUniformIndex(0), interpboundarymethod(k_InterpolateBoundaryNone), fvarinterpboundarymethod(k_InterpolateBoundaryNone), fvarpropagatecorners(false), s_memStatsIncrement(0),
      s_memStatsDecrement(0), m_faceSize(sizeof(HbrFace<T>) + 4 * ((fvarcount + 15) / 16 * sizeof(unsigned int)
#ifdef HBRSTITCH
                                                                   + stitchCount * sizeof(StitchEdge *)
#endif
                                                                       )),
      m_faceAllocator(&m_memory, 512, 0, 0, m_faceSize), m_vertexSize(sizeof(HbrVertex<T>) + (totalfvarwidth ? (sizeof(HbrFVarData<T>) + (totalfvarwidth - 1) * sizeof(float)) : 0)), m_vertexAllocator(&m_memory, 512, 0, 0, m_vertexSize),
      m_faceChildrenAllocator(&m_memory, 512, 0, 0), m_memory(0), m_numCoarseFaces(-1), hasVertexEdits(0), hasCreaseEdits(0), m_transientMode(false)
{
}
...
template <class T> HbrMesh<T>::~HbrMesh()
{
    ...
    for (typename std::vector<HbrHierarchicalEdit<T> *>::iterator hi = hierarchicalEdits.begin(); hi != hierarchicalEdits.end(); ++hi)
    {
        delete *hi;
    }
}
...
template <class T> HbrFace<T> *HbrMesh<T>::NewFace(int nv, const int *vtx, int uindex)
{
...
template <class T> HbrFace<T> *HbrMesh<T>::NewFace(int nv, HbrVertex<T> **vtx, HbrFace<T> *parent, int childindex)
{
    HbrFace<T> *f = 0;
    // Resize if needed
    if (nfaces <= maxFaceID)
...
template <class T> void HbrMesh<T>::Finish()
{
    ...
    for (typename std::vector<HbrVertex<T> *>::iterator vi = vertexlist.begin(); vi != vertexlist.end(); ++vi)
    {
...
template <class T> template <typename OutputIterator> void HbrMesh<T>::GetVertices(OutputIterator lvertices) const
{
    for (int i = 0; i < nvertices; ++i)
    {
        if (vertices[i])
...
template <class T> void HbrMesh<T>::ApplyOperatorAllVertices(HbrVertexOperator<T> &op) const
{
    for (int i = 0; i < nvertices; ++i)
    {
        if (vertices[i])
...
template <class T> template <typename OutputIterator> void HbrMesh<T>::GetFaces(OutputIterator lfaces) const
{
    for (int i = 0; i < nfaces; ++i)
    {
        if (faces[i])
...
template <class T> void HbrMesh<T>::AddHierarchicalEdit(HbrHierarchicalEdit<T> *edit)
{
    hierarchicalEdits.push_back(edit);
    if (dynamic_cast<HbrVertexEdit<T> *>(edit) || dynamic_cast<HbrMovingVertexEdit<T> *>(edit))
    {
        hasVertexEdits = 1;
    }
...
template <class T> void HbrMesh<T>::FreeTransientData()
{
    ...
    for (typename std::vector<HbrFace<T> *>::iterator fi = m_transientFaces.begin(); fi != m_transientFaces.end(); ++fi)
    {
        DeleteFace(*fi);
    }
    // The vertices should now be trivial to purge after the transient
    // faces have been cleared
    for (typename std::vector<HbrVertex<T> *>::iterator vi = m_transientVertices.begin(); vi != m_transientVertices.end(); ++vi)
    {
        DeleteVertex(*vi);
    }
