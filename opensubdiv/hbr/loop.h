...
template <class T> class HbrLoopSubdivision : public HbrSubdivision<T>
{
  public:
    ...
    virtual HbrSubdivision<T> *Clone() const { return new HbrLoopSubdivision<T>(); }
...
template <class T> void HbrLoopSubdivision<T>::transferFVarToChild(HbrMesh<T> *mesh, HbrFace<T> *face, HbrFace<T> *child, int index)
{
    ...
    if (index == 3)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            for (int fvaritem = 0; fvaritem < fvarcount; ++fvaritem)
            {
                ...
                if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone || face->GetEdge(i)->GetFVarSharpness(fvaritem) || face->GetEdge(i)->IsBoundary())
                {
                    // Sharp edge rule
...
template <class T> void HbrLoopSubdivision<T>::transferEditsToChild(HbrFace<T> *face, HbrFace<T> *child, int index)
{
    // Hand down hole tag
...
template <class T> void HbrLoopSubdivision<T>::Refine(HbrMesh<T> *mesh, HbrFace<T> *face)
{
#ifdef HBR_DEBUG
    std::cerr << "\n\nRefining face " << *face << "\n";
#endif
...
template <class T> HbrFace<T> *HbrLoopSubdivision<T>::RefineFaceAtVertex(HbrMesh<T> *mesh, HbrFace<T> *face, HbrVertex<T> *vertex)
{
#ifdef HBR_DEBUG
...
template <class T> void HbrLoopSubdivision<T>::GuaranteeNeighbor(HbrMesh<T> *mesh, HbrHalfedge<T> *edge)
{
    if (edge->GetOpposite())
    {
        return;
...
template <class T> void HbrLoopSubdivision<T>::GuaranteeNeighbors(HbrMesh<T> *mesh, HbrVertex<T> *vertex)
{
#ifdef HBR_DEBUG
...
template <class T> bool HbrLoopSubdivision<T>::HasLimit(HbrMesh<T> *mesh, HbrFace<T> *face)
{
    if (face->IsHole())
        return false;
...
template <class T> bool HbrLoopSubdivision<T>::HasLimit(HbrMesh<T> *mesh, HbrHalfedge<T> *edge)
{
...
template <class T> bool HbrLoopSubdivision<T>::HasLimit(HbrMesh<T> * /* mesh */, HbrVertex<T> *vertex)
{
...
template <class T> HbrVertex<T> *HbrLoopSubdivision<T>::Subdivide(HbrMesh<T> * /* mesh */, HbrFace<T> * /* face */)
{
...
template <class T> HbrVertex<T> *HbrLoopSubdivision<T>::Subdivide(HbrMesh<T> *mesh, HbrHalfedge<T> *edge)
{
#ifdef HBR_DEBUG
...
template <class T> HbrVertex<T> *HbrLoopSubdivision<T>::Subdivide(HbrMesh<T> *mesh, HbrVertex<T> *vertex)
{
    ...
    for (int i = 0; i < passes; ++i)
    {
        ...
        case HbrVertex<T>::k_Dart:
        {
            ...
            HbrSubdivision<T>::AddSurroundingVerticesWithWeight(mesh, vertex, weights[i] * beta, &data);
            break;
        }
        case HbrVertex<T>::k_Crease:
        {
...
template <class T> void HbrLoopSubdivision<T>::refineFaceAtMiddle(HbrMesh<T> *mesh, HbrFace<T> *face)
{
#ifdef HBR_DEBUG
