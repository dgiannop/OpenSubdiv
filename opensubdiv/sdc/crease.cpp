...
//
Crease::Rule Crease::DetermineVertexVertexRule(float vertexSharpness, int sharpEdgeCount) const
{
    if (IsSharp(vertexSharpness))
        return Crease::RULE_CORNER;

    return (sharpEdgeCount > 2) ? Crease::RULE_CORNER : (Crease::Rule)(1 << sharpEdgeCount);
}

Crease::Rule Crease::DetermineVertexVertexRule(float vertexSharpness, int incidentEdgeCount, float const *incidentEdgeSharpness) const
{
    if (IsSharp(vertexSharpness))
        return Crease::RULE_CORNER;
...
float Crease::ComputeFractionalWeightAtVertex(float parentVertexSharpness, float childVertexSharpness, int incidentEdgeCount, float const *parentSharpness, float const *childSharpness) const
{
...
float Crease::SubdivideEdgeSharpnessAtVertex(float edgeSharpness, int incEdgeCountAtVertex, float const *incEdgeSharpness) const
{
    ...
    if (IsSmooth(edgeSharpness))
        return Crease::SHARPNESS_SMOOTH;
    if (IsInfinite(edgeSharpness))
...
void Crease::SubdivideEdgeSharpnessesAroundVertex(int edgeCount, float const *parentSharpness, float *childSharpness) const
{
