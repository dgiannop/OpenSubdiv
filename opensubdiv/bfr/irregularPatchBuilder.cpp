...
//
IrregularPatchBuilder::IrregularPatchBuilder(FaceSurface const &surfaceDescription, Options const &options) : _surface(surfaceDescription), _options(options) { initializeControlHullInventory(); }
...
inline IrregularPatchBuilder::Index const *IrregularPatchBuilder::getCornerIndices(int corner) const { return getSurfaceIndices() + _cornerHullInfo[corner].surfaceIndicesOffset; }

inline IrregularPatchBuilder::Index const *IrregularPatchBuilder::getBaseFaceIndices() const
{
    FaceVertex const &corner0 = _surface.GetCornerTopology(0);
    return getSurfaceIndices() + corner0.GetFaceIndexOffset(corner0.GetFace());
}

inline IrregularPatchBuilder::Index const *IrregularPatchBuilder::getCornerFaceIndices(int corner, int face) const { return getCornerIndices(corner) + _surface.GetCornerTopology(corner).GetFaceIndexOffset(face); }

inline int IrregularPatchBuilder::getLocalControlVertex(Index meshVertIndex) const { return _controlVertMap.find(meshVertIndex)->second; }

inline IrregularPatchBuilder::Index IrregularPatchBuilder::getMeshControlVertex(int localVertIndex) const { return _controlVerts[localVertIndex]; }
...
//  those iterations.
//
void IrregularPatchBuilder::initializeControlHullInventory()
{
    //
    ...
    for (int corner = 0; corner < faceSize; ++corner)
    {
        ...
        if (cSub._numFacesAfter)
        {
            ...
            else if ((cSub._numFacesTotal == 3) && (cTop.GetFaceSize(cTop.GetFaceAfter(2)) == 3))
            {
...
void IrregularPatchBuilder::addMeshControlVertex(Index meshVertIndex)
{
...
void IrregularPatchBuilder::addMeshControlVertices(Index const fVerts[], int fSize)
{
...
void IrregularPatchBuilder::initializeControlVertexMap()
{
    //
    ...
    for (int corner = 0; corner < faceSize; ++corner)
    {
        ...
        if (cHull.singleSharedFace)
        {
            ...
            addMeshControlVertices(getCornerFaceIndices(corner, nextFace), cTop.GetFaceSize(nextFace));
            continue;
        }
...
//  rest are internal:
//
int IrregularPatchBuilder::GatherControlVertexIndices(Index cvIndices[]) const
{
    //
    ...
    for (int corner = 0; corner < faceSize; ++corner)
    {
        ...
        {
            assert(!cSub.IsBoundary() && (cSub._numFacesTotal == 3) && (cTop.GetFaceSize(cTop.GetFaceAfter(2)) == 3));

            cvIndices[numIndices++] = getCornerFaceIndices(corner, cTop.GetFaceAfter(2))[1];
            continue;
        }
        ...
        if (cSub._numFacesAfter && cSub.IsBoundary())
        {
            ...
            cvIndices[numIndices++] = cTop.GetFaceIndexTrailing(nextFace, getCornerIndices(corner));
        }
        if (cSub._numFacesBefore)
        {
            int nextFace = cTop.GetFaceFirst(cSub);
...
int IrregularPatchBuilder::gatherControlFaces(int faceSizes[], int faceVertices[]) const
{
    //
    ...
    *faceSizes++ = faceSize;
    ...
    for (int corner = 0; corner < faceSize; ++corner)
    {
        ...
        if (cHull.singleSharedFace)
        {
            ...
            getControlFaceVertices(faceVerts, faceSize, corner, getCornerFaceIndices(corner, cTop.GetFaceAfter(1)));
            *faceSizes++ = faceSize;
            faceVerts += faceSize;
            continue;
        }
        ...
        if (cSub._numFacesAfter > 1)
        {
            ...
            for (int j = 0; j < N; ++j)
            {
                ...
                if (_useControlVertMap)
                {
                    getControlFaceVertices(faceVerts, S, corner, getCornerFaceIndices(corner, nextFace));
                }
                ...
                {
                    getControlFaceVertices(faceVerts, S, corner, nextVert, (j == (N - 1)));
                }
                *faceSizes++ = S;
                faceVerts += S;
...
int IrregularPatchBuilder::gatherControlVertexSharpness(int vertIndices[], float vertSharpness[]) const
{
    int nSharpVerts = 0;
    for (int i = 0; i < _surface.GetFaceSize(); ++i)
    {
        ...
        {
            vertSharpness[nSharpVerts] = (cSub._localSharpness > 0.0f) ? cSub._localSharpness : _surface.GetCornerTopology(i).GetVertexSharpness();
            vertIndices[nSharpVerts++] = i;
        }
    }
    return nSharpVerts;
...
int IrregularPatchBuilder::gatherControlEdgeSharpness(int edgeVertPairs[], float edgeSharpness[]) const
{
    //
        ...
        if (cSub._numFacesAfter > 1)
        {
            ...
            for (int i = 1; i < cSub._numFacesAfter; ++i)
            {
                ...
                if (Sdc::Crease::IsSharp(sharpness))
                {
                    ...
                    if (_useControlVertMap)
                    {
                        edgeVert = getLocalControlVertex(cTop.GetFaceIndexTrailing(nextFace, cVerts));
                    }
...
//
void IrregularPatchBuilder::getControlFaceVertices(int fVerts[], int numFVerts, int corner, Index const srcVerts[]) const
{
    assert(_useControlVertMap);
...
void IrregularPatchBuilder::getControlFaceVertices(int fVerts[], int numFVerts, int corner, int nextPerimeterVert) const
{
    *fVerts++ = corner;
    for (int i = 1; i < numFVerts; ++i)
    {
...
void IrregularPatchBuilder::getControlFaceVertices(int fVerts[], int numFVerts, int corner, int nextPerimeterVert, bool lastFace) const
{
    int S = numFVerts;
...
//
bool doFacesMatch(int size, int const a[], int const b[], int bStart)
{
...
bool doFacesMatch(int size, int const a[], int const b[])
{
...
void IrregularPatchBuilder::removeDuplicateControlFaces(int faceSizes[], int faceVerts[], int *numFaces, int *numFaceVerts) const
{
    //
    ...
    for (int i = *numFaces - 1; i > 1; --i)
    {
        ...
        if (isDuplicate)
        {
            if (numSizesAfter)
            {
                std::memmove(sizesAfter - 1, sizesAfter, numSizesAfter * sizeof(int));
                std::memmove(vertsAfter - iSize, vertsAfter, numVertsAfter * sizeof(int));
            }
            (*numFaces)--;
            (*numFaceVerts) -= iSize;
        }
...
void IrregularPatchBuilder::sharpenBoundaryControlEdges(int edgeVertPairs[], float edgeSharpness[], int *numSharpEdges) const
{
    //
...
//
internal::IrregularPatchSharedPtr IrregularPatchBuilder::Build()
{
    //
    ...
    numCorners = _surface.GetTag().HasSharpVertices() ? gatherControlVertexSharpness(cornerIndices, cornerWeights) : 0;

    numCreases = _surface.GetTag().HasSharpEdges() ? gatherControlEdgeSharpness(creaseIndices, creaseWeights) : 0;
    ...
    if (controlFacesMayOverlap())
    {
        if (numFaces > 2)
        {
            removeDuplicateControlFaces(faceSizes, faceVerts, &numFaces, &numFaceVerts);
        }
        if (_surface.GetTag().HasBoundaryVertices())
        {
            sharpenBoundaryControlEdges(creaseIndices, creaseWeights, &numCreases);
        }
    }
    ...
    Far::TopologyRefiner *refiner = RefinerFactory::Create(topDescriptor, refinerOptions);
