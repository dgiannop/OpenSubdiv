...
namespace
{
inline int encodeTriBoundaryMask(int eBits, int vBits)
{
    int upperBits = 0;
    int lowerBits = eBits;
...
//  Static methods:
//
int RegularPatchBuilder::GetBoundaryMask(int faceSize, Index const cvs[])
{
    bool isQuad = (faceSize == 4);
    if (isQuad)
    {
        int eMask = ((cvs[1] < 0) << 0) | ((cvs[7] < 0) << 1) | ((cvs[14] < 0) << 2) | ((cvs[8] < 0) << 3);
        return eMask;
    }
    else
    {
        int eMask = ((cvs[1] < 0) << 0) | ((cvs[9] < 0) << 1) | ((cvs[7] < 0) << 2);
        int vMask = (((cvs[0] < 0) | (cvs[3] < 0)) << 0) | (((cvs[2] < 0) | (cvs[6] < 0)) << 1) | (((cvs[10] < 0) | (cvs[11] < 0)) << 2);
...
//
RegularPatchBuilder::RegularPatchBuilder(FaceSurface const &surface) : _surface(surface)
{
    ...
    else if (_isQuad)
    {
        ...
        int eMask = ((C[0].IsBoundary() & (C[0]._numFacesBefore == 0)) << 0) | ((C[1].IsBoundary() & (C[1]._numFacesBefore == 0)) << 1) | ((C[2].IsBoundary() & (C[2]._numFacesBefore == 0)) << 2) | ((C[3].IsBoundary() & (C[3]._numFacesBefore == 0)) << 3);
        _boundaryMask = eMask;
    }
    else
    {
        ...
        FaceVertexSubset const *C     = _surface.GetSubsets();
        int                     eMask = ((C[0].IsBoundary() & (C[0]._numFacesBefore == 0)) << 0) | ((C[1].IsBoundary() & (C[1]._numFacesBefore == 0)) << 1) | ((C[2].IsBoundary() & (C[2]._numFacesBefore == 0)) << 2);
        int                     vMask = (C[0].IsBoundary() << 0) | (C[1].IsBoundary() << 1) | (C[2].IsBoundary() << 2);
...
//
void RegularPatchBuilder::gatherInteriorPatchPoints4(Index P[]) const
{
...
void RegularPatchBuilder::gatherBoundaryPatchPoints4(Index P[]) const
{
    Index const *fvIndices = &_surface.GetIndices()[0];
...
void RegularPatchBuilder::gatherInteriorPatchPoints3(Index P[]) const
{
    Index const *fvIndices = &_surface.GetIndices()[0];
...
void RegularPatchBuilder::gatherBoundaryPatchPoints3(Index P[]) const
{
    Index const *fvIndices = &_surface.GetIndices()[0];
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        case 0:
            ...
            if (!cSub.IsBoundary())
            {
                P[7]    = fvOther[1];
                P[3]    = fvOther[2];
                fvOther = fvIndices + cTop.GetFaceIndexOffset(cTop.GetFaceNext(faceOther));
                P[0]    = fvOther[2];
            }
            else
            {
                P[7] = (cSub._numFacesAfter) ? fvOther[3 - cSub._numFacesAfter] : fvPhantom;
        ...
        case 1:
            ...
            {
                P[1] = (cSub._numFacesAfter) ? fvOther[3 - cSub._numFacesAfter] : fvPhantom;
        ...
        case 2:
            ...
            {
                P[9]  = (cSub._numFacesAfter) ? fvOther[3 - cSub._numFacesAfter] : fvPhantom;
...
int RegularPatchBuilder::GatherControlVertexIndices(Index cvIndices[]) const
{
    if (_isQuad)
    {
        if (_isBoundary)
...
//
void RegularPatchBuilder::print(Index const P[]) const
{
