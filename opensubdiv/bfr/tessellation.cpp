...
template <typename REAL> class Coord2
{
  public:
...
template <typename REAL> class Coord2Array
{
  public:
    ...
    Coord2Array operator+(int offset) { return Coord2Array(_data + offset * _stride, _stride); }

    Coord2<REAL> operator[](int index) { return Coord2<REAL>(_data + index * _stride); }
...
class Facet
{
  public:
    ...
    void Set(int a, int b, int c, int d) { _T[0] = a, _T[1] = b, _T[2] = c, _T[3] = d; }
...
{
  public:
    FacetArray(int *data, int size, int stride) : _data(data), _size(size), _stride(stride) {}
    ~FacetArray() {}

    FacetArray operator+(int offset) { return FacetArray(_data + offset * _stride, _size, _stride); }

    Facet operator[](int index) { return Facet(_data + index * _stride, _size); }
...
//
template <typename REAL> inline int appendUIsoLine(Coord2Array<REAL> P, int n, REAL u, REAL v, REAL dv)
{
...
template <typename REAL> inline int appendVIsoLine(Coord2Array<REAL> P, int n, REAL u, REAL v, REAL du)
{
...
template <typename REAL> inline int appendUVLine(Coord2Array<REAL> P, int n, REAL u, REAL v, REAL du, REAL dv)
{
...
//
inline int appendTri(FacetArray facets, int t0, int t1, int t2)
{
    facets[0].Set(t0, t1, t2);
    return 1;
}

inline int appendQuad(FacetArray facets, int q0, int q1, int q2, int q3, int triangulationSign)
{
...
inline int appendTriFan(FacetArray facets, int size, int startIndex)
{
    for (int i = 1; i <= size; ++i)
    {
        facets[i - 1].Set(startIndex + (i - 1), startIndex + ((i < size) ? i : 0), startIndex + size);
    }
    return size;
}
...
int FacetStrip::connectUniformQuads(FacetArray facets) const
{
    assert(quadTopology);
    ...
    {
        nFacets += appendQuad(facets + nFacets, out0, out0 + 1, in0, outerPrev, quadTriangulate);
    }
    ...
    if (innerEdges)
    {
        ...
        {
            if (i > (innerEdges / 2))
                triSign = -(int)quadTriangulate;

            int outJ = outI + 1;
            int inJ  = (i < innerEdges) ? (inI + inDelta) : innerLast;

            nFacets += appendQuad(facets + nFacets, outI, outJ, inJ, inI, triSign);
        }
    }
    ...
    {
        nFacets += appendQuad(facets + nFacets, outI, outN, outN + 1, inN, -(int)quadTriangulate);
    }
    return nFacets;
}

int FacetStrip::connectUniformTris(FacetArray facets) const
{
    assert(!quadTopology);
...
int FacetStrip::connectNonUniformFacets(FacetArray facets) const
{
    //
    //  General case:
    //
    ...
    for (int facetIndex = 0; facetIndex < nFacetsExpected; ++facetIndex)
    {
        ...
        else
        {
            ...
            if (keepQuads)
            {
                ...
                {
                    int mirroredFacetIndex = nFacetsLeading - 1 - (facetIndex - nFacetsLeading - nFacetsMiddle);

                    generateQuad = (facets[mirroredFacetIndex][3] >= 0);
                }
                else if ((tInner1 > tOuterMiddle) || (tOuter1 > tOuterMiddle))
                {
                    generateQuad = false;
                }
                else
            ...
            if (!generateQuad)
            {
                ...
                bool useOuterEdge = (dtDiagToOuter1 == dtDiagToInner1) ? (tOuter1 > tOuterMiddle) : (dtDiagToOuter1 < dtDiagToInner1);
                if (useOuterEdge)
                {
                    generateTriOuter = true;
                }
...
class quad
{
  public:
    ...
    static int CountNonUniformFacets(int const outerRes[], int const uvRes[], bool triangulate);
    ...
    template <typename REAL> static int GetEdgeCoords(int edge, int edgeRes, Coord2Array<REAL> coords);
    template <typename REAL> static int GetBoundaryCoords(int const edgeRates[], Coord2Array<REAL> coords);
    template <typename REAL> static int GetInteriorCoords(int const uvRes[2], Coord2Array<REAL> coords);

    //  Public methods for identifying and assigning facets:
    static int GetUniformFacets(int edgeRes, bool triangulate, FacetArray facets);
    static int GetSegmentedFacets(int const uvRes[], bool triangulate, FacetArray facets);
    static int GetNonUniformFacets(int const outerRes[], int const innerRes[], int nBoundaryEdges, bool triangulate, FacetArray facets);
  ...
  private:
    ...
    template <typename REAL> static int getCenterCoord(Coord2Array<REAL> coords);
    template <typename REAL> static int getInteriorRingCoords(int uRes, int vRes, REAL uStart, REAL vStart, REAL uDelta, REAL vDelta, Coord2Array<REAL> coords);

    static int getInteriorRingFacets(int uRes, int vRes, int indexOfFirstCoord, bool triangulate, FacetArray facets);
    static int getBoundaryRingFacets(int const outerRes[], int uRes, int vRes, int nBoundaryEdges, bool triangulate, FacetArray facets);
    static int getSingleStripFacets(int uRes, int vRes, int indexOfFirstCoord, bool triangulate, FacetArray facets);
};
...
class tri
{
  public:
    ...
    template <typename REAL> static int GetEdgeCoords(int edge, int edgeRes, Coord2Array<REAL> coords);
    template <typename REAL> static int GetBoundaryCoords(int const edgeRates[], Coord2Array<REAL> coords);
    template <typename REAL> static int GetInteriorCoords(int edgeRes, Coord2Array<REAL> coords);

    //  Public methods for identifying and assigning facets:
    static int GetUniformFacets(int edgeRes, FacetArray facets);
    static int GetNonUniformFacets(int const outerRes[], int innerRes, int nBoundaryEdges, FacetArray facets);
  ...
  private:
    ...
    template <typename REAL> static int getCenterCoord(Coord2Array<REAL> coords);
    template <typename REAL> static int getInteriorRingCoords(int edgeRes, REAL uStart, REAL vStart, REAL tDelta, Coord2Array<REAL> coords);

    static int getInteriorRingFacets(int edgeRes, int indexOfFirstCoord, FacetArray facets);
    static int getBoundaryRingFacets(int const outerRes[], int innerRes, int nBoundaryEdges, FacetArray facets);
};
...
class qsub
{
  public:
    ...
    static int CountNonUniformFacets(int N, int const outerRes[], int innerRes, bool triangulate);
    ...
    template <typename REAL> static int GetEdgeCoords(Parameterization P, int edge, int edgeRes, Coord2Array<REAL> coords);
    template <typename REAL> static int GetBoundaryCoords(Parameterization P, int const edgeRates[], Coord2Array<REAL> coords);
    template <typename REAL> static int GetInteriorCoords(Parameterization P, int edgeRes, Coord2Array<REAL> coords);

    //  Public methods for identifying and assigning facets:
    static int GetUniformFacets(int N, int edgeRes, bool triangulate, FacetArray facets);
    static int GetNonUniformFacets(int N, int const outerRes[], int innerRes, int nBoundaryEdges, bool triangulate, FacetArray facets);
  ...
  private:
    ...
    template <typename REAL> static int getCenterCoord(Coord2Array<REAL> coords);
    template <typename REAL> static int getCenterRingCoords(Parameterization P, REAL tStart, Coord2Array<REAL> coords);
    template <typename REAL> static int getRingEdgeCoords(Parameterization P, int edge, int edgeRes, bool incFirst, bool incLast, REAL tStart, REAL tDelta, Coord2Array<REAL> coords);
    template <typename REAL> static int getInteriorRingCoords(Parameterization P, int edgeRes, REAL tStart, REAL tDelta, Coord2Array<REAL> coords);

    static int getCenterFacets(int N, int indexOfFirstCoord, FacetArray facets);
    static int getInteriorRingFacets(int N, int edgeRes, int indexOfFirstCoord, bool triangulate, FacetArray facets);
    static int getBoundaryRingFacets(int N, int const outerRes[], int innerRes, int nBoundaryEdges, bool triangulate, FacetArray facets);
};

//
//  Implementations for quad functions:
//
inline int quad::CountUniformFacets(int edgeRes, bool triangulate) { return (edgeRes * edgeRes) << (int)triangulate; }

inline int quad::CountSegmentedFacets(int const uvRes[], bool triangulate)
{
...
int quad::countNonUniformEdgeFacets(int outerRes, int innerRes)
{
...
int quad::CountNonUniformFacets(int const outerRes[], int const innerRes[], bool triangulate)
{
    int uRes = innerRes[0];
    int vRes = innerRes[1];
    ...
    int nBoundary = 0;
    nBoundary += uniformEdges[0] ? (innerUEdges + 1 + !uniformCorners[1]) : countNonUniformEdgeFacets(outerRes[0], uRes);
    nBoundary += uniformEdges[1] ? (innerVEdges + 1 + !uniformCorners[2]) : countNonUniformEdgeFacets(outerRes[1], vRes);
    nBoundary += uniformEdges[2] ? (innerUEdges + 1 + !uniformCorners[3]) : countNonUniformEdgeFacets(outerRes[2], uRes);
    nBoundary += uniformEdges[3] ? (innerVEdges + 1 + !uniformCorners[0]) : countNonUniformEdgeFacets(outerRes[3], vRes);
    return nInterior + nBoundary;
}

inline int quad::countUniformCoords(int edgeRes) { return (edgeRes + 1) * (edgeRes + 1); }

inline int quad::CountInteriorCoords(int edgeRes) { return (edgeRes - 1) * (edgeRes - 1); }

inline int quad::CountInteriorCoords(int const uvRes[]) { return (uvRes[0] - 1) * (uvRes[1] - 1); }

template <typename REAL> inline int quad::getCenterCoord(Coord2Array<REAL> coords)
{
    coords[0].Set(0.5f, 0.5f);
    return 1;
}

template <typename REAL> int quad::GetEdgeCoords(int edge, int edgeRes, Coord2Array<REAL> coords)
{
    ...
    {
    case 0:
        return appendVIsoLine<REAL>(coords, n, t0, 0.0f, dt);
    case 1:
        return appendUIsoLine<REAL>(coords, n, 1.0f, t0, dt);
    case 2:
        return appendVIsoLine<REAL>(coords, n, t1, 1.0f, -dt);
    case 3:
        return appendUIsoLine<REAL>(coords, n, 0.0f, t1, -dt);
    }
    return 0;
}

template <typename REAL> int quad::GetBoundaryCoords(int const edgeRates[], Coord2Array<REAL> coords)
{
    int nCoords = 0;
    nCoords += appendVIsoLine<REAL>(coords + nCoords, edgeRates[0], 0.0f, 0.0f, 1.0f / (REAL)edgeRates[0]);
    nCoords += appendUIsoLine<REAL>(coords + nCoords, edgeRates[1], 1.0f, 0.0f, 1.0f / (REAL)edgeRates[1]);
    nCoords += appendVIsoLine<REAL>(coords + nCoords, edgeRates[2], 1.0f, 1.0f, -1.0f / (REAL)edgeRates[2]);
    nCoords += appendUIsoLine<REAL>(coords + nCoords, edgeRates[3], 0.0f, 1.0f, -1.0f / (REAL)edgeRates[3]);
    return nCoords;
}

template <typename REAL> int quad::getInteriorRingCoords(int uRes, int vRes, REAL u0, REAL v0, REAL du, REAL dv, Coord2Array<REAL> coords)
{
    int nCoords = 0;
    if ((uRes > 0) && (vRes > 0))
    {
...
template <typename REAL> int quad::GetInteriorCoords(int const uvRes[2], Coord2Array<REAL> coords)
{
    ...
    {
        nCoords += getInteriorRingCoords(uRes, vRes, u, v, du, dv, coords + nCoords);
    }
    return nCoords;
}

int quad::getSingleStripFacets(int uRes, int vRes, int coord0, bool triangulate, FacetArray facets)
{
    assert((uRes == 1) || (vRes == 1));
...
int quad::getInteriorRingFacets(int uRes, int vRes, int coord0, bool triangulate, FacetArray facets)
{
    ...
    if (totalInnerFacets == 0)
        return 0;

    if (totalInnerFacets == 1)
    {
        return appendQuad(facets, coord0, coord0 + 1, coord0 + 2, coord0 + 3, triangulate);
    }
...
int quad::getBoundaryRingFacets(int const outerRes[], int uRes, int vRes, int nBoundaryEdges, bool triangulate, FacetArray facets)
{
...
int quad::GetSegmentedFacets(int const innerRes[], bool triangulate, FacetArray facets)
{
    //  WIP - may extend later to handle different opposing outer rates
...
int quad::GetNonUniformFacets(int const outerRes[], int const innerRes[], int nBoundaryEdges, bool triangulate, FacetArray facets)
{
    int uRes = innerRes[0];
    int vRes = innerRes[1];
    assert((uRes > 1) && (vRes > 1));
    ...
    int nFacets = getBoundaryRingFacets(outerRes, uRes, vRes, nBoundaryEdges, triangulate, facets);
    ...
    for (int ring = 1; ring < nRings; ++ring)
    {
        ...
        nFacets += getInteriorRingFacets(uRes, vRes, coord0, triangulate, facets + nFacets);
        coord0 += 2 * (uRes + vRes);
    }
    return nFacets;
}

int quad::GetUniformFacets(int res, bool triangulate, FacetArray facets)
{
    ...
    {
        nFacets += getInteriorRingFacets(res, res, coord0, triangulate, facets + nFacets);
        coord0 += 4 * res;
    }
    return nFacets;
}
...
//
inline int tri::CountUniformFacets(int edgeRes) { return edgeRes * edgeRes; }

int tri::CountNonUniformFacets(int const outerRes[], int innerRes)
{
    assert(innerRes > 2);
    ...
    //
    int nBoundary = (nInnerEdges + outerRes[0]) + (nInnerEdges + outerRes[1]) + (nInnerEdges + outerRes[2]);

    return nInterior + nBoundary;
}

inline int tri::countUniformCoords(int edgeRes) { return edgeRes * (edgeRes + 1) / 2; }

inline int tri::CountInteriorCoords(int edgeRes) { return countUniformCoords(edgeRes - 2); }

template <typename REAL> inline int tri::getCenterCoord(Coord2Array<REAL> coords)
{
    coords[0].Set(1.0f / 3.0f, 1.0f / 3.0f);
    return 1;
}

template <typename REAL> int tri::GetEdgeCoords(int edge, int edgeRes, Coord2Array<REAL> coords)
{
    REAL dt = 1.0f / (REAL)edgeRes;
    ...
    case 0:
        return appendVIsoLine<REAL>(coords, n, t0, 0.0f, dt);
    case 1:
        return appendUVLine<REAL>(coords, n, t1, t0, -dt, dt);
    case 2:
        return appendUIsoLine<REAL>(coords, n, 0.0f, t1, -dt);
    }
    return 0;
...
template <typename REAL> int tri::GetBoundaryCoords(int const edgeRates[], Coord2Array<REAL> coords)
{
    int nCoords = 0;
    nCoords += appendVIsoLine<REAL>(coords + nCoords, edgeRates[0], 0.0f, 0.0f, 1.0f / (REAL)edgeRates[0]);
    nCoords += appendUVLine<REAL>(coords + nCoords, edgeRates[1], 1.0f, 0.0f, -1.0f / (REAL)edgeRates[1], 1.0f / (REAL)edgeRates[1]);
    nCoords += appendUIsoLine<REAL>(coords + nCoords, edgeRates[2], 0.0f, 1.0f, -1.0f / (REAL)edgeRates[2]);
    return nCoords;
}

template <typename REAL> int tri::getInteriorRingCoords(int edgeRes, REAL u0, REAL v0, REAL dt, Coord2Array<REAL> coords)
{
    assert(edgeRes);
...
template <typename REAL> int tri::GetInteriorCoords(int edgeRes, Coord2Array<REAL> coords)
{
    ...
    for (int i = 0; i < nIntRings; ++i, ringRes -= 3, u += dt, v += dt)
    {
        ...
        {
            nCoords += getInteriorRingCoords(ringRes, u, v, dt, coords + nCoords);
        }
    }
    return nCoords;
}

int tri::getInteriorRingFacets(int edgeRes, int coord0, FacetArray facets)
{
    //
...
int tri::getBoundaryRingFacets(int const outerRes[], int innerRes, int nBoundaryEdges, FacetArray facets)
{
    //  Identify edges and corners that should preserve uniform behavior:
    bool uniformEdges[3];
...
int tri::GetUniformFacets(int edgeRes, FacetArray facets)
{
    //  The trivial case should have been handled by the caller:
    assert(edgeRes > 1);
...
int tri::GetNonUniformFacets(int const outerRes[], int innerRes, int nBoundaryEdges, FacetArray facets)
{
    assert(innerRes > 2);

    //  First, generate the ring of boundary facets separately:
    int nFacets = getBoundaryRingFacets(outerRes, innerRes, nBoundaryEdges, facets);
...
//
inline int qsub::CountUniformFacets(int N, int edgeRes, bool triangulate)
{
...
int qsub::CountNonUniformFacets(int N, int const outerRes[], int innerRes, bool triangulate)
{
    assert(innerRes > 1);
...
inline int qsub::countUniformCoords(int N, int edgeRes)
{
    int H = edgeRes / 2;
    return (edgeRes & 1) ? (H + 1) * (H + 1) * N + ((N == 3) ? 0 : 1) : H * (H + 1) * N + 1;
}

inline int qsub::CountInteriorCoords(int N, int edgeRes)
{
    assert(edgeRes > 1);
    return countUniformCoords(N, edgeRes - 2);
}

template <typename REAL> inline int qsub::getCenterCoord(Coord2Array<REAL> coords)
{
    coords[0].Set(0.5f, 0.5f);
    return 1;
}

template <typename REAL> int qsub::getRingEdgeCoords(Parameterization P, int edge, int edgeRes, bool incFirst, bool incLast, REAL tOrigin, REAL dt, Coord2Array<REAL> coords)
{
    //
...
template <typename REAL> int qsub::GetEdgeCoords(Parameterization P, int edge, int edgeRes, Coord2Array<REAL> coords) { return getRingEdgeCoords<REAL>(P, edge, edgeRes, false, false, 0.0f, 1.0f / (REAL)edgeRes, coords); }

template <typename REAL> int qsub::GetBoundaryCoords(Parameterization P, int const edgeRates[], Coord2Array<REAL> coords)
{
    int N = P.GetFaceSize();
    ...
    for (int i = 0; i < N; ++i)
    {
        nCoords += getRingEdgeCoords<REAL>(P, i, edgeRates[i], true, false, 0.0f, 1.0f / (REAL)edgeRates[i], coords + nCoords);
    }
    return nCoords;
}

template <typename REAL> int qsub::getInteriorRingCoords(Parameterization P, int edgeRes, REAL tOrigin, REAL dt, Coord2Array<REAL> coords)
{
    assert(edgeRes > 1);
    ...
    for (int i = 0; i < N; ++i)
    {
        nCoords += getRingEdgeCoords(P, i, edgeRes, true, false, tOrigin, dt, coords + nCoords);
    }
    return nCoords;
}

template <typename REAL> int qsub::getCenterRingCoords(Parameterization P, REAL tOrigin, Coord2Array<REAL> coords)
{
    int N = P.GetFaceSize();
...
template <typename REAL> int qsub::GetInteriorCoords(Parameterization P, int edgeRes, Coord2Array<REAL> coords)
{
    ...
    for (int i = 0; i < nIntRings; ++i, ringRes -= 2, t += dt)
    {
        ...
        {
            nCoords += getInteriorRingCoords(P, ringRes, t, dt, coords + nCoords);
        }
    }
    return nCoords;
}

int qsub::getCenterFacets(int N, int coord0, FacetArray facets) { return (N == 3) ? appendTri(facets, coord0, coord0 + 1, coord0 + 2) : appendTriFan(facets, N, coord0); }

int qsub::getInteriorRingFacets(int N, int edgeRes, int coord0, bool triangulate, FacetArray facets)
{
    //
    //  Deal with trivial cases with no inner vertices:
    //
...
int qsub::getBoundaryRingFacets(int N, int const outerRes[], int innerRes, int nBoundaryEdges, bool triangulate, FacetArray facets)
{
...
int qsub::GetUniformFacets(int N, int edgeRes, bool triangulate, FacetArray facets)
{
    ...
    {
        nFacets += getInteriorRingFacets(N, edgeRes, coord0, triangulate, facets + nFacets);
        coord0 += N * edgeRes;
    }
    return nFacets;
}

int qsub::GetNonUniformFacets(int N, int const outerRes[], int innerRes, int nBoundaryEdges, bool triangulate, FacetArray facets)
{
    //  First, generate the ring of boundary facets separately:
    int nFacets = getBoundaryRingFacets(N, outerRes, innerRes, nBoundaryEdges, triangulate, facets);
    ...
    for (int ring = 1; ring < nRings; ++ring)
    {
        ...
        nFacets += getInteriorRingFacets(N, innerRes, coord0, triangulate, facets + nFacets);
        coord0 += N * innerRes;
    }
    return nFacets;
}
...
//
void Tessellation::initializeDefaults()
{
...
bool Tessellation::validateArguments(Parameterization const &p, int numRates, int const rates[], Options const &options)
{
    ...
    if (facetStride && (facetStride < options.GetFacetSize()))
        return false;

    return true;
}

void Tessellation::initialize(Parameterization const &p, int numRates, int const rates[], Options const &options)
{
    //  Validate arguments and initialize simple members:
    initializeDefaults();

    if (!validateArguments(p, numRates, rates, options))
        return;

    _param = p;

    _facetSize   = (short)options.GetFacetSize();
    _facetStride = options.GetFacetStride() ? options.GetFacetStride() : options.GetFacetSize();
...
int Tessellation::initializeRates(int numGivenRates, int const givenRates[])
{
    ...
    else
    {
        ...
        if (numGivenRates > N)
        {
            ...
            _innerRates[1] = ((numGivenRates == 6) && isQuad) ? std::min(givenRates[5], MaxRate) : _innerRates[0];
...
int Tessellation::GetRates(int rates[]) const
{
    int N = _param.GetFaceSize();
...
void Tessellation::initializeInventoryForParamQuad(int sumOfEdgeRates)
{
    ...
    else
    {
        ...
        if ((inner[0] > 1) && (inner[1] > 1))
        {
            ...
            _numFacets         = quad::CountNonUniformFacets(_outerRates, _innerRates, _triangulate);
        }
        else if ((outer[0] == inner[0]) && (inner[0] == outer[2]) && (outer[1] == inner[1]) && (inner[1] == outer[3]))
        {
            _numInteriorPoints = 0;
...
void Tessellation::initializeInventoryForParamTri(int sumOfEdgeRates)
{
...
void Tessellation::initializeInventoryForParamQPoly(int sumOfEdgeRates)
{
    int N   = _param.GetFaceSize();
    int res = _innerRates[0];
    ...
    else
    {
        ...
        {
            _numInteriorPoints = qsub::CountInteriorCoords(N, res);
            _numFacets         = qsub::CountNonUniformFacets(N, _outerRates, res, _triangulate);
        }
        else
        {
            _numInteriorPoints = 1;
...
//
Tessellation::Tessellation(Parameterization const &p, int uniformRate, Options const &options) { initialize(p, 1, &uniformRate, options); }

Tessellation::Tessellation(Parameterization const &p, int numRates, int const rates[], Options const &options) { initialize(p, numRates, rates, options); }
...
//
template <typename REAL> int Tessellation::GetEdgeCoords(int edge, REAL coordBuffer[]) const
{
...
template <typename REAL> int Tessellation::GetBoundaryCoords(REAL coordBuffer[]) const
{
    Coord2Array<REAL> coords(coordBuffer, _coordStride);
...
template <typename REAL> int Tessellation::GetInteriorCoords(REAL coordBuffer[]) const
{
...
int Tessellation::GetFacets(int facetIndices[]) const
{
    ...
    case Parameterization::QUAD:
        if (_isUniform)
        {
            nFacets = quad::GetUniformFacets(_innerRates[0], _triangulate, facets);
        }
        else if (_segmentedFace)
        {
            nFacets = quad::GetSegmentedFacets(_innerRates, _triangulate, facets);
        }
        else
        {
            nFacets = quad::GetNonUniformFacets(_outerRates, _innerRates, _numBoundaryPoints, _triangulate, facets);
        }
        break;
    case Parameterization::TRI:
        if (_isUniform)
        ...
        {
            nFacets = tri::GetNonUniformFacets(_outerRates, _innerRates[0], _numBoundaryPoints, facets);
        }
        break;
    case Parameterization::QUAD_SUBFACES:
        if (_isUniform)
        {
            nFacets = qsub::GetUniformFacets(N, _innerRates[0], _triangulate, facets);
        }
        else
        {
            nFacets = qsub::GetNonUniformFacets(N, _outerRates, _innerRates[0], _numBoundaryPoints, _triangulate, facets);
        }
        break;
    default:
        assert(0);
...
void Tessellation::TransformFacetCoordIndices(int facetIndices[], int commonOffset)
{
    if (_facetSize == 4)
    {
...
void Tessellation::TransformFacetCoordIndices(int facetIndices[], int const boundaryIndices[], int interiorOffset)
{
    for (int i = 0; i < _numFacets; ++i, facetIndices += _facetStride)
    {
        for (int j = 0; j < (int)_facetSize; ++j)
        {
            ...
            if (index >= 0)
            {
                index = (index < _numBoundaryPoints) ? boundaryIndices[index] : (index + interiorOffset);
            }
        }
    }
}

void Tessellation::TransformFacetCoordIndices(int facetIndices[], int const boundaryIndices[], int const interiorIndices[])
{
    for (int i = 0; i < _numFacets; ++i, facetIndices += _facetStride)
    {
        for (int j = 0; j < (int)_facetSize; ++j)
        {
            ...
            if (index >= 0)
            {
                index = (index < _numBoundaryPoints) ? boundaryIndices[index] : interiorIndices[index - _numBoundaryPoints];
            }
        }
    }
}
...
//
template int Tessellation::GetBoundaryCoords<float>(float coordBuffer[]) const;
template int Tessellation::GetBoundaryCoords<double>(double coordBuffer[]) const;

template int Tessellation::GetInteriorCoords<float>(float coordBuffer[]) const;
