...
//
PatchTreeBuilder::PatchTreeBuilder(TopologyRefiner &faceRefiner, Options const &options) : _patchTree(new PatchTree), _faceRefiner(faceRefiner), _faceAtRoot(0), _patchBuilder(0)
{
    //
    ...
    _patchTree->_regPatchSize     = PatchDescriptor(_patchTree->_regPatchType).GetNumControlVertices();
    _patchTree->_irregPatchSize   = PatchDescriptor(_patchTree->_irregPatchType).GetNumControlVertices();
    _patchTree->_patchPointStride = std::max(_patchTree->_regPatchSize, _patchTree->_irregPatchSize);
    ...
    _patchTree->_numRefinedPoints  = _faceRefiner.GetNumVerticesTotal() - _patchTree->_numControlPoints;
    _patchTree->_numSubPatchPoints = _patchTree->_numRefinedPoints;
}

PatchTreeBuilder::~PatchTreeBuilder() { delete _patchBuilder; }

const PatchTree *PatchTreeBuilder::Build()
{
    identifyPatches();
    initializePatches();
...
bool PatchTreeBuilder::rootFaceNeedsRefinement() const
{
    //
...
bool PatchTreeBuilder::testFaceAncestors() const
{
    //  Conditions of overlapping faces that require testing base face:
    return (_patchBuilder->GetRegularFaceSize() == 3) && (_faceRefiner.getLevel(0).getNumEdges() == 3) && (_faceRefiner.getLevel(0).getNumFaces() > 1);
}

bool PatchTreeBuilder::faceAncestorIsRoot(int level, int face) const
{
...
void PatchTreeBuilder::identifyPatches()
{
    //
    ...
    for (int level = 1; level < numLevels; ++level)
    {
        ...
        {
            if (testBaseFace && !faceAncestorIsRoot(level, face))
                continue;
    ...
    _patchTree->_numSubPatchPoints += numIrregPatches * _patchTree->_irregPatchSize;
}

void PatchTreeBuilder::initializePatches()
{
    //  Keep track of the growing index of local points in irregular patches:
    int irregPointIndexBase = _patchTree->_numControlPoints + _patchTree->_numRefinedPoints;
    ...
    for (size_t i = 0; i < _patchFaces.size(); ++i)
    {
        ...
        Index *patchPoints = &_patchTree->_patchPoints[i * _patchTree->_patchPointStride];
        ...
        if (pf.isRegular)
        {
            ...
            patchParam = _patchBuilder->ComputePatchParam(pf.level, pf.face, ptexIndices, true, boundaryMask, true);
            ...
            _patchBuilder->GetRegularPatchPoints(pf.level, pf.face, boundaryMask, patchPoints);
        ...
        else
        {
            ...
            patchParam = _patchBuilder->ComputePatchParam(pf.level, pf.face, ptexIndices, false /*irreg*/, 0 /*mask*/, false);
...
struct ControlRow
{
    ...
    ControlRow operator[](int index) const { return ControlRow(index); }
...
//
template <typename REAL> struct StencilRow
{
    StencilRow() : _data(0), _size(0) {}
    StencilRow(REAL *data, int size) : _data(data), _size(size) {}
    StencilRow(REAL const *data, int size) : _data(const_cast<REAL *>(data)), _size(size) {}
    ...
    StencilRow operator[](int index) const { return StencilRow(_data + index * _size, _size); }
...
template <typename REAL> void PatchTreeBuilder::initializeStencilMatrix()
{
    if (_patchTree->_numSubPatchPoints == 0)
        return;
    ...
    //
    int numPointStencils = _patchTree->_numRefinedPoints + (_patchTree->_numIrregPatches * _patchTree->_irregPatchSize);
    ...
    if (_patchTree->_numIrregPatches)
    {
        ...
        for (size_t i = 0; i < _patchFaces.size(); ++i)
        {
            ...
            {
                getIrregularPatchConversion(_patchFaces[i], irregConvMatrix, irregSourcePoints);

                appendConversionStencilsToMatrix(stencilIndexBase, irregConvMatrix, irregSourcePoints);
...
template <typename REAL> void PatchTreeBuilder::appendConversionStencilsToMatrix(int stencilBaseIndex, SparseMatrix<REAL> const &conversionMatrix, std::vector<Index> const &sourcePoints)
{
    //
...
void PatchTreeBuilder::initializeQuadTree() { _patchTree->buildQuadtree(); }

template <typename REAL> void PatchTreeBuilder::getIrregularPatchConversion(PatchFace const &pf, SparseMatrix<REAL> &conversionMatrix, std::vector<Index> &sourcePoints)
{
    //
    ...
    //
    _patchBuilder->GetIrregularPatchConversionMatrix(pf.level, pf.face, cornerSpans, conversionMatrix);
    ...
    _patchBuilder->GetIrregularPatchSourcePoints(pf.level, pf.face, cornerSpans, &sourcePoints[0]);
