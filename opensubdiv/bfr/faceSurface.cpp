...
//
void FaceSurface::preInitialize(FaceTopology const &faceTopology, Index const faceIndices[])
{
    //
...
void FaceSurface::postInitialize()
{
    //
...
//
void FaceSurface::Initialize(FaceTopology const &vtxTopology, Index const vtxIndices[])
{
    ...
    bool useInfSharpSubsets = _topology->GetTag().HasInfSharpEdges() && !_topology->GetTag().HasInfSharpDarts();
...
void FaceSurface::Initialize(FaceSurface const &vtxSurface, Index const fvarIndices[])
{
    ...
    for (int corner = 0; corner < GetFaceSize(); ++corner)
    {
        ...
        {
            sharpenBySdcFVarLinearInterpolation(&fvarSub, fvarIndices, vtxSub, vtxTop);
        }
...
//
bool FaceSurface::isRegular() const
{
    //
    ...
    //
    if (_combinedTag.HasSharpEdges() || _combinedTag.HasSemiSharpVertices() || _combinedTag.HasIrregularFaceSizes())
    {
        return false;
    }
    ...
    {
        if (_combinedTag.HasInfSharpVertices())
            return false;

        if (GetRegFaceSize() == 4)
        {
            //  Can use bitwise-OR here for reg valence of 4:
            return (_corners[0].GetNumFaces() | _corners[1].GetNumFaces() | _corners[2].GetNumFaces() | _corners[3].GetNumFaces()) == 4;
        }
        else
        {
            return (_corners[0].GetNumFaces() == 6) && (_corners[1].GetNumFaces() == 6) && (_corners[2].GetNumFaces() == 6);
        }
    }
    ...
    for (int i = 0; i < GetFaceSize(); ++i)
    {
        ...
        if (corner.IsSharp())
        {
            if (corner.GetNumFaces() != 1)
                return false;
        }
        else if (corner.IsBoundary())
        {
            if (corner.GetNumFaces() != regBoundaryValence)
                return false;
        }
        else
        {
            if (corner.GetNumFaces() != regInteriorValence)
                return false;
        }
    }
    return true;
...
void FaceSurface::reviseSdcOptionsInEffect()
{
    //
    ...
    if (options.GetTriangleSubdivision() != Sdc::Options::TRI_SUB_CATMARK)
    {
        ...
        if ((GetSdcScheme() != Sdc::SCHEME_CATMARK) || !tags.HasIrregularFaceSizes())
        {
...
//
void FaceSurface::sharpenBySdcVtxBoundaryInterpolation(FaceVertexSubset *vtxSub, FaceVertex const &vtxTop) const
{
...
//
bool hasMoreThanTwoFVarSubsets(FaceVertex const &top, Index const fvarIndices[])
{
    ...
    for (int face = 0; face < numFaces; ++face)
    {
        ...
        numOtherEdgesDiscts += (faceNext < 0) || !top.FaceIndicesMatchAcrossEdge(face, faceNext, fvarIndices);

        if (numOtherEdgesDiscts > 2)
            return true;
    }
    return false;
}
...
//
bool hasDependentSharpness(FaceVertex const &topology, FaceVertexSubset const &subset) { return ((topology.GetNumFaces() - subset.GetNumFaces()) > 1) && topology.GetTag().HasSharpEdges() && !subset.GetTag().HasSharpEdges(); }
...
//
float getDependentSharpness(FaceVertex const &top, FaceVertexSubset const &subset)
{
...
//
void FaceSurface::sharpenBySdcFVarLinearInterpolation(FaceVertexSubset *fvarSub, Index const fvarIndices[], FaceVertexSubset const &vtxSub, FaceVertex const &vtxTop) const
{
    ...
    case Sdc::Options::FVAR_LINEAR_CORNERS_PLUS1:
        ...
        //
        isSharp = (fvarSub->GetNumFaces() == 1) || fvar_plus::hasMoreThanTwoFVarSubsets(vtxTop, fvarIndices);
        if (!isSharp && fvar_plus::hasDependentSharpness(vtxTop, *fvarSub))
        {
            //  Sharpen if sharp edges of other subset affects this one
            vtxTop.SharpenSubset(fvarSub, fvar_plus::getDependentSharpness(vtxTop, *fvarSub));
        }
        break;
...
//
void FaceSurface::print(bool printVerts) const
{
