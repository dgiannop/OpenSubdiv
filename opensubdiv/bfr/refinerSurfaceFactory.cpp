...
//
RefinerSurfaceFactoryBase::RefinerSurfaceFactoryBase(Far::TopologyRefiner const &mesh, Options const &factoryOptions)
    : SurfaceFactory(mesh.GetSchemeType(), mesh.GetSchemeOptions(), factoryOptions), _mesh(mesh), _numFaces(mesh.GetLevel(0).GetNumFaces()), _numFVarChannels(mesh.GetNumFVarChannels())
{
...
//
inline int RefinerSurfaceFactoryBase::getFaceVaryingChannel(FVarID fvarID) const { return ((0 <= fvarID) && (fvarID < _numFVarChannels)) ? (int)fvarID : -1; }
...
//
bool RefinerSurfaceFactoryBase::isFaceHole(Index face) const { return _mesh.HasHoles() && _mesh.getLevel(0).isFaceHole(face); }

int RefinerSurfaceFactoryBase::getFaceSize(Index baseFace) const { return _mesh.GetLevel(0).GetFaceVertices(baseFace).size(); }

//
//  Specifying vertex or face-varying indices for a face:
//
int RefinerSurfaceFactoryBase::getFaceVertexIndices(Index baseFace, Index indices[]) const
{
...
int RefinerSurfaceFactoryBase::getFaceFVarValueIndices(Index baseFace, FVarID fvarID, Index indices[]) const
{
...
//
int RefinerSurfaceFactoryBase::populateFaceVertexDescriptor(Index baseFace, int cornerVertex, VertexDescriptor *vertexDescriptor) const
{
    ...
    vd.Initialize(nFaces);
    {
        ...
        if (vTag._incidIrregFace)
        {
            ...
            {
                vd.SetIncidentFaceSize(i, baseLevel.getFaceVertices(vFaces[i]).size());
            }
        }
...
//
int RefinerSurfaceFactoryBase::getFaceVertexPointIndices(Index baseFace, int cornerVertex, Index indices[], int vtxOrFVarChannel) const
{
    Vtr::internal::Level const &baseLevel = _mesh.getLevel(0);
...
int RefinerSurfaceFactoryBase::getFaceVertexIncidentFaceVertexIndices(Index baseFace, int cornerVertex, Index indices[]) const { return getFaceVertexPointIndices(baseFace, cornerVertex, indices, -1); }

int RefinerSurfaceFactoryBase::getFaceVertexIncidentFaceFVarValueIndices(Index baseFace, int corner, FVarID fvarID, Index indices[]) const
{
    int fvarChannel = getFaceVaryingChannel(fvarID);
    if (fvarChannel < 0)
        return 0;
...
//
bool RefinerSurfaceFactoryBase::getFaceNeighborhoodVertexIndicesIfRegular(Index baseFace, Index vtxIndices[]) const
{
    //
    ...
    if (fTag._xordinary || fTag._nonManifold || fTag._incidIrregFace || fTag._semiSharp || fTag._semiSharpEdges || fTag._infIrregular)
    {
        return false;
    }
    ...
    else
    {
        ...
        for (int i = 0; i < fVerts.size(); ++i)
        {
            ...
            {
                if (vTag._rule != Sdc::Crease::RULE_CORNER)
                    return false;
            }
            else
            {
                if (vTag._rule != Sdc::Crease::RULE_CREASE)
                    return false;
            }
        }
    }
...
bool RefinerSurfaceFactoryBase::getFaceNeighborhoodFVarValueIndicesIfRegular(Index baseFace, FVarID fvarID, Index fvarIndices[]) const
{
    int fvarChannel = getFaceVaryingChannel(fvarID);
    if (fvarChannel < 0)
        return false;
...
//
template <typename POINT> int gatherPatchPoints4(Level const &level, Index face, IArray const &fVerts, POINT P[], int fvar)
{
    static int const pointsPerCorner[4][4] = {{5, 4, 0, 1}, {6, 2, 3, 7}, {10, 11, 15, 14}, {9, 13, 12, 8}};
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        if (vFaces.size() == 4)
        {
            ...
            IArray FV = (fvar < 0) ? level.getFaceVertices(fj) : level.getFaceFVarValues(fj, fvar);
        ...
        {
            Index FVcorner = (fvar < 0) ? vIndex : level.getFaceFVarValues(vFaces[0], fvar)[vInFace[0]];
        ...
        else if (vFaces[0] == face)
        {
            Index  f1 = vFaces[1];
            int    j1 = vInFace[1];
            IArray FV = (fvar < 0) ? level.getFaceVertices(f1) : level.getFaceFVarValues(f1, fvar);
        ...
        else
        {
            Index  f0 = vFaces[0];
            int    j0 = vInFace[0];
            IArray FV = (fvar < 0) ? level.getFaceVertices(f0) : level.getFaceFVarValues(f0, fvar);
...
//
template <typename POINT> int gatherPatchPoints3(Level const &level, Index face, IArray const &fVerts, POINT P[], int fvar)
{
    static int const pointsPerCorner[3][4] = {{4, 3, 0, 1}, {5, 2, 6, 9}, {8, 11, 10, 7}};
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        if (vFaces.size() == 6)
        {
            ...
            IArray FV0 = (fvar < 0) ? level.getFaceVertices(f0) : level.getFaceFVarValues(f0, fvar);
            Index  f1  = vFaces[_mod6(iOpposite + 1)];
            int    j1  = vInFace[_mod6(iOpposite + 1)];
            IArray FV1 = (fvar < 0) ? level.getFaceVertices(f1) : level.getFaceFVarValues(f1, fvar);
        ...
        else if (vFaces[0] == face)
        {
            Index  f2 = vFaces[2];
            int    j2 = vInFace[2];
            IArray FV = (fvar < 0) ? level.getFaceVertices(f2) : level.getFaceFVarValues(f2, fvar);
...
//
int RefinerSurfaceFactoryBase::getFacePatchPointIndices(Index baseFace, Index indices[], int vtxOrFVarChannel) const
{
    Vtr::internal::Level const &baseLevel = _mesh.getLevel(0);
    ...
    {
        return gatherPatchPoints4(baseLevel, baseFace, baseFaceVerts, indices, vtxOrFVarChannel);
    }
    else
    {
        return gatherPatchPoints3(baseLevel, baseFace, baseFaceVerts, indices, vtxOrFVarChannel);
    }
}
