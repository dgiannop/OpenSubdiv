...
class SurfaceFactory::SurfaceSet
{
  ...
  public:
    ...
    {
        if (vtxSurf)
            vtxSurf->reinitialize();
        if (varSurf)
            varSurf->reinitialize();
  ...
  public:
    ...
    FVarID GetFVarSurfaceID(int i) const { return fvarIDs ? fvarIDs[i] : FVarID(i); }
...
//
SurfaceFactory::SurfaceFactory(Sdc::SchemeType subdivScheme, Sdc::Options const &subdivOptions, Options const &factoryOptions) : _topologyCache(0)
{
...
void SurfaceFactory::setSubdivisionOptions(Sdc::SchemeType subdivScheme, Sdc::Options const &subdivOptions)
{
    ...
    _linearScheme = (Sdc::SchemeTypeTraits::GetLocalNeighborhoodSize(_subdivScheme) == 0);

    _linearFVarInterp = _linearScheme || (_subdivOptions.GetFVarLinearInterpolation() == Sdc::Options::FVAR_LINEAR_ALL);

    //  Initialize members related to the "face has limit" test:
    _rejectSmoothBoundariesForLimit = !_linearScheme && (_subdivOptions.GetVtxBoundaryInterpolation() == Sdc::Options::VTX_BOUNDARY_NONE);

    _rejectIrregularFacesForLimit = !_linearScheme && (_regFaceSize == 3);

    _testNeighborhoodForLimit = _rejectSmoothBoundariesForLimit || _rejectIrregularFacesForLimit;
}

void SurfaceFactory::setFactoryOptions(Options const &factoryOptions)
{
...
void SurfaceFactory::setInternalCache(SurfaceFactoryCache *cache)
{
...
//
inline bool SurfaceFactory::faceHasLimitSimple(Index faceIndex, int faceSize) const { return (faceSize >= 3) && (faceSize <= Limits::MaxFaceSize()) && !isFaceHole(faceIndex); }

bool SurfaceFactory::faceHasLimitNeighborhood(FaceTopology const &topology) const
{
    assert(_testNeighborhoodForLimit);

    MultiVertexTag tag = topology.GetTag();

    if ((_rejectSmoothBoundariesForLimit && tag.HasNonSharpBoundary()) || (_rejectIrregularFacesForLimit && tag.HasIrregularFaceSizes()))
    {
        return false;
    }
    return true;
...
bool SurfaceFactory::faceHasLimitNeighborhood(Index faceIndex) const
{
    assert(_testNeighborhoodForLimit);
    ...
    for (int i = 0; i < faceSize; ++i)
    {
        ...
        if (_rejectSmoothBoundariesForLimit)
        {
            if (faceVtxTag.IsUnOrdered())
            {
                ...
                if (getFaceVertexIncidentFaceVertexIndices(faceIndex, i, cFaceVertIndices) < 0)
                    return false;

                faceVtx.ConnectUnOrderedFaces(cFaceVertIndices);
            }
            if (faceVtxTag.HasNonSharpBoundary())
                return false;
        }
        if (_rejectIrregularFacesForLimit)
        {
            if (faceVtxTag.HasIrregularFaceSizes())
                return false;
        }
    }
    return true;
...
bool SurfaceFactory::FaceHasLimitSurface(Index faceIndex) const
{
...
Parameterization SurfaceFactory::GetFaceParameterization(Index faceIndex) const { return Parameterization(_subdivScheme, getFaceSize(faceIndex)); }
...
//  10% in extreme cases.
//
KeyIntType packTopologyKey(FaceSurface const &surface, IrregularPatchBuilder::Options options)
{
    //
    ...
    if (combinedTag.HasSharpEdges() || combinedTag.HasSemiSharpVertices() || combinedTag.HasIrregularFaceSizes())
    {
        return false;
    }
    ...
    for (int i = 0; i < surface.GetFaceSize(); ++i)
    {
        ...
        {
            if (!KeyBits::BoundaryValenceFits(valence))
                return false;
        }
        else
        {
            if (!KeyBits::InteriorValenceFits(valence))
                return false;
        }
    }
...
//
KeyIntType hashTopologyKey(FaceSurface const &surface, IrregularPatchBuilder::Options options)
{
    //
    ...
    for (int i = 0; i < faceSize; ++i)
    {
        ...
        hashBufferSize += cSub._tag.IsSemiSharp() ? sizeof(float) : 0;
        hashBufferSize += cSub._tag.HasUnCommonFaceSizes() ? (sizeof(short) * N) : 0;
        hashBufferSize += cSub._tag.HasSharpEdges() ? (sizeof(float) * (N - cSub.IsBoundary())) : 0;
    ...
    for (int corner = 0; corner < faceSize; ++corner)
    {
        ...
        {
            float sharpness = (cSub._localSharpness > 0.0f) ? cSub._localSharpness : cTop.GetVertexSharpness();
        ...
        if (cHeader.hasFaceSizes)
        {
            ...
            for (int i = 0, f = cTop.GetFaceFirst(cSub); i < n; f = cTop.GetFaceNext(f), ++i)
            {
...
//
void SurfaceFactory::assignLinearSurface(SurfaceType *surfacePtr, Index faceIndex, FVarID const *fvarPtrOrVtx) const
{
...
void SurfaceFactory::assignRegularSurface(SurfaceType *surfacePtr, Index const patchPoints[]) const
{
    SurfaceType &surface = *surfacePtr;
    ...
    surface.setRegPatchMask(RegularPatchBuilder::GetBoundaryMask(_regFaceSize, patchPoints));
...
void SurfaceFactory::assignRegularSurface(SurfaceType *surfacePtr, FaceSurface const &descriptor) const
{
    SurfaceType &surface = *surfacePtr;
    ...
    //
    builder.GatherControlVertexIndices(surface.resizeCVs(builder.GetNumControlVertices()));
...
void SurfaceFactory::assignIrregularSurface(SurfaceType *surfacePtr, FaceSurface const &descriptor) const
{
    //
    ...
    else
    {
        ...
        //
        SurfaceFactoryCache::KeyType key = hashTopologyKey(descriptor, buildOptions);
    ...
    //  Gather the patch control points from the given indices:
    builder.GatherControlVertexIndices(surface.resizeCVs(patch->GetNumControlPoints()));
...
void SurfaceFactory::copyNonLinearSurface(SurfaceType *surfaceDstPtr, SurfaceType const &surfaceSrc, FaceSurface const &descriptor) const
{
...
//
bool SurfaceFactory::initFaceNeighborhoodTopology(Index faceIndex, FaceTopology *faceTopologyPtr) const
{
...
bool SurfaceFactory::gatherFaceNeighborhoodTopology(Index faceIndex, FaceTopology *faceTopologyPtr) const
{
...
int SurfaceFactory::gatherFaceNeighborhoodIndices(Index faceIndex, FaceTopology const &faceTopology, FVarID const *fvarPtrOrVtx, Index controlIndices[]) const
{
    ...
    for (int i = 0; i < faceSize; ++i)
    {
        int numFaceVerts = (fvarPtrOrVtx == 0) ? getFaceVertexIncidentFaceVertexIndices(faceIndex, i, indices) : getFaceVertexIncidentFaceFVarValueIndices(faceIndex, i, *fvarPtrOrVtx, indices);
...
bool SurfaceFactory::isFaceNeighborhoodRegular(Index faceIndex, FVarID const *fvarPtrOrVtx, Index indices[]) const
{
    return (fvarPtrOrVtx == 0) ? getFaceNeighborhoodVertexIndicesIfRegular(faceIndex, indices) : getFaceNeighborhoodFVarValueIndicesIfRegular(faceIndex, *fvarPtrOrVtx, indices);
}

//
//  Main internal methods to populate set of limit Surfaces:
//
bool SurfaceFactory::populateAllSurfaces(Index faceIndex, SurfaceSet *surfaceSetPtr) const
{
    ...
    bool hasNonLinearSurfaces = (surfaces.HasVertexSurface() && !_linearScheme) || (numFVarSurfaces && !_linearFVarInterp);

    bool hasLinearSurfaces = surfaces.HasVaryingSurface() || (surfaces.HasVertexSurface() && _linearScheme) || (numFVarSurfaces && _linearFVarInterp);
...
bool SurfaceFactory::populateLinearSurfaces(Index faceIndex, SurfaceSet *surfaceSetPtr) const
{
    SurfaceSet &surfaces = *surfaceSetPtr;
...
bool SurfaceFactory::populateNonLinearSurfaces(Index faceIndex, SurfaceSet *surfaceSetPtr) const
{
    SurfaceSet &surfaces = *surfaceSetPtr;
    ...
    bool vtxIsExplicitlyRegular = isFaceNeighborhoodRegular(faceIndex, 0, vtxIndices);
    ...
    else
    {
        ...
        if (vtxIsNonLinear || faceTopology.HasUnOrderedCorners())
        {
            ...
            if (gatherFaceNeighborhoodIndices(faceIndex, faceTopology, 0, vtxIndices) < 0)
            {
                return false;
            }
    ...
    if (fvarIsNonLinear)
    {
        ...
        for (int i = 0; i < numFVarSurfaces; ++i)
        {
            ...
            bool fvarIsExplicitlyRegular = vtxIsExplicitlyRegular && isFaceNeighborhoodRegular(faceIndex, &fvarID, fvIndices);
            ...
            if (gatherFaceNeighborhoodIndices(faceIndex, faceTopology, &fvarID, fvIndices) < 0)
            {
                return false;
            }
            ...
            {
                copyNonLinearSurface(&fvarSurf, *surfaces.GetVertexSurface(), fvarSurfDesc);
            }
...
//
bool SurfaceFactory::initSurfaces(Index faceIndex, internal::SurfaceData *vtxSurface, internal::SurfaceData *varSurface, internal::SurfaceData *fvarSurfaces, int fvarCount, FVarID const fvarIDs[]) const
{
