...
inline void PatchTree::TreeNode::SetChildren(int index)
{
    for (int i = 0; i < 4; ++i)
    {
...
inline void PatchTree::TreeNode::SetChild(int quadrant, int index, bool isLeaf)
{
...
//
PatchTree::PatchTree()
    : _useDoublePrecision(false), _patchesIncludeNonLeaf(false), _patchesAreTriangular(false), _regPatchType(PatchDescriptor::NON_PATCH), _irregPatchType(PatchDescriptor::NON_PATCH), _regPatchSize(0), _irregPatchSize(0), _patchPointStride(0),
      _numSubFaces(0), _numControlPoints(0), _numRefinedPoints(0), _numSubPatchPoints(0), _numIrregPatches(0), _treeDepth(-1)
{
}
...
//
PatchTree::PatchPointArray PatchTree::GetSubPatchPoints(int patchIndex) const { return PatchPointArray(&_patchPoints[patchIndex * _patchPointStride], _patchParams[patchIndex].IsRegular() ? _regPatchSize : _irregPatchSize); }

template <typename REAL> int PatchTree::EvalSubPatchBasis(int patchIndex, REAL u, REAL v, REAL wP[], REAL wDu[], REAL wDv[], REAL wDuu[], REAL wDuv[], REAL wDvv[]) const
{
    PatchParam const &param = _patchParams[patchIndex];

    return Far::internal::EvaluatePatchBasis(param.IsRegular() ? _regPatchType : _irregPatchType, param, u, v, wP, wDu, wDv, wDuu, wDuv, wDvv);
}

template <typename REAL> int PatchTree::EvalSubPatchStencils(int patchIndex, REAL u, REAL v, REAL sP[], REAL sDu[], REAL sDv[], REAL sDuu[], REAL sDuv[], REAL sDvv[]) const
{
    //
    ...
    if ((param.GetDepth() == 0) && param.IsRegular() && !param.GetBoundary())
    {
        ...
        return Far::internal::EvaluatePatchBasis(_regPatchType, param, u, v, sP, sDu, sDv, sDuu, sDuv, sDuv);
    }

    //  Invoke according to precision of the internal stencil matrix:
    if (_useDoublePrecision)
    {
        return evalSubPatchStencils<double>(patchIndex, u, v, sP, sDu, sDv, sDuu, sDuv, sDvv);
    }
    else
    {
        return evalSubPatchStencils<float>(patchIndex, u, v, sP, sDu, sDv, sDuu, sDuv, sDvv);
    }
}

namespace
{
template <typename REAL_SRC, typename REAL_DST> void addToArray(REAL_DST *dst, int n, REAL_DST w, REAL_SRC const *src)
{
    if (isWeightZero(w))
        return;
...
template <typename REAL_MATRIX, typename REAL> int PatchTree::evalSubPatchStencils(int patchIndex, REAL u, REAL v, REAL sP[], REAL sDu[], REAL sDv[], REAL sDuu[], REAL sDuv[], REAL sDvv[]) const
{
    PatchParam const &param = _patchParams[patchIndex];
    ...
    Far::internal::EvaluatePatchBasis(param.IsRegular() ? _regPatchType : _irregPatchType, param, u, v, wP, wDu, wDv, wDuu, wDuv, wDvv);
    ...
    for (int i = 0; i < patchPoints.size(); ++i)
    {
        ...
        {
            std::vector<REAL_MATRIX> const &pStencilMtx = getStencilMatrix<REAL_MATRIX>();
            assert(!pStencilMtx.empty());

            REAL_MATRIX const *pStencilRow = &pStencilMtx[(pIndex - _numControlPoints) * _numControlPoints];
...
namespace
{
template <typename T> inline int transformUVToQuadQuadrant(T const &median, T &u, T &v)
{
    int uHalf = (u >= median);
    if (uHalf)
        u -= median;
...
template <typename T> int inline transformUVToTriQuadrant(T const &median, T &u, T &v, bool &rotated)
{
    if (!rotated)
    {
        if (u >= median)
...
inline PatchTree::TreeNode *PatchTree::assignLeafOrChildNode(TreeNode *node, bool isLeaf, int quadrant, int patchIndex)
{
...
int PatchTree::searchQuadtree(double u, double v, int subFace, int searchDepth) const
{
    //
    ...
    {
        int quadrant = _patchesAreTriangular ? transformUVToTriQuadrant(median, u, v, triRotated) : transformUVToQuadQuadrant(median, u, v);
...
//
template int PatchTree::EvalSubPatchBasis<float>(int patchIndex, float u, float v, float wP[], float wDu[], float wDv[], float wDuu[], float wDuv[], float wDvv[]) const;
template int PatchTree::EvalSubPatchStencils<float>(int patchIndex, float u, float v, float sP[], float sDu[], float sDv[], float sDuu[], float sDuv[], float sDvv[]) const;

template int PatchTree::EvalSubPatchBasis<double>(int patchIndex, double u, double v, double wP[], double wDu[], double wDv[], double wDuu[], double wDuv[], double wDvv[]) const;
template int PatchTree::EvalSubPatchStencils<double>(int patchIndex, double u, double v, double sP[], double sDu[], double sDv[], double sDuu[], double sDuv[], double sDvv[]) const;
