...
class PatchDescriptor
{
  ...
  public:
    ...
    PatchDescriptor() : _type(NON_PATCH) {}

    /// \brief Constructor
    PatchDescriptor(int type) : _type(type) {}

    /// \brief Copy Constructor
    PatchDescriptor(PatchDescriptor const &d) : _type(d.GetType()) {}
    ...
    static inline bool IsAdaptive(Type type) { return type > TRIANGLES; }

    /// \brief Returns true if the type is an adaptive patch
    bool IsAdaptive() const { return IsAdaptive(this->GetType()); }
    ...
    /// \brief Returns the number of control vertices expected for a patch of the
    /// type described
    short GetNumControlVertices() const { return GetNumControlVertices(this->GetType()); }

    /// \brief Deprecated @see PatchDescriptor#GetNumControlVertices
    short GetNumFVarControlVertices() const { return GetNumFVarControlVertices(this->GetType()); }
...
inline short PatchDescriptor::GetNumControlVertices(Type type)
{
    switch (type)
    {
    case REGULAR:
        return GetRegularPatchSize();
    case LOOP:
        return 12;
    case QUADS:
        return 4;
    ...
    case GREGORY_BOUNDARY:
        return GetGregoryPatchSize();
    case GREGORY_BASIS:
        return GetGregoryBasisPatchSize();
...
inline short PatchDescriptor::GetNumFVarControlVertices(Type type) { return PatchDescriptor::GetNumControlVertices(type); }

// Allows ordering of patches by type
inline bool PatchDescriptor::operator<(PatchDescriptor const other) const { return (_type < other._type); }
