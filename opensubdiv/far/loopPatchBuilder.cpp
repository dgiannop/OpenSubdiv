...
//
template <typename REAL> struct LoopLimits
{
  public:
    //
    ...
    class LimitVertex
    {
      public:
        ...
        void Assign(int faces, int edges)
        {
            _nFaces = faces;
            _nEdges = edges;
        }
  ...
  public:
    ...
    static void ComputeInteriorPointWeights(int valence, int faceInRing, REAL *pWeights, REAL *epWeights, REAL *emWeights);

    static void ComputeBoundaryPointWeights(int valence, int faceInRing, REAL *pWeights, REAL *epWeights, REAL *emWeights);
};

template <typename REAL> void LoopLimits<REAL>::ComputeInteriorPointWeights(int valence, int faceInRing, REAL *pWeights, REAL *epWeights, REAL *emWeights)
{
    ...
    if (!computeTangentPoints)
    {
        ...
        SchemeLoop().ComputeVertexLimitMask(vertex, pMask, Sdc::Crease::RULE_SMOOTH);
    }
    else
    {
        //
        ...
        SchemeLoop().ComputeVertexLimitMask(vertex, pMask, t1Mask, t2Mask, Sdc::Crease::RULE_SMOOTH);
...
template <typename REAL> void LoopLimits<REAL>::ComputeBoundaryPointWeights(int valence, int faceInRing, REAL *pWeights, REAL *epWeights, REAL *emWeights)
{
    ...
    if (!computeTangentPoints)
    {
        ...
        SchemeLoop().ComputeVertexLimitMask(vertex, pMask, Sdc::Crease::RULE_CREASE);
    }
    else
    {
        //
        ...
        SchemeLoop().ComputeVertexLimitMask(vertex, pMask, t1Mask, t2Mask, Sdc::Crease::RULE_CREASE);
...
namespace
{
template <typename REAL> class SparseMatrixRow
{
  public:
    SparseMatrixRow(SparseMatrix<REAL> &matrix, int row) : _size(matrix.GetRowSize(row)), _indices(matrix.SetRowColumns(row).begin()), _weights(matrix.SetRowElements(row).begin()) {}
...
#endif
template <typename REAL> inline bool _isZero(REAL w) { return (w == (REAL)0.0); }
#ifdef __INTEL_COMPILER
#pragma warning(pop)
#endif

template <typename REAL> void _initializeFullMatrix(SparseMatrix<REAL> &M, int nRows, int nColumns)
{
...
template <typename REAL> void _resizeMatrix(SparseMatrix<REAL> &matrix, int numRows, int numColumns, int numElements, int const rowSizes[])
{
...
template <typename REAL> void _addSparsePointToFullRow(REAL *fullRow, SparseMatrixRow<REAL> const &p, REAL s, int *indexMask)
{
...
template <typename REAL> void _combineSparsePointsInFullRow(SparseMatrixRow<REAL> &p, REAL aCoeff, SparseMatrixRow<REAL> const &a, REAL bCoeff, SparseMatrixRow<REAL> const &b, int rowSize, REAL *rowBuffer, int *maskBuffer)
{
...
template <typename REAL> void _addSparseRowToFull(REAL *fullRow, SparseMatrix<REAL> const &M, int sparseRow, REAL s)
{
...
template <typename REAL> void _combineSparseMatrixRowsInFull(SparseMatrix<REAL> &dstMatrix, int dstRowIndex, SparseMatrix<REAL> const &srcMatrix, int numSrcRows, int const srcRowIndices[], REAL const *srcRowWeights)
{
...
template <typename REAL> void _matrixPrintDensity(const char *prefix, SparseMatrix<REAL> const &M)
{
    ...
    printf("%s(%dx%d = %d):  elements = %d, non-zero = %d, density = %.1f\n", prefix, M.GetNumRows(), M.GetNumColumns(), fullSize, sparseSize, nonZeroSize, (REAL)nonZeroSize * 100.0f / (REAL)fullSize);
}
...
//
template <typename REAL> void _removeValence2Duplicates(SparseMatrix<REAL> &M)
{
...
//
template <typename REAL> class GregoryTriConverter
{
  public:
    typedef REAL                    Weight;
  ...
  private:
    ...
    void resizeMatrixIsolatedIrregular(Matrix &matrix, int irregCornerIndex, int irregValence) const;
    ...
    void computeIrregularEdgePoints(int cornerIndex, Matrix &matrix, Weight *weightBuffer) const;

    void assignRegularFacePoints(int cornerIndex, Matrix &matrix) const;
    void computeIrregularFacePoints(int cornerIndex, Matrix &matrix, Weight *weightBuffer, int *indexBuffer) const;

    void computeIrregularInteriorEdgePoints(int cornerIndex, Point &P, Point &Ep, Point &Em, Weight *weightBuffer) const;
    void computeIrregularBoundaryEdgePoints(int cornerIndex, Point &P, Point &Ep, Point &Em, Weight *weightBuffer) const;

    int  getIrregularFacePointSize(int cornerNear, int cornerFar) const;
    void computeIrregularFacePoint(int cornerNear, int edgeInNearRing, int cornerFar, Point const &p, Point const &eNear, Point const &eFar, Point &fNear, REAL signForSideOfEdge /* -1.0 or 1.0 */, Weight *rowWeights, int *columnMask) const;

    void assignRegularMidEdgePoint(int edgeIndex, Matrix &matrix) const;
    void computeIrregularMidEdgePoint(int edgeIndex, Matrix &matrix, Weight *rowWeights, int *columnMask) const;

    void promoteCubicEdgePointsToQuartic(Matrix &matrix, Weight *rowWeights, int *columnMask) const;
...
//
template <typename REAL> GregoryTriConverter<REAL>::GregoryTriConverter(SourcePatch const &sourcePatch) { Initialize(sourcePatch); }

template <typename REAL> GregoryTriConverter<REAL>::GregoryTriConverter(SourcePatch const &sourcePatch, Matrix &sparseMatrix)
{
    Initialize(sourcePatch);
    Convert(sparseMatrix);
}

template <typename REAL> void GregoryTriConverter<REAL>::Initialize(SourcePatch const &sourcePatch)
{
    //
    ...
    for (int cIndex = 0; cIndex < 3; ++cIndex)
    {
        ...
        corner.isRegular = ((corner.numFaces << corner.isBoundary) == 6) && !corner.isSharp;
        if (corner.isRegular)
        {
        ...
        {
            corner.faceAngle    = (corner.isBoundary ? REAL(M_PI) : REAL(2.0 * M_PI)) / REAL(corner.numFaces);
    ...
    _isIsolatedInteriorPatch = (irregularCount == 1) && (boundaryCount == 0) && (irregularValence > 2) && (sharpCount == 0);
...
template <typename REAL> void GregoryTriConverter<REAL>::Convert(Matrix &matrix) const
{
    //
...
template <typename REAL> void GregoryTriConverter<REAL>::resizeMatrixIsolatedIrregular(Matrix &matrix, int cornerIndex, int cornerValence) const
{
...
template <typename REAL> void GregoryTriConverter<REAL>::resizeMatrixUnisolated(Matrix &matrix) const
{
    int rowSizes[18];
    ...
    for (int cIndex = 0; cIndex < 3; ++cIndex)
    {
        ...
        if (!corner.fpIsRegular || !corner.fmIsRegular)
        {
            ...
            {
                rowSize[3] = getIrregularFacePointSize(cIndex, corner.fpIsCopied ? cPrev : cNext);
            }
            if (!corner.fmIsRegular)
            {
                rowSize[4] = getIrregularFacePointSize(cIndex, corner.fmIsCopied ? cNext : cPrev);
            }
        }
        ...
        else if (corner.isRegular && cornerNext.isRegular && (corner.epOnBoundary == cornerNext.emOnBoundary))
        {
            midEdgeSize = 4;
        }
        else
...
template <typename REAL> void GregoryTriConverter<REAL>::assignRegularEdgePoints(int cIndex, Matrix &matrix) const
{
...
template <typename REAL> void GregoryTriConverter<REAL>::computeIrregularEdgePoints(int cIndex, Matrix &matrix, Weight *weightBuffer) const
{
    Point p(matrix, 5 * cIndex + 0);
    Point ep(matrix, 5 * cIndex + 1);
    Point em(matrix, 5 * cIndex + 2);
...
template <typename REAL> void GregoryTriConverter<REAL>::computeIrregularInteriorEdgePoints(int cIndex, Point &p, Point &ep, Point &em, Weight *ringWeights) const
{
    CornerTopology const &corner = _corners[cIndex];
    ...
    //
    LoopLimits<REAL>::ComputeInteriorPointWeights(valence, corner.faceInRing, pWeights, epWeights, emWeights);
...
template <typename REAL> void GregoryTriConverter<REAL>::computeIrregularBoundaryEdgePoints(int cIndex, Point &p, Point &ep, Point &em, Weight *ringWeights) const
{
    CornerTopology const &corner = _corners[cIndex];
    ...
    //  computes pre-allocated ring weights for P, Ep and Em:
    //
    LoopLimits<REAL>::ComputeBoundaryPointWeights(valence, corner.faceInRing, pWeights, epWeights, emWeights);
...
template <typename REAL> int GregoryTriConverter<REAL>::getIrregularFacePointSize(int cIndexNear, int cIndexFar) const
{
    CornerTopology const &nearCorner = _corners[cIndexNear];
    CornerTopology const &farCorner  = _corners[cIndexFar];

    if (nearCorner.isSharp && farCorner.isSharp)
        return 2;

    int nearSize = nearCorner.ringPoints.GetSize() - 3;
    int farSize  = farCorner.ringPoints.GetSize() - 3;

    return 4 + (((nearSize > 0) && !nearCorner.isSharp) ? nearSize : 0) + (((farSize > 0) && !farCorner.isSharp) ? farSize : 0);
}

template <typename REAL>
void GregoryTriConverter<REAL>::computeIrregularFacePoint(int cIndexNear, int edgeInNearCornerRing, int cIndexFar, Point const &p, Point const &eNear, Point const &eFar, Point &fNear, REAL signForSideOfEdge, Weight *rowWeights, int *columnMask) const
{
...
template <typename REAL> void GregoryTriConverter<REAL>::assignRegularFacePoints(int cIndex, Matrix &matrix) const
{
    CornerTopology const &corner = _corners[cIndex];
...
template <typename REAL> void GregoryTriConverter<REAL>::computeIrregularFacePoints(int cIndex, Matrix &matrix, Weight *rowWeights, int *columnMask) const
{
    ...
    if (!corner.fpIsRegular && !corner.fpIsCopied)
    {
        ...
        computeIrregularFacePoint(cIndex, iEdgeP, cNext, p, ep, emNext, fp, 1.0, rowWeights, columnMask);
    }
    if (!corner.fmIsRegular && !corner.fmIsCopied)
    {
        int iEdgeM = (corner.faceInRing + 1) % corner.valence;
        computeIrregularFacePoint(cIndex, iEdgeM, cPrev, p, em, epPrev, fm, -1.0, rowWeights, columnMask);
    }
...
template <typename REAL> void GregoryTriConverter<REAL>::assignRegularMidEdgePoint(int edgeIndex, Matrix &matrix) const
{
    ...
    else
    {
        ...
        int oppositeInRing = corner.isBoundary ? (corner.faceInRing - 1) : ((corner.faceInRing + 5) % 6);
...
template <typename REAL> void GregoryTriConverter<REAL>::computeIrregularMidEdgePoint(int edgeIndex, Matrix &matrix, Weight *rowWeights, int *columnMask) const
{
    //
    ...
    _combineSparsePointsInFullRow(M, (REAL)0.5f, E0p, (REAL)0.5f, E1m, _numSourcePoints, rowWeights, columnMask);
}

template <typename REAL> void GregoryTriConverter<REAL>::promoteCubicEdgePointsToQuartic(Matrix &matrix, Weight *rowWeights, int *columnMask) const
{
    //
    ...
    for (int cIndex = 0; cIndex < 3; ++cIndex)
    {
        ...
        for (int ePair = 0; ePair < 2; ++ePair)
        {
            ...
            {
                _combineSparsePointsInFullRow(E, (REAL)0.25f, P, (REAL)0.75f, E, _numSourcePoints, rowWeights, columnMask);
            }
        }
    }
}
...
namespace
{
template <typename REAL> void _printPoint(SparseMatrixRow<REAL> const &p, bool printIndices = true, bool printWeights = true)
{
    printf("  Point size = %d:\n", p._size);
    if (printIndices)
    {
...
template <typename REAL> void _printMatrix(SparseMatrix<REAL> const &matrix, bool printIndices = true, bool printWeights = true)
{
...
void _printSourcePatch(SourcePatch const &patch, bool printCornerInfo = true, bool printRingPoints = true)
{
    ...
    if (printCornerInfo)
    {
        ...
        {
            printf("%6d:  boundary = %d, sharp = %d, numFaces = %d, in-ring = %d, ringSize = %d\n", i, patch._corners[i]._boundary, patch._corners[i]._sharp, patch._corners[i]._numFaces, patch._corners[i]._patchFace, patch._ringSizes[i]);
        }
    }
    if (printRingPoints)
    {
...
//
template <typename REAL> void convertToLinear(SourcePatch const &sourcePatch, SparseMatrix<REAL> &matrix)
{
    typedef REAL Weight;
    ...
    int numElements = sourcePatch.GetCornerRingSize(0) + sourcePatch.GetCornerRingSize(1) + sourcePatch.GetCornerRingSize(2);
    ...
    for (int cIndex = 0; cIndex < 3; ++cIndex)
    {
        ...
        {
            LoopLimits<REAL>::ComputeBoundaryPointWeights(1 + sourceCorner._numFaces, sourceCorner._patchFace, &weightBuffer[0], 0, 0);
        ...
        {
            LoopLimits<REAL>::ComputeInteriorPointWeights(sourceCorner._numFaces, sourceCorner._patchFace, &weightBuffer[0], 0, 0);
...
template <typename REAL> void convertToGregory(SourcePatch const &sourcePatch, SparseMatrix<REAL> &matrix)
{
    GregoryTriConverter<REAL> gregoryConverter(sourcePatch);
    gregoryConverter.Convert(matrix);
}

template <typename REAL> void convertToLoop(SourcePatch const &sourcePatch, SparseMatrix<REAL> &matrix)
{
    //
    ...
    //
    REAL const gregoryToLoopMatrix[12][15] = {{8.214411f, 7.571190f, -7.690082f, 2.237840f, -1.118922f, -16.428828f, 0.666666f, 0.666666f, 2.237835f, 6.309870f, 0.666666f, -1.690100f, -0.428812f, -0.428805f, 0.214407f},
                                              {-0.304687f, 0.609374f, 6.752593f, 0.609374f, -0.304687f, 0.609378f, -3.333333f, -3.333333f, 0.609378f, -1.247389f, -3.333333f, -1.247389f, 3.276037f, 3.276037f, -1.638020f},
                                              {-1.118922f, 2.237840f, -7.690082f, 7.571190f, 8.214411f, 2.237835f, 0.666666f, 0.666666f, -16.428828f, -1.690100f, 0.666666f, 6.309870f, -0.428805f, -0.428812f, 0.214407f},
                                              {8.214411f, -16.428828f, 6.309870f, -0.428812f, 0.214407f, 7.571190f, 0.666666f, 0.666666f, -0.428805f, -7.690082f, 0.666666f, -1.690100f, 2.237840f, 2.237835f, -1.118922f},
                                              {-0.813368f, 1.626735f, -0.773435f, -1.039929f, 0.519965f, 1.626735f, 0.666666f, 0.666666f, -1.039930f, -0.773435f, 0.666666f, 1.226558f, -1.039929f, -1.039930f, 0.519965f},
                                              {0.519965f, -1.039929f, -0.773435f, 1.626735f, -0.813368f, -1.039930f, 0.666666f, 0.666666f, 1.626735f, 1.226558f, 0.666666f, -0.773435f, -1.039930f, -1.039929f, 0.519965f},
                                              {0.214407f, -0.428812f, 6.309870f, -16.428828f, 8.214411f, -0.428805f, 0.666666f, 0.666666f, 7.571190f, -1.690100f, 0.666666f, -7.690082f, 2.237835f, 2.237840f, -1.118922f},
                                              {-0.304687f, 0.609378f, -1.247389f, 3.276037f, -1.638020f, 0.609374f, -3.333333f, -3.333333f, 3.276037f, 6.752593f, -3.333333f, -1.247389f, 0.609374f, 0.609378f, -0.304687f},
                                              {0.519965f, -1.039930f, 1.226558f, -1.039930f, 0.519965f, -1.039929f, 0.666666f, 0.666666f, -1.039929f, -0.773435f, 0.666666f, -0.773435f, 1.626735f, 1.626735f, -0.813368f},
                                              {-1.638020f, 3.276037f, -1.247389f, 0.609378f, -0.304687f, 3.276037f, -3.333333f, -3.333333f, 0.609374f, -1.247389f, -3.333333f, 6.752593f, 0.609378f, 0.609374f, -0.304687f},
                                              {-1.118922f, 2.237835f, -1.690100f, -0.428805f, 0.214407f, 2.237840f, 0.666666f, 0.666666f, -0.428812f, -7.690082f, 0.666666f, 6.309870f, 7.571190f, -16.428828f, 8.214411f},
                                              {0.214407f, -0.428805f, -1.690100f, 2.237835f, -1.118922f, -0.428812f, 0.666666f, 0.666666f, 2.237840f, 6.309870f, 0.666666f, -7.690082f, -16.428828f, 7.571190f, 8.214411f}};
...
//
PatchDescriptor::Type patchTypeFromBasisArray[] = {PatchDescriptor::NON_PATCH,        // undefined
...
LoopPatchBuilder::LoopPatchBuilder(TopologyRefiner const &refiner, Options const &options) : PatchBuilder(refiner, options)
{
    _regPatchType   = patchTypeFromBasisArray[_options.regBasisType];
    _irregPatchType = (_options.irregBasisType == BASIS_UNSPECIFIED) ? _regPatchType : patchTypeFromBasisArray[_options.irregBasisType];
...
PatchDescriptor::Type LoopPatchBuilder::patchTypeFromBasis(BasisType basis) const { return patchTypeFromBasisArray[(int)basis]; }

template <typename REAL> int LoopPatchBuilder::convertSourcePatch(SourcePatch const &sourcePatch, PatchDescriptor::Type patchType, SparseMatrix<REAL> &matrix) const
{
...
int LoopPatchBuilder::convertToPatchType(SourcePatch const &sourcePatch, PatchDescriptor::Type patchType, SparseMatrix<float> &matrix) const { return convertSourcePatch(sourcePatch, patchType, matrix); }
int LoopPatchBuilder::convertToPatchType(SourcePatch const &sourcePatch, PatchDescriptor::Type patchType, SparseMatrix<double> &matrix) const { return convertSourcePatch(sourcePatch, patchType, matrix); }
