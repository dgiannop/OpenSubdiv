...
//  std::cos(), std::sin(), etc.
//
template <typename REAL> struct CatmarkLimits
{
  public:
    typedef REAL Weight;

    static void ComputeInteriorPointWeights(int valence, int faceInRing, Weight *pWeights, Weight *epWeights, Weight *emWeights);

    static void ComputeBoundaryPointWeights(int valence, int faceInRing, Weight *pWeights, Weight *epWeights, Weight *emWeights);
...
//
template <typename REAL> inline double CatmarkLimits<REAL>::computeCoefficient(int valence)
{
    static double const efTable[] = {0.0,
                                     0.0,
                                     0.0,
...
template <typename REAL> void CatmarkLimits<REAL>::ComputeInteriorPointWeights(int valence, int faceInRing, Weight *pWeights, Weight *epWeights, Weight *emWeights)
{
    //
...
template <typename REAL> void CatmarkLimits<REAL>::ComputeBoundaryPointWeights(int valence, int faceInRing, Weight *pWeights, Weight *epWeights, Weight *emWeights)
{
    ...
    StackBuffer<Weight, 64, true> tanWeights(weightWidth);
    {
        ...
        double alpha_0k = -((1.0f + 2.0f * c) * std::sqrt(1.0f + c)) * div3kc / std::sqrt(1.0f - c);
...
//     P0         e0+      e1-         P1
//
template <typename REAL> class GregoryConverter
{
  public:
    typedef REAL                    Weight;
...
//
template <typename REAL> GregoryConverter<REAL>::GregoryConverter(SourcePatch const &sourcePatch) { Initialize(sourcePatch); }

template <typename REAL> GregoryConverter<REAL>::GregoryConverter(SourcePatch const &sourcePatch, Matrix &sparseMatrix)
{
    Initialize(sourcePatch);
    Convert(sparseMatrix);
}

template <typename REAL> void GregoryConverter<REAL>::Initialize(SourcePatch const &sourcePatch)
{
    //
    ...
    for (int cIndex = 0; cIndex < 4; ++cIndex)
    {
        ...
        corner.isRegular = ((corner.numFaces << corner.isBoundary) == 4) && !corner.isSharp;
        if (corner.isRegular)
        {
        ...
        {
            corner.faceAngle    = (corner.isBoundary ? REAL(M_PI) : (2.0f * REAL(M_PI))) / REAL(corner.numFaces);
...
template <typename REAL> void GregoryConverter<REAL>::Convert(Matrix &matrix) const
{
    //
...
template <typename REAL> void GregoryConverter<REAL>::resizeMatrixIsolatedIrregular(Matrix &matrix, int cornerIndex, int cornerValence) const
{
...
template <typename REAL> void GregoryConverter<REAL>::resizeMatrixUnisolated(Matrix &matrix) const
{
    int rowSizes[20];
...
template <typename REAL> void GregoryConverter<REAL>::assignRegularEdgePoints(int cIndex, Matrix &matrix) const
{
...
template <typename REAL> void GregoryConverter<REAL>::computeIrregularEdgePoints(int cIndex, Matrix &matrix, Weight *weightBuffer) const
{
    Point p(matrix, 5 * cIndex + 0);
    Point ep(matrix, 5 * cIndex + 1);
    Point em(matrix, 5 * cIndex + 2);
...
template <typename REAL> void GregoryConverter<REAL>::computeIrregularInteriorEdgePoints(int cIndex, Point &p, Point &ep, Point &em, Weight *ringWeights) const
{
    CornerTopology const &corner = _corners[cIndex];
    ...
    //
    CatmarkLimits<REAL>::ComputeInteriorPointWeights(valence, corner.faceInRing, pWeights, epWeights, emWeights);
...
template <typename REAL> void GregoryConverter<REAL>::computeIrregularBoundaryEdgePoints(int cIndex, Point &p, Point &ep, Point &em, Weight *ringWeights) const
{
    CornerTopology const &corner = _corners[cIndex];
    ...
    //  computes pre-allocated ring weights for P, Ep and Em:
    //
    CatmarkLimits<REAL>::ComputeBoundaryPointWeights(valence, corner.faceInRing, pWeights, epWeights, emWeights);
...
template <typename REAL> int GregoryConverter<REAL>::getIrregularFacePointSize(int cIndexNear, int cIndexFar) const
{
    CornerTopology const &corner    = _corners[cIndexNear];
    CornerTopology const &adjCorner = _corners[cIndexFar];

    if (corner.isSharp && adjCorner.isSharp)
        return 2;

    int thisSize = corner.isSharp ? 6 : (1 + corner.ringPoints.GetSize());

    int adjSize = (adjCorner.isRegular || adjCorner.isSharp) ? 0 : (1 + adjCorner.ringPoints.GetSize() - 6);

    return thisSize + adjSize;
}

template <typename REAL>
void GregoryConverter<REAL>::computeIrregularFacePoint(int cIndexNear, int edgeInNearCornerRing, int cIndexFar, Point const &p, Point const &eNear, Point const &eFar, Point &fNear, REAL signForSideOfEdge, Weight *rowWeights, int *columnMask) const
{
...
template <typename REAL> void GregoryConverter<REAL>::assignRegularFacePoints(int cIndex, Matrix &matrix) const
{
...
template <typename REAL> void GregoryConverter<REAL>::computeIrregularFacePoints(int cIndex, Matrix &matrix, Weight *rowWeights, int *columnMask) const
{
...
//
template <typename REAL> class BSplineConverter
{
  public:
    typedef REAL                 Weight;
    typedef SparseMatrix<Weight> Matrix;
  ...
  private:
    ...
    void buildIrregularCornerMatrix(int irregularCornerValence, int numSourcePoints, int const rowsForXPoints[7], Matrix &matrix) const;
...
template <typename REAL> BSplineConverter<REAL>::BSplineConverter(SourcePatch const &sourcePatch) { Initialize(sourcePatch); }
template <typename REAL> BSplineConverter<REAL>::BSplineConverter(SourcePatch const &sourcePatch, Matrix &matrix)
{
    Initialize(sourcePatch);
    Convert(matrix);
}

template <typename REAL> void BSplineConverter<REAL>::Initialize(SourcePatch const &sourcePatch)
{
    _sourcePatch = &sourcePatch;
    _gregoryConverter.Initialize(sourcePatch);
}

template <typename REAL> void BSplineConverter<REAL>::Convert(Matrix &matrix) const
{
    if (_gregoryConverter.IsIsolatedInteriorPatch())
    {
        convertIrregularCorner(_gregoryConverter.GetIsolatedInteriorCorner(), matrix);
    }
    else
    {
...
template <typename REAL> void BSplineConverter<REAL>::convertFromGregory(Matrix const &G, Matrix &B) const
{
    //
    ...
    //
    static int const pIndices[4][9] = {{3, 1, 2, 0, 8, 18, 7, 16, 13}, {8, 6, 7, 5, 3, 13, 12, 1, 18}, {13, 11, 12, 10, 18, 8, 17, 6, 3}, {18, 16, 17, 15, 13, 3, 2, 11, 8}};

    static int const epIndices[4][6] = {{3, 1, 8, 7, 18, 13}, {8, 6, 13, 12, 3, 18}, {13, 11, 18, 17, 8, 3}, {18, 16, 3, 2, 13, 8}};
    static int const emIndices[4][6] = {{3, 2, 18, 16, 8, 13}, {8, 7, 3, 1, 13, 18}, {13, 12, 8, 6, 18, 3}, {18, 17, 13, 11, 3, 8}};

    static int const fIndices[4][4] = {{3, 8, 18, 13}, {8, 13, 3, 18}, {13, 18, 8, 3}, {18, 3, 13, 8}};
...
template <typename REAL> void BSplineConverter<REAL>::buildIrregularCornerMatrix(int irregularCornerValence, int numSourcePoints, int const rowsForXPoints[7], Matrix &matrix) const
{
...
template <typename REAL> void BSplineConverter<REAL>::convertIrregularCorner(int irregularCorner, Matrix &matrix) const
{
    //
    ...
    //
    static int const xRowsAll[4][7] = {{0, 1, 4, 2, 8, 3, 12}, {3, 7, 2, 11, 1, 15, 0}, {15, 14, 11, 13, 7, 12, 3}, {12, 8, 13, 4, 14, 0, 15}};
    ...
    {
        pPoints[i] = (pNext < numSourcePoints) ? pNext : (pNext - numSourcePoints + 4);
    }
    ...
    static int const extPointRowsAll[4][5] = {{7, 11, 15, 14, 13}, {14, 13, 12, 8, 4}, {8, 4, 0, 1, 2}, {1, 2, 3, 7, 11}};
...
//  for consistency.
//
template <typename REAL> class LinearConverter
{
  public:
    typedef REAL                 Weight;
    typedef SparseMatrix<Weight> Matrix;
...
template <typename REAL> LinearConverter<REAL>::LinearConverter(SourcePatch const &sourcePatch) { Initialize(sourcePatch); }
template <typename REAL> LinearConverter<REAL>::LinearConverter(SourcePatch const &sourcePatch, Matrix &matrix)
{
    Initialize(sourcePatch);
    Convert(matrix);
}

template <typename REAL> void LinearConverter<REAL>::Initialize(SourcePatch const &sourcePatch) { _sourcePatch = &sourcePatch; }

template <typename REAL> void LinearConverter<REAL>::Convert(Matrix &matrix) const
{
    ...
    for (int cIndex = 0; cIndex < 4; ++cIndex)
    {
        ...
        if (sourceCorner._boundary)
        {
            CatmarkLimits<REAL>::ComputeBoundaryPointWeights(1 + sourceCorner._numFaces, sourceCorner._patchFace, &weightBuffer[0], 0, 0);
        ...
        {
            CatmarkLimits<REAL>::ComputeInteriorPointWeights(sourceCorner._numFaces, sourceCorner._patchFace, &weightBuffer[0], 0, 0);
...
template <typename REAL> int CatmarkPatchBuilder::convertSourcePatch(SourcePatch const &sourcePatch, PatchDescriptor::Type patchType, SparseMatrix<REAL> &matrix) const
{
...
int CatmarkPatchBuilder::convertToPatchType(SourcePatch const &sourcePatch, PatchDescriptor::Type patchType, SparseMatrix<float> &matrix) const { return convertSourcePatch(sourcePatch, patchType, matrix); }
int CatmarkPatchBuilder::convertToPatchType(SourcePatch const &sourcePatch, PatchDescriptor::Type patchType, SparseMatrix<double> &matrix) const { return convertSourcePatch(sourcePatch, patchType, matrix); }

CatmarkPatchBuilder::CatmarkPatchBuilder(TopologyRefiner const &refiner, Options const &options) : PatchBuilder(refiner, options)
{
...
PatchDescriptor::Type CatmarkPatchBuilder::patchTypeFromBasis(BasisType basis) const { return patchTypeFromBasisArray[(int)basis]; }
