...
//
//
bool TopologyRefinerFactoryBase::prepareComponentTopologySizing(TopologyRefiner &refiner)
{
    ...
    int fVertCount = baseLevel.getNumFaceVertices(fCount - 1) + baseLevel.getOffsetOfFaceVertices(fCount - 1);
...
bool TopologyRefinerFactoryBase::prepareComponentTopologyAssignment(TopologyRefiner &refiner, bool fullValidation, TopologyCallback callback, void const *callbackData)
{
    Vtr::internal::Level &baseLevel = refiner.getLevel(0);
...
bool TopologyRefinerFactoryBase::prepareComponentTagsAndSharpness(TopologyRefiner &refiner)
{
    //
    ...
    bool makeBoundaryFacesHoles = (options.GetVtxBoundaryInterpolation() == Sdc::Options::VTX_BOUNDARY_NONE) && (Sdc::SchemeTypeTraits::GetLocalNeighborhoodSize(refiner.GetSchemeType()) > 0);
    ...
    if (makeBoundaryFacesHoles)
    {
        for (Vtr::Index vIndex = 0; vIndex < baseLevel.getNumVertices(); ++vIndex)
        {
            ...
            {
                excludeFaces = (baseLevel.getNumEdgeFaces(vEdges[i]) == 1) && !Sdc::Crease::IsInfinite(baseLevel.getEdgeSharpness(vEdges[i]));
            }
            if (excludeFaces)
            {
    ...
    for (Vtr::Index vIndex = 0; vIndex < baseLevel.getNumVertices(); ++vIndex)
    {
        ...
        if (vTag._infSharpEdges)
        {
            ...
            //  inf-sharp Rule:
            Sdc::Crease::Rule infRule = creasing.DetermineVertexVertexRule((vTag._infSharp ? vSharpness : 0.0f), infSharpEdgeCount);
            ...
            if (infRule == Sdc::Crease::RULE_CREASE)
            {
                ...
                if (!vTag._xordinary && !vTag._nonManifold)
                {
                    ...
                    else
                    {
                        ...
                        {
                            vTag._infIrregular = (baseLevel.getEdgeTag(vEdges[0])._infSharp != baseLevel.getEdgeTag(vEdges[2])._infSharp);
                        }
                        else if (schemeRegularInteriorValence == 6)
                        {
                            vTag._infIrregular = (baseLevel.getEdgeTag(vEdges[0])._infSharp != baseLevel.getEdgeTag(vEdges[3])._infSharp) || (baseLevel.getEdgeTag(vEdges[1])._infSharp != baseLevel.getEdgeTag(vEdges[4])._infSharp);
                        }
                    }
                }
            }
...
bool TopologyRefinerFactoryBase::prepareFaceVaryingChannels(TopologyRefiner &refiner)
{
    Vtr::internal::Level &baseLevel = refiner.getLevel(0);
    ...
    for (int channel = 0; channel < refiner.GetNumFVarChannels(); ++channel)
    {
        if (baseLevel.getNumFVarValues(channel) == 0)
        {
            ...
            snprintf(msg, 1024,
                     "Failure in TopologyRefinerFactory<>::Create() -- "
                     "face-varying channel %d has no values.",
                     channel);
            Error(FAR_RUNTIME_ERROR, msg);
            return false;
        }
