...
PtexIndices::PtexIndices(TopologyRefiner const &refiner) { initializePtexIndices(refiner); }

PtexIndices::~PtexIndices() {}

void PtexIndices::initializePtexIndices(TopologyRefiner const &refiner)
{
...
int PtexIndices::GetNumFaces() const { return _ptexIndices.back(); }

int PtexIndices::GetFaceId(Index f) const
{
...
inline Index getAdjacentFace(Vtr::internal::Level const &level, Index edge, Index face)
{
...
void PtexIndices::GetAdjacency(TopologyRefiner const &refiner, int face, int quadrant, int adjFaces[4], int adjEdges[4]) const
{
    int regFaceSize = Sdc::SchemeTypeTraits::GetRegularFaceSize(refiner.GetSchemeType());
        ...
        for (int i = 0; i < regFaceSize; ++i)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    adjFaces[i] = _ptexIndices[adjface] + (aedges.FindIndex(edge) + 1) % aedges.size();
                    adjEdges[i] = 3;
                }
                assert(adjFaces[i] != -1);
            }
    ...
    else if (regFaceSize == 4)
    {
        ...
        */
        ...
        int nextQuadrant = (quadrant + 1) % fedges.size(), prevQuadrant = (quadrant + fedges.size() - 1) % fedges.size();
    ...
    {
        Far::Error(FAR_RUNTIME_ERROR, "Failure in PtexIndices::GetAdjacency() -- "
