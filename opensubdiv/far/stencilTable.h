...
template <typename REAL> class StencilReal
{
  public:
    ...
    ///
    StencilReal(int *size, Index *indices, REAL *weights) : _size(size), _indices(indices), _weights(weights) {}
    ...
    int *GetSizePtr() const { return _size; }

    /// \brief Returns the control vertices' indices
    Index const *GetVertexIndices() const { return _indices; }

    /// \brief Returns the interpolation weights
    REAL const *GetWeights() const { return _weights; }
...
class Stencil : public StencilReal<float>
{
  ...
  public:
    ...
    Stencil(int *size, Index *indices, float *weights) : BaseStencil(size, indices, weights) {}
};
...
template <typename REAL> class StencilTableReal
{
  ...
  public:
    ...
    int GetNumStencils() const { return (int)_sizes.size(); }

    /// \brief Returns the number of control vertices indexed in the table
    int GetNumControlVertices() const { return _numControlVertices; }
    ...
    std::vector<int> const &GetSizes() const { return _sizes; }

    /// \brief Returns the offset to a given stencil (factory may leave empty)
    std::vector<Index> const &GetOffsets() const { return _offsets; }

    /// \brief Returns the indices of the control vertices
    std::vector<Index> const &GetControlIndices() const { return _indices; }

    /// \brief Returns the stencil interpolation weights
    std::vector<REAL> const &GetWeights() const { return _weights; }
    ...
    ///
    template <class T, class U> void UpdateValues(T const &srcValues, U &dstValues, Index start = -1, Index end = -1) const { this->update(srcValues, dstValues, _weights, start, end); }

    template <class T1, class T2, class U> void UpdateValues(T1 const &srcBase, int numBase, T2 const &srcRef, U &dstValues, Index start = -1, Index end = -1) const { this->update(srcBase, numBase, srcRef, dstValues, _weights, start, end); }

    //  Pointer interface for backward compatibility
    template <class T, class U> void            UpdateValues(T const *src, U *dst, Index start = -1, Index end = -1) const { this->update(src, dst, _weights, start, end); }
    template <class T1, class T2, class U> void UpdateValues(T1 const *srcBase, int numBase, T2 const *srcRef, U *dst, Index start = -1, Index end = -1) const { this->update(srcBase, numBase, srcRef, dst, _weights, start, end); }
  ...
  protected:
    ...
    template <class T, class U> void            update(T const &srcValues, U &dstValues, std::vector<REAL> const &valueWeights, Index start, Index end) const;
    template <class T1, class T2, class U> void update(T1 const &srcBase, int numBase, T2 const &srcRef, U &dstValues, std::vector<REAL> const &valueWeights, Index start, Index end) const;
  ...
  protected:
    ...
    StencilTableReal(int numControlVerts) : _numControlVertices(numControlVerts) {}
...
class StencilTable : public StencilTableReal<float>
{
  ...
  public:
    Stencil GetStencil(Index index) const { return Stencil(BaseTable::GetStencil(index)); }
    Stencil operator[](Index index) const { return Stencil(BaseTable::GetStencil(index)); }
  ...
  protected:
    ...
    StencilTable(int numControlVerts, std::vector<int> const &offsets, std::vector<int> const &sizes, std::vector<int> const &sources, std::vector<float> const &weights, bool includeCoarseVerts, size_t firstOffset)
        : BaseTable(numControlVerts, offsets, sizes, sources, weights, includeCoarseVerts, firstOffset)
    {
    }
};
...
///
template <typename REAL> class LimitStencilReal : public StencilReal<REAL>
{
  public:
    /// \brief Constructor
    ///
    ...
    LimitStencilReal(int *size, Index *indices, REAL *weights, REAL *duWeights = 0, REAL *dvWeights = 0, REAL *duuWeights = 0, REAL *duvWeights = 0, REAL *dvvWeights = 0)
        : StencilReal<REAL>(size, indices, weights), _duWeights(duWeights), _dvWeights(dvWeights), _duuWeights(duuWeights), _duvWeights(duvWeights), _dvvWeights(dvvWeights)
    {
    }

    /// \brief Returns the u derivative weights
    REAL const *GetDuWeights() const { return _duWeights; }

    /// \brief Returns the v derivative weights
    REAL const *GetDvWeights() const { return _dvWeights; }

    /// \brief Returns the uu derivative weights
    REAL const *GetDuuWeights() const { return _duuWeights; }

    /// \brief Returns the uv derivative weights
    REAL const *GetDuvWeights() const { return _duvWeights; }

    /// \brief Returns the vv derivative weights
    REAL const *GetDvvWeights() const { return _dvvWeights; }
...
class LimitStencil : public LimitStencilReal<float>
{
  ...
  public:
    ...
    LimitStencil(int *size, Index *indices, float *weights, float *duWeights = 0, float *dvWeights = 0, float *duuWeights = 0, float *duvWeights = 0, float *dvvWeights = 0)
        : BaseStencil(size, indices, weights, duWeights, dvWeights, duuWeights, duvWeights, dvvWeights)
    {
    }
};
...
///
template <typename REAL> class LimitStencilTableReal : public StencilTableReal<REAL>
{
  protected:
    LimitStencilTableReal(int numControlVerts, std::vector<int> const &offsets, std::vector<int> const &sizes, std::vector<int> const &sources, std::vector<REAL> const &weights, std::vector<REAL> const &duWeights, std::vector<REAL> const &dvWeights,
                          std::vector<REAL> const &duuWeights, std::vector<REAL> const &duvWeights, std::vector<REAL> const &dvvWeights, bool includeCoarseVerts, size_t firstOffset);
  ...
  public:
    ...
    std::vector<REAL> const &GetDuWeights() const { return _duWeights; }

    /// \brief Returns the 'v' derivative stencil interpolation weights
    std::vector<REAL> const &GetDvWeights() const { return _dvWeights; }

    /// \brief Returns the 'uu' derivative stencil interpolation weights
    std::vector<REAL> const &GetDuuWeights() const { return _duuWeights; }

    /// \brief Returns the 'uv' derivative stencil interpolation weights
    std::vector<REAL> const &GetDuvWeights() const { return _duvWeights; }

    /// \brief Returns the 'vv' derivative stencil interpolation weights
    std::vector<REAL> const &GetDvvWeights() const { return _dvvWeights; }
    ...
    ///
    template <class T, class U> void UpdateDerivs(T const &srcValues, U &uderivs, U &vderivs, int start = -1, int end = -1) const
    {
        this->update(srcValues, uderivs, _duWeights, start, end);
        this->update(srcValues, vderivs, _dvWeights, start, end);
    }

    template <class T1, class T2, class U> void UpdateDerivs(T1 const &srcBase, int numBase, T2 const &srcRef, U &uderivs, U &vderivs, int start = -1, int end = -1) const
    {
    ...
    //  Pointer interface for backward compatibility
    template <class T, class U> void UpdateDerivs(T const *src, U *uderivs, U *vderivs, int start = -1, int end = -1) const
    {
    ...
    /// @param end        Index of last destination derivative to update
    ///
    template <class T, class U> void Update2ndDerivs(T const &srcValues, U &uuderivs, U &uvderivs, U &vvderivs, int start = -1, int end = -1) const
    {
    ...
    template <class T1, class T2, class U> void Update2ndDerivs(T1 const &srcBase, int numBase, T2 const &srcRef, U &uuderivs, U &uvderivs, U &vvderivs, int start = -1, int end = -1) const
    {
    ...
    //  Pointer interface for backward compatibility
    template <class T, class U> void Update2ndDerivs(T const *src, T *uuderivs, U *uvderivs, U *vvderivs, int start = -1, int end = -1) const
    {
...
class LimitStencilTable : public LimitStencilTableReal<float>
{
  ...
  public:
    LimitStencil GetLimitStencil(Index index) const { return LimitStencil(BaseTable::GetLimitStencil(index)); }
    LimitStencil operator[](Index index) const { return LimitStencil(BaseTable::GetLimitStencil(index)); }

  protected:
    LimitStencilTable(int numControlVerts, std::vector<int> const &offsets, std::vector<int> const &sizes, std::vector<int> const &sources, std::vector<float> const &weights, std::vector<float> const &duWeights, std::vector<float> const &dvWeights,
                      std::vector<float> const &duuWeights, std::vector<float> const &duvWeights, std::vector<float> const &dvvWeights, bool includeCoarseVerts, size_t firstOffset)
        : BaseTable(numControlVerts, offsets, sizes, sources, weights, duWeights, dvWeights, duuWeights, duvWeights, dvvWeights, includeCoarseVerts, firstOffset)
    {
    }
};

// Update values by applying cached stencil weights to new control values
template <typename REAL> template <class T1, class T2, class U> void StencilTableReal<REAL>::update(T1 const &srcBase, int numBase, T2 const &srcRef, U &dstValues, std::vector<REAL> const &valueWeights, Index start, Index end) const
{
...
template <typename REAL> template <class T, class U> void StencilTableReal<REAL>::update(T const &srcValues, U &dstValues, std::vector<REAL> const &valueWeights, Index start, Index end) const
{
    int const *  sizes   = &_sizes.at(0);
    Index const *indices = &_indices.at(0);
    REAL const * weights = &valueWeights.at(0);
...
template <typename REAL> inline void StencilTableReal<REAL>::generateOffsets()
{
    Index offset   = 0;
...
template <typename REAL> inline void StencilTableReal<REAL>::resize(int nstencils, int nelems)
{
...
template <typename REAL> inline void StencilTableReal<REAL>::reserve(int nstencils, int nelems)
{
...
template <typename REAL> inline void StencilTableReal<REAL>::shrinkToFit()
{
...
template <typename REAL> inline void StencilTableReal<REAL>::finalize()
{
    shrinkToFit();
    generateOffsets();
}
...
template <typename REAL> inline StencilReal<REAL> StencilTableReal<REAL>::GetStencil(Index i) const
{
    assert((!_offsets.empty()) && i < (int)_offsets.size());

    Index ofs = _offsets[i];

    return StencilReal<REAL>(const_cast<int *>(&_sizes[i]), const_cast<Index *>(&_indices[ofs]), const_cast<REAL *>(&_weights[ofs]));
}

template <typename REAL> inline StencilReal<REAL> StencilTableReal<REAL>::operator[](Index index) const { return GetStencil(index); }

template <typename REAL> inline void LimitStencilTableReal<REAL>::resize(int nstencils, int nelems)
{
...
template <typename REAL> inline LimitStencilReal<REAL> LimitStencilTableReal<REAL>::GetLimitStencil(Index i) const
{
    assert((!this->GetOffsets().empty()) && i < (int)this->GetOffsets().size());

    Index ofs = this->GetOffsets()[i];

    if (!_duWeights.empty() && !_dvWeights.empty() && !_duuWeights.empty() && !_duvWeights.empty() && !_dvvWeights.empty())
    {
        return LimitStencilReal<REAL>(const_cast<int *>(&this->GetSizes()[i]), const_cast<Index *>(&this->GetControlIndices()[ofs]), const_cast<REAL *>(&this->GetWeights()[ofs]), const_cast<REAL *>(&GetDuWeights()[ofs]),
                                      const_cast<REAL *>(&GetDvWeights()[ofs]), const_cast<REAL *>(&GetDuuWeights()[ofs]), const_cast<REAL *>(&GetDuvWeights()[ofs]), const_cast<REAL *>(&GetDvvWeights()[ofs]));
    }
    ...
    {
        return LimitStencilReal<REAL>(const_cast<int *>(&this->GetSizes()[i]), const_cast<Index *>(&this->GetControlIndices()[ofs]), const_cast<REAL *>(&this->GetWeights()[ofs]));
    }
}

template <typename REAL> inline LimitStencilReal<REAL> LimitStencilTableReal<REAL>::operator[](Index index) const { return GetLimitStencil(index); }
