...
template <typename REAL> class StencilTableFactoryReal
{
  public:
    ...
    struct Options
    {
        Options() : interpolationMode(INTERPOLATE_VERTEX), generateOffsets(false), generateControlVerts(false), generateIntermediateLevels(true), factorizeIntermediateLevels(true), maxLevel(10), fvarChannel(0) {}
    ...
    {
        return AppendLocalPointStencilTable(refiner, baseStencilTable, localPointStencilTable, factorize);
    }
  ...
  private:
    ...
    static void generateControlVertStencils(int numControlVerts, StencilReal<REAL> &dst);

    // Internal method to splice local point stencils
    static StencilTableReal<REAL> const *appendLocalPointStencilTable(TopologyRefiner const &refiner, StencilTableReal<REAL> const *baseStencilTable, StencilTableReal<REAL> const *localPointStencilTable, int channel, bool factorize);
};
...
template <typename REAL> class LimitStencilTableFactoryReal
{
  public:
    ...
    struct Options
    {
        Options() : interpolationMode(INTERPOLATE_VERTEX), generate1stDerivatives(true), generate2ndDerivatives(false), fvarChannel(0) {}
...
class StencilTableFactory : public StencilTableFactoryReal<float>
{
  ...
  public:
    static StencilTable const *Create(TopologyRefiner const &refiner, Options options = Options()) { return static_cast<StencilTable const *>(BaseFactory::Create(refiner, options)); }

    static StencilTable const *Create(int numTables, StencilTable const **tables) { return static_cast<StencilTable const *>(BaseFactory::Create(numTables, reinterpret_cast<BaseTable const **>(tables))); }
    ...
    {
        return static_cast<StencilTable const *>(BaseFactory::AppendLocalPointStencilTable(refiner, static_cast<BaseTable const *>(baseStencilTable), static_cast<BaseTable const *>(localPointStencilTable), factorize));
    }
    ...
    {
        return static_cast<StencilTable const *>(BaseFactory::AppendLocalPointStencilTableVarying(refiner, static_cast<BaseTable const *>(baseStencilTable), static_cast<BaseTable const *>(localPointStencilTable), factorize));
    }
    ...
    {
        return static_cast<StencilTable const *>(BaseFactory::AppendLocalPointStencilTableFaceVarying(refiner, static_cast<BaseTable const *>(baseStencilTable), static_cast<BaseTable const *>(localPointStencilTable), channel, factorize));
    }
};
...
class LimitStencilTableFactory : public LimitStencilTableFactoryReal<float>
{
  ...
  public:
    ...
    {
        return static_cast<LimitStencilTable const *>(BaseFactory::Create(refiner, locationArrays, static_cast<BaseTable const *>(cvStencils), patchTable, options));
    }
};
