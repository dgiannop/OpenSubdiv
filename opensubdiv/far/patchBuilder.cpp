...
//
inline int fastMod4(int x) { return x & 0x3; }
inline int fastMod3(int x)
{
    static int const mod3Array[] = {0, 1, 2, 0, 1, 2};
    return mod3Array[x];
}
inline int fastModN(int x, int N) { return (x < N) ? x : (x - N); }
...
//
inline Level::ETag getSingularEdgeMask(bool includeAllInfSharpEdges = false)
{
    Level::ETag eTagMask;
    eTagMask.clear();
...
inline bool isEdgeSingular(Level const &level, FVarLevel const *fvarLevel, Index eIndex, Level::ETag eTagMask)
{
...
void identifyManifoldCornerSpan(Level const &level, Index fIndex, int fCorner, Level::ETag eTagMask, Level::VSpan &vSpan, int fvc = -1)
{
...
void identifyNonManifoldCornerSpan(Level const &level, Index fIndex, int fCorner, Level::ETag eTagMask, Level::VSpan &vSpan, int fvc = -1)
{
    FVarLevel const *fvarLevel = (fvc < 0) ? 0 : &level.getFVarLevel(fvc);
...
inline int countManifoldCornerSpan(Level const &level, Index fIndex, int fCorner, Level::ETag eTagMask, int fvc = -1)
{
    Level::VSpan vSpan;
    identifyManifoldCornerSpan(level, fIndex, fCorner, eTagMask, vSpan, fvc);
    return vSpan._numFaces;
}
inline int countNonManifoldCornerSpan(Level const &level, Index fIndex, int fCorner, Level::ETag eTagMask, int fvc = -1)
{
    Level::VSpan vSpan;
...
//
int gatherTriRegularRingAroundVertex(Level const &level, Index vIndex, int ringPoints[], int fvarChannel)
{
    ...
    for (int i = 0; i < vFaces.size(); ++i)
    {
        ...
        //
        ConstIndexArray fPoints = (fvarChannel < 0) ? level.getFaceVertices(vFaces[i]) : level.getFaceFVarValues(vFaces[i], fvarChannel);
...
int gatherRegularPartialRingAroundVertex(Level const &level, Index vIndex, Level::VSpan const &span, int ringPoints[], int fvarChannel)
{
    ...
    for (int i = 0; i < nFaces; ++i)
    {
        ...
        ConstIndexArray fPoints = (fvarChannel < 0) ? level.getFaceVertices(thisFace) : level.getFaceFVarValues(thisFace, fvarChannel);
...
int encodeTriBoundaryMask(int eBits, int vBits)
{
    int upperBits = 0;
    int lowerBits = eBits;
...
void decodeTriBoundaryMask(int mask, int &eBits, int &vBits)
{
...
inline Index getNextFaceInVertFaces(Level const &level, int thisFaceInVFaces, ConstIndexArray const &vFaces, ConstLocalIndexArray const &vInFaces, bool manifold, int &vInNextFace)
{
    Index nextFace;
    if (manifold)
    {
...
inline Index getPrevFaceInVertFaces(Level const &level, int thisFaceInVFaces, ConstIndexArray const &vFaces, ConstLocalIndexArray const &vInFaces, bool manifold, int &vInPrevFace)
{
    Index prevFace;
    if (manifold)
    {
...
inline ConstIndexArray getFacePoints(Level const &level, Index faceIndex, int fvarChannel) { return (fvarChannel < 0) ? level.getFaceVertices(faceIndex) : level.getFaceFVarValues(faceIndex, fvarChannel); }
...
//
PatchBuilder *PatchBuilder::Create(TopologyRefiner const &refiner, Options const &options)
{
...
PatchBuilder::PatchBuilder(TopologyRefiner const &refiner, Options const &options) : _refiner(refiner), _options(options)
{
    //
...
//
bool PatchBuilder::IsFaceAPatch(int levelIndex, Index faceIndex) const
{
    Level const &level = _refiner.getLevel(levelIndex);

    //  Faces tagged as holes are not patches (no limit surface)
    if (_refiner.HasHoles() && level.isFaceHole(faceIndex))
        return false;
...
bool PatchBuilder::IsFaceALeaf(int levelIndex, Index faceIndex) const
{
...
bool PatchBuilder::IsPatchRegular(int levelIndex, Index faceIndex, int fvc) const
{
    if (_schemeIsLinear)
    {
    ...
    {
        bool featureRequiresFurtherInspection = fCompVTag._nonManifold || (_options.approxSmoothCornerWithSharp && fCompVTag._xordinary && fCompVTag._boundary) || (testInfSharpFeatures && fCompVTag._infIrregular && fCompVTag._infSharpEdges);
    ...
    for (int i = 0; i < _schemeRegFaceSize; ++i)
    {
        ...
        if ((vTag.getBits() & irregFeatureMask) == 0)
            continue;

        if (vTag._nonManifold)
        {
            //  Identify the span containing the face and assess:
            int nSpanFaces = countNonManifoldCornerSpan(level, faceIndex, i, getSingularEdgeMask(testInfSharpFeatures), fvc);
            if (vTag._infSharp)
            {
                if (nSpanFaces != 1)
                    return false;
            ...
            {
                if (nSpanFaces != (nRegBoundaryFaces))
                    return false;
            }
            continue;
        }
        ...
        if (vTag._xordinary)
        {
            ...
            if (_options.approxSmoothCornerWithSharp && vTag._boundary && !vTag._infSharp)
            {
                Level::ETag eTags[4];
        ...
        if (vTag._infIrregular)
        {
            ...
            if (vTag._infSharpCrease && vTag._boundary)
                return false;

            //  Identify the span containing the face and assess:
            int nSpanFaces = countManifoldCornerSpan(level, faceIndex, i, getSingularEdgeMask(true), fvc);
...
int PatchBuilder::GetRegularPatchBoundaryMask(int levelIndex, Index faceIndex, int fvarChannel) const
{
    if (_schemeIsLinear)
    {
    ...
    int eBits = (((eTags[0].getBits() & eFeatureMask) != 0) << 0) | (((eTags[1].getBits() & eFeatureMask) != 0) << 1) | (((eTags[2].getBits() & eFeatureMask) != 0) << 2);
    if (_schemeRegFaceSize == 4)
    {
    ...
    int vBits = (((vTags[0].getBits() & vFeatureMask) != 0) << 0) | (((vTags[1].getBits() & vFeatureMask) != 0) << 1) | (((vTags[2].getBits() & vFeatureMask) != 0) << 2);

    return (eBits || vBits) ? encodeTriBoundaryMask(eBits, vBits) : 0;
}

void PatchBuilder::GetIrregularPatchCornerSpans(int levelIndex, Index faceIndex, Level::VSpan cornerSpans[4], int fvarChannel) const
{
    Level const &level = _refiner.getLevel(levelIndex);
    ...
    for (int i = 0; i < _schemeRegFaceSize; ++i)
    {
        ...
        bool testInfSharpEdges = testInfSharpFeatures && vTag._infSharpEdges && (vTag._rule != Sdc::Crease::RULE_DART);
        ...
        if (testInfSharpEdges || isFVarMisMatch || isNonManifold)
        {
            if (isNonManifold)
            {
                identifyNonManifoldCornerSpan(level, faceIndex, i, singularEdgeMask, cornerSpans[i], fvarChannel);
            }
            else
            {
                identifyManifoldCornerSpan(level, faceIndex, i, singularEdgeMask, cornerSpans[i], fvarChannel);
            }
        }
        else
        {
        ...
        {
            cornerSpans[i]._sharp = testInfSharpEdges ? !vTag._infSharpCrease : vTag._infSharp;
        }

        //  Legacy option -- reinterpret a smooth corner as sharp:
        bool smoothCorner = !cornerSpans[i]._sharp;
        if (smoothCorner && _options.approxSmoothCornerWithSharp && vTag._xordinary && vTag._boundary && !vTag._infSharp && !vTag._nonManifold)
        {
            int nFaces            = cornerSpans[i].isAssigned() ? cornerSpans[i]._numFaces : level.getVertexFaces(level.getFaceVertices(faceIndex)[i]).size();
            cornerSpans[i]._sharp = (nFaces == 1);
        }
    }
}

int PatchBuilder::getRegularFacePoints(int levelIndex, Index faceIndex, Index patchPoints[], int fvarChannel) const
{
    Level const &level = _refiner.getLevel(levelIndex);

    ConstIndexArray facePoints = (fvarChannel < 0) ? level.getFaceVertices(faceIndex) : level.getFaceFVarValues(faceIndex, fvarChannel);
...
int PatchBuilder::getQuadRegularPatchPoints(int levelIndex, Index faceIndex, int regBoundaryMask, Index patchPoints[], int fvarChannel) const
{
    ...
    static int const patchPointsPerCorner[4][4] = {{5, 4, 0, 1}, {6, 2, 3, 7}, {10, 11, 15, 14}, {9, 13, 12, 8}};
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        //  incident face:
        //
        bool interiorCorner = interiorPatch || (((eMask & (1 << i)) | (eMask & (1 << fastMod4(i + 3)))) == 0);
        if (interiorCorner)
        {
        ...
        else if (eMask & (1 << i))
        {
            ...
            int   vInFNext;
            Index fNext = getNextFaceInVertFaces(level, fInVFaces, vFaces, vInFaces, !level.getVertexTag(v)._nonManifold, vInFNext);
        ...
        else
        {
            ...
            int   vInFPrev;
            Index fPrev = getPrevFaceInVertFaces(level, fInVFaces, vFaces, vInFaces, !level.getVertexTag(v)._nonManifold, vInFPrev);
...
int PatchBuilder::getTriRegularPatchPoints(int levelIndex, Index faceIndex, int regBoundaryMask, Index patchPoints[], int fvarChannel) const
{
    if (regBoundaryMask < 0)
    {
        regBoundaryMask = GetRegularPatchBoundaryMask(levelIndex, faceIndex);
    ...
    static int const patchPointsPerCorner[3][4] = {{4, 7, 3, 0}, {5, 1, 2, 6}, {8, 9, 11, 10}};
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        else if (eMask & (1 << i))
        {
            ...
            {
                Index f1 = getNextFaceInVertFaces(level, fInVFaces, vFaces, vInFaces, false, vInf2);
                f2       = getNextFaceInVertFaces(level, vFaces.FindIndex(f1), vFaces, vInFaces, false, vInf2);
            }
            ConstIndexArray f2Points = getFacePoints(level, f2, fvarChannel);
        ...
        else if (eMask & (1 << fastMod3(i + 2)))
        {
            ...
            if (level.getVertexTag(v)._nonManifold)
            {
                Index f1 = getPrevFaceInVertFaces(level, fInVFaces, vFaces, vInFaces, false, vInf0);
                f0       = getPrevFaceInVertFaces(level, vFaces.FindIndex(f1), vFaces, vInFaces, false, vInf0);
            }
        ...
        else
        {
            ...
            int   vInf2;
            Index f2 = getNextFaceInVertFaces(level, fInVFaces, vFaces, vInFaces, !level.getVertexTag(v)._nonManifold, vInf2);
...
int PatchBuilder::GetRegularPatchPoints(int levelIndex, Index faceIndex, int regBoundaryMask, Index patchPoints[], int fvarChannel) const
{
    if (_schemeIsLinear)
    {
        return getRegularFacePoints(levelIndex, faceIndex, patchPoints, fvarChannel);
    }
    else if (_schemeRegFaceSize == 4)
    {
        return getQuadRegularPatchPoints(levelIndex, faceIndex, regBoundaryMask, patchPoints, fvarChannel);
    }
    else
    {
        return getTriRegularPatchPoints(levelIndex, faceIndex, regBoundaryMask, patchPoints, fvarChannel);
    }
    return 0;
}

int PatchBuilder::assembleIrregularSourcePatch(int levelIndex, Index faceIndex, Level::VSpan const cornerSpans[], SourcePatch &sourcePatch) const
{
    //
...
//
int PatchBuilder::gatherIrregularSourcePoints(int levelIndex, Index faceIndex, Level::VSpan const cornerSpans[4], SourcePatch &sourcePatch, Index patchVerts[], int fvarChannel) const
{
    //
    ...
    for (int corner = 0; corner < sourcePatch._numCorners; ++corner)
    {
        ...
        if (cornerSpans[corner].isAssigned())
        {
            sourceRingSize = gatherRegularPartialRingAroundVertex(level, cornerVertex, cornerSpans[corner], sourceRingVertices, fvarChannel);
        }
        else if (sourcePatch._numCorners == 4)
        {
            sourceRingSize = level.gatherQuadRegularRingAroundVertex(cornerVertex, sourceRingVertices, fvarChannel);
        }
        else
        {
            sourceRingSize = gatherTriRegularRingAroundVertex(level, cornerVertex, sourceRingVertices, fvarChannel);
        }

        //  Gather the ring of local points from the patch:
        int patchRingSize = sourcePatch.GetCornerRingPoints(corner, patchRingPoints);
...
int PatchBuilder::GetIrregularPatchSourcePoints(int levelIndex, Index faceIndex, Level::VSpan const cornerSpans[], Index sourcePoints[], int fvarChannel) const
{
    SourcePatch sourcePatch;
    assembleIrregularSourcePatch(levelIndex, faceIndex, cornerSpans, sourcePatch);

    return gatherIrregularSourcePoints(levelIndex, faceIndex, cornerSpans, sourcePatch, sourcePoints, fvarChannel);
}
...
//
template <typename REAL> int PatchBuilder::GetIrregularPatchConversionMatrix(int levelIndex, Index faceIndex, Level::VSpan const cornerSpans[], SparseMatrix<REAL> &conversionMatrix) const
{
    SourcePatch sourcePatch;
    assembleIrregularSourcePatch(levelIndex, faceIndex, cornerSpans, sourcePatch);

    return convertToPatchType(sourcePatch, GetIrregularPatchType(), conversionMatrix);
}
template int PatchBuilder::GetIrregularPatchConversionMatrix<float>(int levelIndex, Index faceIndex, Level::VSpan const cornerSpans[], SparseMatrix<float> &conversionMatrix) const;
template int PatchBuilder::GetIrregularPatchConversionMatrix<double>(int levelIndex, Index faceIndex, Level::VSpan const cornerSpans[], SparseMatrix<double> &conversionMatrix) const;

bool PatchBuilder::IsRegularSingleCreasePatch(int levelIndex, Index faceIndex, SingleCreaseInfo &creaseInfo) const
{
    if (_schemeRegFaceSize != 4)
        return false;
    ...
    return level.isSingleCreasePatch(faceIndex, &creaseInfo.creaseSharpness, &creaseInfo.creaseEdgeInFace);
}

PatchParam PatchBuilder::ComputePatchParam(int levelIndex, Index faceIndex, PtexIndices const &ptexIndices, bool isRegular, int boundaryMask, bool computeTransitionMask) const
{
    // Move up the hierarchy accumulating u,v indices to the coarse level:
    int depth              = levelIndex;
    int childIndexInParent = 0, u = 0, v = 0, ofs = 1;

    int regFaceSize = _schemeRegFaceSize;

    bool irregBase = _refiner.GetLevel(depth).GetFaceVertices(faceIndex).size() != regFaceSize;
    ...
    for (int i = depth; i > 0; --i)
    {
        ...
        Index parentFaceIndex = refinement.getChildFaceParentFace(childFaceIndex);

        irregBase = parentLevel.getFaceVertices(parentFaceIndex).size() != regFaceSize;
        ...
        if (_schemeRegFaceSize == 3)
        {
            ...
            childIndexInParent = refinement.getChildFaceInParentFace(childFaceIndex);
        ...
        else
        {
            ...
            ConstIndexArray children = refinement.getFaceChildFaces(parentFaceIndex);
    ...
    {
        transitionMask = _refiner.getRefinement(levelIndex).getParentFaceSparseTag(faceIndex)._transitional;
    }

    PatchParam param;
    param.Set(ptexIndex, (short)u, (short)v, (unsigned short)depth, irregBase, (unsigned short)boundaryMask, (unsigned short)transitionMask, (unsigned short)isRegular);
    return param;
}
...
//
void SourcePatch::Finalize(int size)
{
    //
    ...
    for (int cIndex = 0; cIndex < _numCorners; ++cIndex)
    {
        ...
        bool prevIsVal2Interior = ((_corners[cPrev]._numFaces == 2) && !_corners[cPrev]._boundary);
        bool thisIsVal2Interior = ((_corners[cIndex]._numFaces == 2) && !_corners[cIndex]._boundary);
        bool nextIsVal2Interior = ((_corners[cNext]._numFaces == 2) && !_corners[cNext]._boundary);
        ...
        if ((corner._numFaces + corner._boundary) > 2)
        {
            ...
            _localRingSizes[cIndex] = _ringSizes[cIndex] - (_numCorners - 1) - corner._sharesWithPrev - corner._sharesWithNext;
...
int SourcePatch::GetCornerRingPoints(int corner, int ringPoints[]) const
{
