...
TopologyRefiner::TopologyRefiner(Sdc::SchemeType schemeType, Sdc::Options schemeOptions)
    : _subdivType(schemeType), _subdivOptions(schemeOptions), _isUniform(true), _hasHoles(false), _hasIrregFaces(false), _regFaceSize(Sdc::SchemeTypeTraits::GetRegularFaceSize(schemeType)), _maxLevel(0), _uniformOptions(0), _adaptiveOptions(0),
      _totalVertices(0), _totalEdges(0), _totalFaces(0), _totalFaceVertices(0), _maxValence(0), _baseLevelOwned(true)
{
...
TopologyRefiner::TopologyRefiner(TopologyRefiner const &source)
    : _subdivType(source._subdivType), _subdivOptions(source._subdivOptions), _isUniform(true), _hasHoles(source._hasHoles), _hasIrregFaces(source._hasIrregFaces), _regFaceSize(source._regFaceSize), _maxLevel(0), _uniformOptions(0), _adaptiveOptions(0),
...
//
void TopologyRefiner::initializeInventory()
{
    if (_levels.size())
    {
...
void TopologyRefiner::updateInventory(Vtr::internal::Level const &newLevel)
{
...
void TopologyRefiner::appendLevel(Vtr::internal::Level &newLevel)
{
...
void TopologyRefiner::appendRefinement(Vtr::internal::Refinement &newRefinement) { _refinements.push_back(&newRefinement); }

void TopologyRefiner::assembleFarLevels()
{
...
//
int TopologyRefiner::GetNumFVarValuesTotal(int channel) const
{
    int sum = 0;
    for (int i = 0; i < (int)_levels.size(); ++i)
    {
...
void TopologyRefiner::RefineUniform(UniformOptions options)
{
    ...
    {
        Error(FAR_RUNTIME_ERROR, "Failure in TopologyRefiner::RefineUniform() -- base level is uninitialized.");
        return;
    }
    if (_refinements.size())
    {
        Error(FAR_RUNTIME_ERROR, "Failure in TopologyRefiner::RefineUniform() -- previous refinements already applied.");
        return;
    }
    ...
    {
        refineOptions._minimalTopology = options.fullTopologyInLastLevel ? false : (i == (int)options.refinementLevel);
...
void FeatureMask::InitializeFeatures(Options const &options, int regFaceSize)
{
    //
...
void FeatureMask::ReduceFeatures(Options const &options)
{
...
void TopologyRefiner::RefineAdaptive(AdaptiveOptions options, ConstIndexArray baseFacesToRefine)
{
    if (_levels[0]->getNumVertices() == 0)
    {
        Error(FAR_RUNTIME_ERROR, "Failure in TopologyRefiner::RefineAdaptive() -- base level is uninitialized.");
        return;
    }
    if (_refinements.size())
    {
        Error(FAR_RUNTIME_ERROR, "Failure in TopologyRefiner::RefineAdaptive() -- previous refinements already applied.");
        return;
    }
    ...
    for (int i = 1; i <= potentialMaxLevel; ++i)
    {
        ...
        internal::FeatureMask const &levelFeatures = (i <= shallowLevel) ? moreFeaturesMask : lessFeaturesMask;
...
//
inline bool doesInfSharpVTagHaveFeatures(Vtr::internal::Level::VTag compVTag, internal::FeatureMask const &featureMask)
{
    ...
    if (compVTag._infIrregular)
    {
        ...
        {
            return compVTag._boundary ? featureMask.selectXOrdinaryBoundary : featureMask.selectInfSharpIrregularCrease;
        }
...
inline bool doesInfSharpFaceHaveFeatures(Vtr::internal::Level::VTag compVTag, Vtr::internal::Level::VTag vTags[], int numVerts, internal::FeatureMask const &featureMask)
{
    //
...
//  that result from it.
//
bool doesFaceHaveFeatures(Vtr::internal::Level const &level, Index face, internal::FeatureMask const &featureMask, int regFaceSize)
{
...
//
bool doesFaceHaveDistinctFaceVaryingFeatures(Vtr::internal::Level const &level, Index face, internal::FeatureMask const &featureMask, int fvarChannel)
{
    using Vtr::internal::Level;
...
//   refinement.
//
void TopologyRefiner::selectFeatureAdaptiveComponents(Vtr::internal::SparseSelector &selector, internal::FeatureMask const &featureMask, ConstIndexArray facesToRefine)
{
    //
    ...
    for (int fIndex = 0; fIndex < numFacesToRefine; ++fIndex)
    {
        ...
        if (HasHoles() && level.isFaceHole(face))
            continue;
        ...
        if (!selectFace && featureMask.selectFVarFeatures)
        {
            for (int channel = 0; !selectFace && (channel < numFVarChannels); ++channel)
            {
                ...
                {
                    selectFace = doesFaceHaveDistinctFaceVaryingFeatures(level, face, featureMask, channel);
                }
            }
        }
        if (selectFace)
...
void TopologyRefiner::selectLinearIrregularFaces(Vtr::internal::SparseSelector &selector, ConstIndexArray facesToRefine)
{
    //
