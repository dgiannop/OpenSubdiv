...
class PatchMap
{
  ...
  private:
    ...
    template <class T> static int transformUVToTriQuadrant(T const &median, T &u, T &v, bool &rotated);
...
template <class T> int inline PatchMap::transformUVToTriQuadrant(T const &median, T &u, T &v, bool &rotated)
{
    if (!rotated)
    {
        if (u >= median)
