...
struct TestOptions
{
    TestOptions() : refineLevel(2), refineAdaptive(true), createPatches(true), createStencils(true), endCapType(Far::PatchTableFactory::Options::ENDCAP_GREGORY_BASIS) {}
...
struct TestResult
{
    TestResult() : level(-1), timeTotal(0), timeRefine(0), timePatchFactory(0), timeStencilFactory(0), timeAppendStencil(0) {}
...
template <typename REAL> static TestResult RunPerfTest(Shape const &shape, TestOptions const &options)
{
    ...
    {
        Far::TopologyRefiner::AdaptiveOptions rOptions = poptions.GetRefineAdaptiveOptions();
    ...
    if (options.createPatches && options.createStencils)
    {
        ...
        if (FarStencilTable const *vertexStencilsWithLocalPoints = FarStencilTableFactory::AppendLocalPointStencilTable(*refiner, vertexStencils, patchTable->GetLocalPointStencilTable<REAL>()))
        {
...
struct PrintOptions
{
    PrintOptions() : csvFormat(false), refineTime(true), patchTime(true), stencilTime(true), appendTime(true), totalTime(true) {}
...
static void PrintShape(ShapeDesc const &shapeDesc, PrintOptions const &)
{
...
static void PrintResult(TestResult const &result, PrintOptions const &options)
{

    //  If only printing the total, combine on same line as level:
    if (!options.refineTime && !options.patchTime && !options.stencilTime && !options.appendTime)
    {
    ...
    {
        printf("    TopologyRefiner::Refine     %f %5.2f%%\n", result.timeRefine, result.timeRefine / result.timeTotal * 100);
    }
    if (options.patchTime)
    {
        printf("    PatchTableFactory::Create   %f %5.2f%%\n", result.timePatchFactory, result.timePatchFactory / result.timeTotal * 100);
    }
    if (options.stencilTime)
    {
        printf("    StencilTableFactory::Create %f %5.2f%%\n", result.timeStencilFactory, result.timeStencilFactory / result.timeTotal * 100);
    }
    if (options.appendTime)
    {
        printf("    StencilTableFactory::Append %f %5.2f%%\n", result.timeAppendStencil, result.timeAppendStencil / result.timeTotal * 100);
    }
...
static void PrintHeaderCSV(PrintOptions const &options)
{
    ...
    if (options.patchTime)
        printf(",patch");
    if (options.stencilTime)
        printf(",stencilFactory");
    if (options.appendTime)
        printf(",stencilAppend");
    if (options.totalTime)
        printf(",total");
    printf("\n");
}
static void PrintResultCSV(TestResult const &result, PrintOptions const &options)
{
...
static int parseIntArg(char const *argString, int dfltValue = 0)
{
    char *argEndptr;
    int   argValue = (int)strtol(argString, &argEndptr, 10);
    if (*argEndptr != 0)
    {
        fprintf(stderr, "Warning: non-integer option parameter '%s' ignored\n", argString);
        argValue = dfltValue;
    }
    return argValue;
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            if (++i < argc)
                maxLevel = parseIntArg(argv[i], maxLevel);
        }
        ...
        else if (!strcmp(argv[i], "-e"))
        {
            ...
            {
                testOptions.endCapType = Far::PatchTableFactory::Options::ENDCAP_BILINEAR_BASIS;
            }
            else if (!strcmp(type, "regular"))
            {
                testOptions.endCapType = Far::PatchTableFactory::Options::ENDCAP_BSPLINE_BASIS;
            }
            else if (!strcmp(type, "gregory"))
            {
                testOptions.endCapType = Far::PatchTableFactory::Options::ENDCAP_GREGORY_BASIS;
            }
            else
            {
    ...
    if (!objFiles.empty())
    {
        for (size_t i = 0; i < objFiles.size(); ++i)
        {
            ...
            {
                fprintf(stderr, "Warning: cannot open shape file '%s'\n", objFile);
            }
        }
    }
