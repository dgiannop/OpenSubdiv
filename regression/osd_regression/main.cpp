...
//------------------------------------------------------------------------------
int checkVertexBuffer(const FarTopologyRefiner &refiner, xyzmesh *hmesh, const float *vbData, int numElements)
{

    int   count       = 0;
    float deltaAvg[3] = {0.0f, 0.0f, 0.0f}, deltaCnt[3] = {0.0f, 0.0f, 0.0f};
    ...
    // is 'none'
    std::vector<bool> *hbrVertexOnBoundaryPtr = (hmesh->GetInterpolateBoundaryMethod() == xyzmesh::k_InterpolateBoundaryNone) ? &hbrVertexOnBoundaryData : NULL;

    GetReorderedHbrVertexData(refiner, *hmesh, &hbrVertexData, hbrVertexOnBoundaryPtr);
    ...
    for (int i = 0; i < nverts; ++i)
    {
        ...
        float delta[3] = {hbrPos[0] - ov[0], hbrPos[1] - ov[1], hbrPos[2] - ov[2]};
        ...
        if (dist > PRECISION)
        {
            printf("// HbrVertex<T> %d fails : dist=%.10f (%.10f %.10f %.10f)"
                   " (%.10f %.10f %.10f)\n",
                   i, dist, hbrPos[0], hbrPos[1], hbrPos[2], ov[0], ov[1], ov[2]);
            count++;
        }
    }
    ...
    printf("    delta ratio : (%d/%d %d/%d %d/%d)\n", (int)deltaCnt[0], nverts, (int)deltaCnt[1], nverts, (int)deltaCnt[2], nverts);
    printf("    average delta : (%.10f %.10f %.10f)\n", deltaAvg[0], deltaAvg[1], deltaAvg[2]);
    if (count == 0)
        printf("  success !\n");
...
//------------------------------------------------------------------------------
static void buildStencilTable(const FarTopologyRefiner &refiner, Far::StencilTable const **vertexStencils, Far::StencilTable const **varyingStencils)
{
...
//------------------------------------------------------------------------------
static int checkMeshCPU(FarTopologyRefiner *refiner, const std::vector<xyzVV> &coarseverts, xyzmesh *refmesh)
{
    ...
    Osd::CpuVertexBuffer *vb = Osd::CpuVertexBuffer::Create(3, refiner->GetNumVerticesTotal());

    vb->UpdateData(coarseverts[0].GetPos(), 0, (int)coarseverts.size());

    Osd::CpuEvaluator::EvalStencils(vb, Osd::BufferDescriptor(0, 3, 3), vb, Osd::BufferDescriptor(refiner->GetLevel(0).GetNumVertices() * 3, 3, 3), vertexStencils);

    int result = checkVertexBuffer(*refiner, refmesh, vb->BindCpuBuffer(), vb->GetNumElements());
...
//------------------------------------------------------------------------------
static int checkMeshCPUGL(FarTopologyRefiner *refiner, const std::vector<xyzVV> &coarseverts, xyzmesh *refmesh)
{
    ...
    Osd::CpuGLVertexBuffer *vb = Osd::CpuGLVertexBuffer::Create(3, refiner->GetNumVerticesTotal());
...
//------------------------------------------------------------------------------
static int checkMesh(char const *msg, std::string const &shape, int levels, Scheme scheme, int backend)
{
    ...
    xyzmesh *refmesh = interpolateHbrVertexData<xyzVV>(shape.c_str(), scheme, levels);

    std::vector<xyzVV> farVertexData;

    FarTopologyRefiner *refiner = InterpolateFarVertexData(shape.c_str(), scheme, levels, farVertexData);
...
void _glfw_error_callback(int error, const char *description) { printf("GLFW reported error %d: %s\n", error, description); }
