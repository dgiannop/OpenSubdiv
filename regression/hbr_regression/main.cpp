...
static bool g_allowWeakRegression = true, g_strictRegressionFailure = false, g_verbose = false;
...
//------------------------------------------------------------------------------
static void writeObj(const char *fname, xyzmesh const *mesh, int firstface, int lastface, int firstvert, int lastvert)
{
...
static int checkMesh(ShapeDesc const &r, int levels)
{
    ...
    int firstface = 0, lastface = mesh->GetNumFaces(), firstvert = 0, lastvert = mesh->GetNumVertices(), nverts;
    ...
    for (int l = 0; l < levels; ++l)
    {
        ...
        for (int i = firstvert; i < lastvert; ++i)
        {
            const float *apos = mesh->GetVertex(i)->GetData().GetPos(), *bpos = &sh->verts[(i - firstvert) * 3];
            ...
            float delta[3] = {apos[0] - bpos[0], apos[1] - bpos[1], apos[2] - bpos[2]};
                ...
                else
                {
                    if (g_verbose)
                    {
                        printf("\n// HbrVertex<T> %d fails : dist=%.10f "
                               ...
                               i, dist, apos[0], apos[1], apos[2], bpos[0], bpos[1], bpos[2]);
                    }
                    ++errcount;
                }
            }
        ...
        if (errcount)
        {
            ...
            writeObj(errfile.str().c_str(), mesh, firstface, lastface, firstvert, lastvert);
    ...
    if (g_verbose)
    {
        printf("\n  delta ratio : (%d/%d %d/%d %d/%d)", (int)deltaCnt[0], nverts, (int)deltaCnt[1], nverts, (int)deltaCnt[2], nverts);
        printf("\n  average delta : (%.10f %.10f %.10f)", deltaAvg[0], deltaAvg[1], deltaAvg[2]);
    }
