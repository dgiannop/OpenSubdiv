...
static void generate(char const *shapeStr, char const *name, int levels, Scheme scheme = kCatmark)
{
    ...
    int firstface = 0, lastface = mesh->GetNumFaces(), firstvert = 0, lastvert = mesh->GetNumVertices();
...
int main(int argc, char **argv)
{
    ...
    if (g_objfile.size())
    {
        ...
        generate(shapeStr, g_objfile.c_str(), levels, g_scheme);
    ...
    else if (g_shapeindex >= 0)
    {
        ...
        {
            for (int i = 0; i < (int)g_shapes.size(); ++i)
                generate(g_shapes[i].data.c_str(), g_shapes[i].name.c_str(), levels, g_shapes[i].scheme);
        }
        else
            generate(g_shapes[g_shapeindex].data.c_str(), g_shapes[g_shapeindex].name.c_str(), levels, g_shapes[g_shapeindex].scheme);
    }
}
