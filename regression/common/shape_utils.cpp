...
//------------------------------------------------------------------------------
Shape *Shape::parseObj(char const *shapestr, Scheme shapescheme, bool isLeftHanded, bool parsemtl)
{
...
//------------------------------------------------------------------------------
Shape *Shape::parseObj(ShapeDesc const &shapeDesc, bool parsemtl) { return parseObj(shapeDesc.data.c_str(), shapeDesc.scheme, shapeDesc.isLeftHanded, parsemtl); }
...
std::string Shape::genShape(char const *name) const
{
    ...
    for (int i = 0, idx = 0; i < (int)nvertsPerFace.size(); ++i)
    {
        ...
        {
            int vert = faceverts[idx + j] + 1, uv = (int)faceuvs.size() > 0 ? faceuvs[idx + j] + 1 : vert, normal = (int)facenormals.size() > 0 ? facenormals[idx + j] + 1 : vert;
            sh << vert << "/" << uv << "/" << normal << " ";
        }
        sh << "\\n\"\n";
        idx += nvertsPerFace[i];
