...
inline Scheme ConvertSdcTypeToShapeScheme(OpenSubdiv::Sdc::SchemeType sdcScheme)
{
...
inline OpenSubdiv::Sdc::SchemeType ConvertShapeSchemeToSdcType(Scheme shapeScheme)
{
...
inline OpenSubdiv::Sdc::SchemeType GetSdcType(Shape const &shape) { return ConvertShapeSchemeToSdcType(shape.scheme); }
    ...
    for (int i = 0; i < (int)shape.tags.size(); ++i)
    {
        ...
        if (t->name == "interpolateboundary")
        {
            ...
            case 0:
                ...
                break;
            case 1:
                result.SetVtxBoundaryInterpolation(Options::VTX_BOUNDARY_EDGE_AND_CORNER);
                break;
            case 2:
                result.SetVtxBoundaryInterpolation(Options::VTX_BOUNDARY_EDGE_ONLY);
                break;
...
void InterpolateFVarData(OpenSubdiv::Far::TopologyRefiner &refiner, Shape const &shape, std::vector<float> &fvarData);

//------------------------------------------------------------------------------

template <class T> OpenSubdiv::Far::TopologyRefiner *InterpolateFarVertexData(Shape const &shape, int maxlevel, std::vector<T> &data)
{
    ...
    // Far interpolation
    FarTopologyRefiner *refiner = FarTopologyRefinerFactory::Create(shape, FarTopologyRefinerFactory::Options(GetSdcType(shape), GetSdcOptions(shape)));
    assert(refiner);
    ...
    {
        data[i].SetPosition(shape.verts[i * 3 + 0], shape.verts[i * 3 + 1], shape.verts[i * 3 + 2]);
    }
...
template <class T> OpenSubdiv::Far::TopologyRefiner *InterpolateFarVertexData(const char *shapeStr, Scheme scheme, int maxlevel, std::vector<T> &data)
{

    Shape const *shape = Shape::parseObj(shapeStr, scheme);

    OpenSubdiv::Far::TopologyRefiner *refiner = InterpolateFarVertexData(*shape, maxlevel, data);
...
template <> inline bool TopologyRefinerFactory<Shape>::resizeComponentTopology(Far::TopologyRefiner &refiner, Shape const &shape)
{
...
//----------------------------------------------------------
template <> inline bool TopologyRefinerFactory<Shape>::assignComponentTopology(Far::TopologyRefiner &refiner, Shape const &shape)
{
...
//----------------------------------------------------------
template <> inline bool TopologyRefinerFactory<Shape>::assignFaceVaryingTopology(Far::TopologyRefiner &refiner, Shape const &shape)
{
    ...
    if (!shape.faceuvs.empty())
    {
        ...
        int nfaces = getNumBaseFaces(refiner), channel = createBaseFVarChannel(refiner, (int)shape.uvs.size() / 2);
...
//----------------------------------------------------------
template <> inline bool TopologyRefinerFactory<Shape>::assignComponentTags(Far::TopologyRefiner &refiner, Shape const &shape)
{
    ...
    for (int i = 0; i < (int)shape.tags.size(); ++i)
    {
        ...
        if (t->name == "crease")
        {
            ...
            for (int j = 0; j < (int)t->intargs.size() - 1; j += 2)
            {
                ...
                else
                {
                    ...
                    setBaseEdgeSharpness(refiner, edge, std::max(0.0f, ((nfloat > 1) ? t->floatargs[j] : t->floatargs[0])));
                }
            }
        }
        else if (t->name == "corner")
        {
            ...
            for (int j = 0; j < (int)t->intargs.size(); ++j)
            {
                ...
                {
                    int nfloat = (int)t->floatargs.size();
                    setBaseVertexSharpness(refiner, vertex, std::max(0.0f, ((nfloat > 1) ? t->floatargs[j] : t->floatargs[0])));
                }
            }
        }
    }
...
template <> inline void TopologyRefinerFactory<Shape>::reportInvalidTopology(TopologyRefinerFactory::TopologyError /* errCode */, char const *msg, Shape const & /* shape */) { Warning(msg); }
