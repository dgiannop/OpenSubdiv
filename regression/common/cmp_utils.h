...
namespace
{
template <class Face, class Edge, class Vertex> struct LevelMapT
{
...
template <class T> void GetReorderedHbrVertexData(const OpenSubdiv::Far::TopologyRefiner &farRefiner, const OpenSubdiv::HbrMesh<T> &hbrMesh, std::vector<T> *hbrVertexData, std::vector<bool> *hbrVertexOnBoundaryData = NULL)
{
    ...
    struct Mapper
    {
        ...
        Mapper(const OpenSubdiv::Far::TopologyRefiner &refiner, const OpenSubdiv::HbrMesh<T> &hmesh)
        {
            ...
            { // Populate base level
                ...
                int nfaces = refBaseLevel.GetNumFaces(), nedges = refBaseLevel.GetNumEdges(), nverts = refBaseLevel.GetNumVertices();
                ...
                for (int edge = 0; edge < nedges; ++edge)
                {
                    ...
                    Hvertex const *v0 = hmesh.GetVertex(farVerts[0]), *v1 = hmesh.GetVertex(farVerts[1]);
            ...
            for (int level = 1, ecount = 0; level <= refiner.GetMaxLevel(); ++level)
            {
                ...
                LevelMap &previous = maps[level - 1], &current = maps[level];
                ...
                for (int edge = 0; edge < refLevel.GetNumEdges(); ++edge)
                {
                    ...
                    Hvertex const *v0 = current.verts[farVerts[0]], *v1 = current.verts[farVerts[1]];
                    assert(v0 && v1);
