...
//------------------------------------------------------------------------------
template <class T> void applyTags(OpenSubdiv::HbrMesh<T> *mesh, Shape const *sh)
{
    ...
    for (int i = 0; i < (int)sh->tags.size(); ++i)
    {
        ...
        if (t->name == "crease")
        {
            ...
            {
                OpenSubdiv::HbrVertex<T> *  v = mesh->GetVertex(t->intargs[j]), *w = mesh->GetVertex(t->intargs[j + 1]);
        ...
        else if (t->name == "facevaryinginterpolateboundary")
        {
            ...
            case 3:
                ...
                break;
            default:
                printf("unknown facevarying interpolate boundary : %d\n", t->intargs[0]);
                break;
            }
        }
        ...
        else if (t->name == "smoothtriangles")
        {
            ...
            OpenSubdiv::HbrCatmarkSubdivision<T> *scheme = dynamic_cast<OpenSubdiv::HbrCatmarkSubdivision<T> *>(mesh->GetSubdivision());
            ...
            if (t->intargs[0] == 1)
                scheme->SetTriangleSubdivisionMethod(OpenSubdiv::HbrCatmarkSubdivision<T>::k_Old);
            else if (t->intargs[0] == 2)
                scheme->SetTriangleSubdivisionMethod(OpenSubdiv::HbrCatmarkSubdivision<T>::k_New);
            else
        ...
        else if (t->name == "creasemethod")
        {
            ...
            if (t->stringargs[0] == "normal")
                scheme->SetCreaseSubdivisionMethod(OpenSubdiv::HbrSubdivision<T>::k_CreaseNormal);
            else if (t->stringargs[0] == "chaikin")
                scheme->SetCreaseSubdivisionMethod(OpenSubdiv::HbrSubdivision<T>::k_CreaseChaikin);
            else
            ...
            for (int j = 0; j < nops; ++j)
            {
                ...
                for (int k = 0; k < (int)t->intargs.size();)
                {
                    ...
                    if (opnames[j] != "sharpness")
                    {
                        ...
                        OpenSubdiv::HbrVertexEdit<T> *edit = new OpenSubdiv::HbrVertexEdit<T>(faceid, nsubfaces, subfaces, vertexid, vvindex[j], floatwidths[j], isP[j], opmodifiers[j], xformed);
                    ...
                    else
                    {
                        ...
                        {
                            OpenSubdiv::HbrCornerEdit<T> *edit = new OpenSubdiv::HbrCornerEdit<T>(faceid, nsubfaces, subfaces, vertexid, opmodifiers[j], t->floatargs[floatidx]);
                            mesh->AddHierarchicalEdit(edit);
                        }
                        else
                        {
                            OpenSubdiv::HbrCreaseEdit<T> *edit = new OpenSubdiv::HbrCreaseEdit<T>(faceid, nsubfaces, subfaces, vertexid, opmodifiers[j], t->floatargs[floatidx]);
                            mesh->AddHierarchicalEdit(edit);
                        }
                    }
...
//------------------------------------------------------------------------------
template <class T> std::string hbrToObj(OpenSubdiv::HbrMesh<T> *mesh)
{
...
//------------------------------------------------------------------------------
template <class T> OpenSubdiv::HbrMesh<T> *createMesh(Scheme scheme = kCatmark, int fvarwidth = 0)
{
    ...
    static int indices[2] = {0, 1}, widths[2] = {1, 1};

    int const fvarcount = fvarwidth > 0 ? 2 : 0, *fvarindices = fvarwidth > 0 ? indices : NULL, *fvarwidths = fvarwidth > 0 ? widths : NULL;

    switch (scheme)
    {
    case kBilinear:
        mesh = new OpenSubdiv::HbrMesh<T>(&_bilinear, fvarcount, fvarindices, fvarwidths, fvarwidth);
        break;

    case kLoop:
        mesh = new OpenSubdiv::HbrMesh<T>(&_loop, fvarcount, fvarindices, fvarwidths, fvarwidth);
        break;

    case kCatmark:
        mesh = new OpenSubdiv::HbrMesh<T>(&_catmark, fvarcount, fvarindices, fvarwidths, fvarwidth);
        break;
    }
...
//------------------------------------------------------------------------------
template <class T> void createVerticesWithPositions(Shape const *sh, OpenSubdiv::HbrMesh<T> *mesh)
{
...
//------------------------------------------------------------------------------
template <class T> void createVertices(Shape const *sh, OpenSubdiv::HbrMesh<T> *mesh)
{
...
//------------------------------------------------------------------------------
template <class T> void copyVertexPositions(Shape const *sh, OpenSubdiv::HbrMesh<T> *mesh, std::vector<float> &verts)
{
...
//------------------------------------------------------------------------------
template <class T> void createTopology(Shape const *sh, OpenSubdiv::HbrMesh<T> *mesh, Scheme scheme)
{
    ...
    mesh->GetSubdivision()->SetCreaseSubdivisionMethod(OpenSubdiv::HbrSubdivision<T>::k_CreaseNormal);

    if (OpenSubdiv::HbrCatmarkSubdivision<T> *hscheme = dynamic_cast<OpenSubdiv::HbrCatmarkSubdivision<T> *>(mesh->GetSubdivision()))
    {

        hscheme->SetTriangleSubdivisionMethod(OpenSubdiv::HbrCatmarkSubdivision<T>::k_Normal);
    }
...
//------------------------------------------------------------------------------
template <class T> void createFaceVaryingUV(Shape const *sh, OpenSubdiv::HbrMesh<T> *mesh)
{
...
//------------------------------------------------------------------------------
template <class T> OpenSubdiv::HbrMesh<T> *simpleHbr(Shape const *sh, std::vector<float> *verts = 0, bool fvar = false)
{
...
template <class T> OpenSubdiv::HbrMesh<T> *simpleHbr(char const *Shapestr, Scheme scheme, std::vector<float> *verts = 0, bool fvar = false)
{
...
//------------------------------------------------------------------------------
template <class T> OpenSubdiv::HbrMesh<T> *simpleHbr(Shape const *sh, std::vector<float> &verts, bool fvar = false)
{
...
template <class T> OpenSubdiv::HbrMesh<T> *simpleHbr(char const *Shapestr, Scheme scheme, std::vector<float> &verts, bool fvar = false)
{
...
//------------------------------------------------------------------------------
template <class T> OpenSubdiv::HbrMesh<T> *interpolateHbrVertexData(Shape const *sh, int maxlevel)
{
...
template <class T> OpenSubdiv::HbrMesh<T> *interpolateHbrVertexData(char const *Shapestr, Scheme scheme, int maxlevel)
{
...
template <class T> bool hbrVertexOnBoundary(const OpenSubdiv::HbrVertex<T> *v)
{
    if (!v)
        return false;
    ...
    else
    {
        ...
        if (pe)
        {
            return hbrVertexOnBoundary(pe->GetOrgVertex()) || hbrVertexOnBoundary(pe->GetDestVertex());
        }
        else
        {
