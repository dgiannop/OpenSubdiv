...
class ObjWriter
{
  public:
    ...
    void WriteVertexNormals(std::vector<float> const &du, std::vector<float> const &dv);
    void WriteVertexUVs(std::vector<float> const &uv);

    void WriteFaces(std::vector<int> const &faceVertices, int faceSize, bool writeNormalIndices = false, bool writeUVIndices = false);
...
//
ObjWriter::ObjWriter(std::string const &filename) : _fptr(0), _numVertices(0), _numNormals(0), _numUVs(0), _numFaces(0)
{
...
void ObjWriter::WriteVertexPositions(std::vector<float> const &pos, int dim)
{
...
void ObjWriter::getNormal(float N[3], float const du[3], float const dv[3]) const
{
...
void ObjWriter::WriteVertexNormals(std::vector<float> const &du, std::vector<float> const &dv)
{
...
void ObjWriter::WriteVertexUVs(std::vector<float> const &uv)
{
...
void ObjWriter::WriteFaces(std::vector<int> const &faceVertices, int faceSize, bool includeNormalIndices, bool includeUVIndices)
{
...
void ObjWriter::WriteGroupName(char const *prefix, int index) { fprintf(_fptr, "g %s%d\n", prefix ? prefix : "", index); }
