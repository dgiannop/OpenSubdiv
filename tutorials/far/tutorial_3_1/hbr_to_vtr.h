...
inline bool compareType(std::type_info const &t1, std::type_info const &t2)
{
...
//
template <class T> static OpenSubdiv::SdcType getSdcOptions(OpenSubdiv::HbrMesh<T> const &mesh, OpenSubdiv::SdcOptions *options)
{
    ...
    else if (compareType(typeid(*subdivision), typeid(OpenSubdiv::HbrCatmarkSubdivision<T>)))
    {
        ...
        {
        case HCatmarkSubdiv::k_Normal:
            tris = SdcOptions::TRI_SUB_NORMAL;
            break;
        case HCatmarkSubdiv::k_Old:
            tris = SdcOptions::TRI_SUB_OLD;
            break;
        case HCatmarkSubdiv::k_New:
            tris = SdcOptions::TRI_SUB_NEW;
            break;
        }
    }
    ...
    {
    case HMesh::k_InterpolateBoundaryNone:
        vvbi = SdcOptions::VTX_BOUNDARY_NONE;
        break;
    case HMesh::k_InterpolateBoundaryEdgeOnly:
        vvbi = SdcOptions::VTX_BOUNDARY_EDGE_ONLY;
        break;
    case HMesh::k_InterpolateBoundaryEdgeAndCorner:
        vvbi = SdcOptions::VTX_BOUNDARY_EDGE_AND_CORNER;
        break;
    default:
        assert(0);
    }
    ...
    {
    case HMesh::k_InterpolateBoundaryNone:
        fvbi = SdcOptions::FVAR_BOUNDARY_BILINEAR;
        break;
    case HMesh::k_InterpolateBoundaryEdgeOnly:
        fvbi = SdcOptions::FVAR_BOUNDARY_EDGE_ONLY;
        break;
    case HMesh::k_InterpolateBoundaryEdgeAndCorner:
        fvbi = SdcOptions::FVAR_BOUNDARY_EDGE_AND_CORNER;
        break;
    case HMesh::k_InterpolateBoundaryAlwaysSharp:
        fvbi = SdcOptions::FVAR_BOUNDARY_ALWAYS_SHARP;
        break;
    default:
        assert(0);
    }
    ...
    {
    case HSubdiv::k_CreaseNormal:
        creaseMethod = OpenSubdiv::SdcOptions::CREASE_UNIFORM;
        break;
    case HSubdiv::k_CreaseChaikin:
        creaseMethod = OpenSubdiv::SdcOptions::CREASE_CHAIKIN;
        break;
    };
...
template <class T> class HbrConverter
{
  ...
  public:
    ...
    OpenSubdiv::SdcType const &GetType() const { return _type; }

    // Returns subdivision options
    OpenSubdiv::SdcOptions const &GetOptions() const { return _options; }

    // The HbrMesh being converted
    OpenSubdiv::HbrMesh<T> const &GetHbrMesh() const { return _hmesh; }
...
template <> void FarTopologyRefinerFactory<OsdHbrConverter>::resizeComponentTopology(FarTopologyRefiner &refiner, OsdHbrConverter const &conv)
{

    OsdHbrMesh const &hmesh = conv.GetHbrMesh();

    int nfaces = hmesh.GetNumFaces(), nverts = hmesh.GetNumVertices();
    ...
    for (int i = 0; i < nverts; ++i)
    {
        ...
        class GatherOperator : public OpenSubdiv::HbrHalfedgeOperator<OpenSubdiv::OsdVertex>
        {
          ...
          public:
            int vertEdgeCount, vertFaceCount;
...
template <> void FarTopologyRefinerFactory<OsdHbrConverter>::assignComponentTopology(FarTopologyRefiner &refiner, OsdHbrConverter const &conv)
{
    ...
    { // Vert relations
        for (int i = 0; i < getNumBaseVertices(refiner); ++i)
        {
            ...
            class GatherOperator : public OpenSubdiv::HbrHalfedgeOperator<OpenSubdiv::OsdVertex>
            {
                ...
                Index *_dstVertFaces, *_dstVertEdges;

                LocalIndex *_dstVertInFaceIndices, *_dstVertInEdgeIndices;

              public:
                GatherOperator(FarTopologyRefiner &refiner, OsdHbrConverter const &conv, OsdHbrVertex const *v, int idx) : _conv(conv), _v(v)
                {

                    _dstVertFaces = getBaseVertexFaces(refiner, idx).begin(), _dstVertEdges = getBaseVertexEdges(refiner, idx).begin();

                    _dstVertInFaceIndices = getBaseVertexFaceLocalIndices(refiner, idx).begin(), _dstVertInEdgeIndices = getBaseVertexEdgeLocalIndices(refiner, idx).begin();
                }
...
template <> void FarTopologyRefinerFactory<OsdHbrConverter>::assignComponentTags(FarTopologyRefiner &refiner, OsdHbrConverter const &conv)
{
